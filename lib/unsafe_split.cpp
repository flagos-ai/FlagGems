...
std::vector<at::Tensor> unsafe_split(const at::Tensor& self, c10::SymInt split_size, int64_t dim) {
  ...
  if (dim_size == 0) {
    ...
    outs.reserve(1);
    at::Tensor out = split_view(self, sizes, strides, dim, 0, 0, base_storage_offset, dim_stride);
  ...
  for (int64_t i = 0; i < full_splits; ++i, start += split_size_int) {
    at::Tensor out =
        split_view(self, sizes, strides, dim, start, split_size_int, base_storage_offset, dim_stride);
    reset_version_counter(out, reset_version);
    outs.emplace_back(std::move(out));
  }
  if (tail_size != 0) {
    at::Tensor out = split_view(self, sizes, strides, dim, start, tail_size, base_storage_offset, dim_stride);
    reset_version_counter(out, reset_version);
    outs.emplace_back(std::move(out));
  }
  return outs;
...
std::vector<at::Tensor> unsafe_split_with_sizes(const at::Tensor& self,
  ...
  for (const c10::SymInt& split_size : split_sizes) {
    ...
    TORCH_CHECK(length >= 0, "split_with_sizes expects split_sizes have only non-negative entries");
    at::Tensor out = split_view(self, sizes, strides, dim, start, length, base_storage_offset, dim_stride);
    reset_version_counter(out, reset_version);
    outs.emplace_back(std::move(out));
    start += length;
  }
