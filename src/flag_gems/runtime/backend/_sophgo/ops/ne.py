...
def _ne_tensor_contig_kernel(
    ...
    # Keep the physical launch grid within SG2260's 64-program limit. Each
    # program processes additional tiles locally when the tensor is larger.
...
def _ne_scalar_contig_kernel(
    ...
    for tile_id in range(pid, total_tiles, GRID_SIZE):
        ...
        if DIVISIBLE_N:
            a = tl.load(a_ptr + offsets).to(tl.float32)
            tl.store(out_ptr + offsets, a != value.to(tl.float32))
        else:
            mask = offsets < n_elements
            a = tl.load(a_ptr + offsets, mask=mask, other=0.0).to(tl.float32)
            tl.store(out_ptr + offsets, a != value.to(tl.float32), mask=mask)


# These are semantic fallbacks for broadcasting, non-contiguous tensors and
# dtypes that are not covered by the contiguous kernels.
@pointwise_dynamic(promotion_methods=[(0, 1, "ALWAYS_BOOL")], config=_small_config)
@triton.jit
def _ne_func_small(x, y):
    return x.to(tl.float32) != y.to(tl.float32)


@pointwise_dynamic(promotion_methods=[(0, 1, "ALWAYS_BOOL")], config=_large_config)
@triton.jit
def _ne_func_large(x, y):
    return x.to(tl.float32) != y.to(tl.float32)
...
def _launch_tensor_fast_path(a, b):
    ...
    with torch_device_fn.device(a.device):
        _ne_tensor_contig_kernel[(grid_size,)](
            a,
            b,
            out,
            n_elements,
...
def _launch_scalar_fast_path(a, value):
    ...
    grid_size = _launch_grid(n_elements, block_size)
    with torch_device_fn.device(a.device):
        _ne_scalar_contig_kernel[(grid_size,)](
            a,
            value,
            out,
            n_elements,
...
def ne(A, B):
    ...
    if A.numel() <= _SMALL_BLOCK_SIZE:
        return _ne_func_small(A, B)
    return _ne_func_large(A, B)


def ne_scalar(A, B):
    logger.debug("SOPHGO GEMS NE SCALAR")
    if _can_use_scalar_fast_path(A, B):
        return _launch_scalar_fast_path(A, B)
    if A.numel() <= _SMALL_BLOCK_SIZE:
        return _ne_func_scalar_small(A, B)
    return _ne_func_scalar_large(A, B)
