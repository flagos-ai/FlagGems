...
def block_cumsum_kernel(
    ...
    gridx = tle.program_id(0).to(tl.int32)
    gridy = tle.program_id(1).to(tl.int32)
...
def normed_cumsum(inp, dim=-1):
    ...
    with torch_device_fn.device(inp.device.index):
        ...
        elif isinstance(device_props, dict):
            # For cases returning a dict (like triton), use a fixed value
            num_sms = device_props.get("multiprocessor_count", 64) or 64
        else:
            num_sms = 64  # default value
        TILE = 1024
