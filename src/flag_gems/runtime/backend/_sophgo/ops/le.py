...
@libentry()
@triton.jit
def _le_tensor_contiguous_kernel(
    a_ptr,
    b_ptr,
    out_ptr,
    n_elements,
...
@libentry()
@triton.jit
def _le_scalar_contiguous_kernel(
    a_ptr,
    value,
    out_ptr,
    n_elements,
...
def _launch_tensor_fast_path(a, b):
    ...
    with torch_device_fn.device(a.device):
        _le_tensor_contiguous_kernel[(grid_size,)](
            a,
            b,
            out,
            n_elements,
...
def _launch_scalar_fast_path(a, value):
    ...
    grid_size = _launch_grid(n_elements, block_size)
    with torch_device_fn.device(a.device):
        _le_scalar_contiguous_kernel[(grid_size,)](
            a,
            value,
            out,
            n_elements,
...
def le(A, B):
    ...
    if A.numel() <= _SMALL_BLOCK_SIZE:
        return _le_func_small(A, B)
    return _le_func_large(A, B)


def le_scalar(A, B):
    logger.debug("SOPHGO GEMS LE SCALAR")
    if _can_use_scalar_fast_path(A, B):
        return _launch_scalar_fast_path(A, B)
    if A.numel() <= _SMALL_BLOCK_SIZE:
        return _le_func_scalar_small(A, B)
    return _le_func_scalar_large(A, B)
