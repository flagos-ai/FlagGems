...
def _gen_index_decomp(code: IndentedBuffer, rank: int, flat_start_expr: str) -> None:
    """Scalar i64 tile-start decomposition + i32 vector carry-propagation.

    Strategy: split flat index into (scalar tile-start) + (vector lane offset).
    - flat_base = flat_start_expr  →  scalar, computed once per tile in i64
    ...
    code.writeline("# i32 vector carry propagation (values <= dim_size + tile_size)")
    if rank == 1:
