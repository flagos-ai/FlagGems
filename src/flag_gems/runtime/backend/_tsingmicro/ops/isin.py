...
def isin_by_comparation(
    ...
    if M <= 1024:
        ...
        BLOCK_M, BLOCK_N, num_warps = launch_arg(4, 128, N, 1)
    ctas_num = min(16 // num_warps, triton.cdiv(M, BLOCK_M))
...
def isin_by_search(
    ...
    if M <= 1048576:  # 2 ** 20 = 1024 * 1024
        _, BLOCK_M, num_warps = launch_arg(None, 512, M, 1)
    elif M <= 4194304:  # 2 ** 22 = 1024 * 4096
        _, BLOCK_M, num_warps = launch_arg(None, 1024, M, 1)
    elif M <= 8388608:  # 2 ** 23 = 1024 * 8192
        _, BLOCK_M, num_warps = launch_arg(None, 2048, M, 1)
    elif M <= 268435456:  # 2 ** 28 = 1024 * 262144
        _, BLOCK_M, num_warps = launch_arg(None, 4096, M, 1)
    else:
        _, BLOCK_M, num_warps = launch_arg(None, 2048, M, 1)
