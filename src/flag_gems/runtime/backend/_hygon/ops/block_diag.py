...
@libentry()
@triton.jit
def block_diag_fused_strided_kernel(
    out_ptr,
    base_ptr,
    input_stride,
    ...
    """Initialize the output and copy regularly spaced diagonal blocks."""
    offs = ext.program_id(0) * BLOCK_SIZE + tl.arange(0, BLOCK_SIZE)
    out_mask = offs < OUT_NUMEL
    row = offs // TOTAL_COLS
    col = offs % TOTAL_COLS
...
@libentry()
@triton.jit
def block_diag_fused_ptrargs_kernel(
    out_ptr,
    p0,
    p1,
    p2,
    ...
    """Initialize the output and copy diagonal blocks from scalar pointers."""
    offs = ext.program_id(0) * BLOCK_SIZE + tl.arange(0, BLOCK_SIZE)
    out_mask = offs < OUT_NUMEL
    row = offs // TOTAL_COLS
    col = offs % TOTAL_COLS
    ...
    ptr_val = tl.where(row_block == 0, p0, 0)
    ptr_val = tl.where(row_block == 1, p1, ptr_val)
    ptr_val = tl.where(row_block == 2, p2, ptr_val)
    ptr_val = tl.where(row_block == 3, p3, ptr_val)
    if NUM_BLOCKS > 4:
        ptr_val = tl.where(row_block == 4, p4, ptr_val)
        ptr_val = tl.where(row_block == 5, p5, ptr_val)
        ptr_val = tl.where(row_block == 6, p6, ptr_val)
        ptr_val = tl.where(row_block == 7, p7, ptr_val)
    if NUM_BLOCKS > 8:
        ptr_val = tl.where(row_block == 8, p8, ptr_val)
        ptr_val = tl.where(row_block == 9, p9, ptr_val)
        ptr_val = tl.where(row_block == 10, p10, ptr_val)
        ptr_val = tl.where(row_block == 11, p11, ptr_val)
        ptr_val = tl.where(row_block == 12, p12, ptr_val)
        ptr_val = tl.where(row_block == 13, p13, ptr_val)
        ptr_val = tl.where(row_block == 14, p14, ptr_val)
        ptr_val = tl.where(row_block == 15, p15, ptr_val)
    src_ptr = ptr_val.to(tl.pointer_type(out_ptr.dtype.element_ty))
    src_off = (row % BLOCK_ROWS) * BLOCK_COLS + col % BLOCK_COLS
    value = tl.load(src_ptr + src_off, mask=out_mask & diagonal, other=0.0)
    tl.store(out_ptr + offs, value, mask=out_mask)
...
# Scalar pointer args supported by the fused uniform kernel.
_MAX_PTR_ARGS = 16
...
def block_diag(*tensors):
    ...
    if fast_path:
        ...
        if n == 1:
            ...
            stride = (tensors[1].data_ptr() - base_ptr_val) // elem_bytes
            if n <= 2:
        ...
        fused_path = regular_stride or n <= _MAX_PTR_ARGS
        out_factory = torch.empty if fused_path else torch.zeros
        out = out_factory((total_rows, total_cols), dtype=dtype0, device=device)
        ...
        if fused_path:
            ...
            block_size = 2048 if out_numel > 1024 * 1024 else 1024
            fused_grid = ((out_numel + block_size - 1) // block_size,)
            with torch_device_fn.device(device):
                if regular_stride:
                    block_diag_fused_strided_kernel[fused_grid](
                        out,
                        t0,
                        stride,
                        OUT_NUMEL=out_numel,
                    ...
                    ptr_args = [t.data_ptr() for t in tensors]
                    ptr_args += [0] * (_MAX_PTR_ARGS - n)
                    block_diag_fused_ptrargs_kernel[fused_grid](
                        out,
                        *ptr_args,
                        OUT_NUMEL=out_numel,
                        BLOCK_ROWS=block_rows,
