...
def _thnn_fused_lstm_cell_kernel(
    ...
    # Grid: (batch_size, cdiv(hidden_size, BLOCK_SIZE)) -- the second grid
    # dimension both fixes correctness for hidden_size > BLOCK_SIZE (the old
    # kernel silently dropped elements past BLOCK_SIZE) and parallelizes the
    # hidden dimension for small batches.
    batch_idx = tl.program_id(0)
    hid_blk = tl.program_id(1)
    ...
    offsets = hid_blk * BLOCK_SIZE + tl.arange(0, BLOCK_SIZE)
...
def _thnn_fused_lstm_cell(
    ...
    # BLOCK_SIZE caps at 128 for occupancy; the second grid dimension covers
    # hidden_size > BLOCK_SIZE.
    BLOCK_SIZE = max(32, min(triton.next_power_of_2(hidden_size), 128))

    # Bias pointers are never dereferenced when the corresponding constexpr
    # flag is False (the loads are compiled out), so reuse cx as a dummy to
    # avoid two extra device allocations per call.
    dummy = cx
