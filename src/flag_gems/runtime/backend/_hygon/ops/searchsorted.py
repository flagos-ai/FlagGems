...
def _searchsorted_scalar_kernel(
    sorted_sequence,
    value_ptr,
    sorter,
    out,
    ...
    SCALAR_IS_FLOAT: tl.constexpr,
    SEQUENCE_IS_FLOAT: tl.constexpr,
    SEQUENCE_IS_UINT8: tl.constexpr,
):
    offsets = tl.arange(0, SAMPLE_SIZE)
    if VALUE_IS_TENSOR:
        value = tl.load(value_ptr)
    elif SCALAR_IS_FLOAT and not SEQUENCE_IS_FLOAT:
        value = value_scalar.to(tl.float32)
    else:
        value = value_scalar.to(sorted_sequence.dtype.element_ty)
    if SEQUENCE_IS_UINT8 and not SCALAR_IS_FLOAT:
    ...
    if HAS_SORTER:
        sorted_indices = tl.load(
            sorter + sample_indices * sorter_stride, mask=sample_mask, other=0
        ).to(tl.int32)
    sample_values = tl.load(
        sorted_sequence + sorted_indices * sequence_stride,
        mask=sample_mask,
        other=0,
    )
    if SEQUENCE_IS_UINT8:
        if SCALAR_IS_FLOAT:
            sample_values = sample_values.to(tl.float32)
        else:
            sample_values = sample_values.to(tl.int32)
    sample_go_right = tl.where(
    ...
    while low < high:
        ...
        sorted_index = mid
        if HAS_SORTER:
            sorted_index = tl.load(sorter + mid * sorter_stride).to(tl.int32)
        mid_value = tl.load(sorted_sequence + sorted_index * sequence_stride)
        if SEQUENCE_IS_UINT8:
            if SCALAR_IS_FLOAT:
                mid_value = mid_value.to(tl.float32)
            else:
                mid_value = mid_value.to(tl.int32)
        go_left = tl.where(right, value < mid_value, value <= mid_value)

        high = tl.where(go_left, mid, high)
        low = tl.where(~go_left, mid + 1, low)
...
def _prepare_scalar_out(
    ...
    if out is None:
        return torch.empty_strided(
            (), (), dtype=out_dtype, device=sorted_sequence.device
        )
    if out.dtype != out_dtype:
        raise RuntimeError(
            "torch.searchsorted(): output tensor's dtype is wrong, it can only be "
    ...
    if out.device != sorted_sequence.device:
        raise RuntimeError(
            "torch.searchsorted(): output tensor must be on the same device as input"
        )
...
def _searchsorted_scalar_hygon(
    sorted_sequence: torch.Tensor,
    value,
    *,
    out_int32: bool,
    ...
    if isinstance(value, complex):
        raise NotImplementedError(
            "searchsorted is not implemented for values dtype torch.complex64"
        )

    out = _prepare_scalar_out(sorted_sequence, out_int32, out)
    sequence_len = sorted_sequence.shape[0]
    if sequence_len == 0:
        out.zero_()
        return out
    if sequence_len >= torch.iinfo(torch.int32).max:
        values = torch.scalar_tensor(value, device=sorted_sequence.device)
    ...
    value_is_tensor = sorted_sequence.dtype == torch.float64 and isinstance(
        value, float
    )
    value_ptr = (
        torch.tensor(value, dtype=torch.float64, device=sorted_sequence.device)
        if value_is_tensor
        else sorted_sequence
    )
    sorter_ptr = sorter if sorter is not None else sorted_sequence
    sorter_stride = sorter.stride(0) if sorter is not None else 0

    with torch_device_fn.device(sorted_sequence.device):
        bucket_size = triton.cdiv(sequence_len, _HYGON_SCALAR_SAMPLES)
        _searchsorted_scalar_kernel[(1,)](
            sorted_sequence,
            value_ptr,
            sorter_ptr,
            out,
            value,
            sequence_len,
            sorted_sequence.stride(0),
            sorter_stride,
            bucket_size,
            right,
            SAMPLE_SIZE=_HYGON_SCALAR_SAMPLES,
            HAS_SORTER=sorter is not None,
            VALUE_IS_TENSOR=value_is_tensor,
            SCALAR_IS_FLOAT=isinstance(value, float),
            SEQUENCE_IS_FLOAT=sorted_sequence.dtype.is_floating_point,
            SEQUENCE_IS_UINT8=sorted_sequence.dtype == torch.uint8,
            num_warps=1,
        )
    return out
