...
def scaled_dot_product_attention_forward(
    ...
    logger.debug("GEMS_HYGON SCALED DOT PRODUCT ATTENTION FORWARD")
    # shape constraints
...
def scaled_dot_product_attention_backward(
    ...
    logger.debug("GEMS_HYGON SCALED DOT PRODUCT ATTENTION BACKWARD")
    # shape constraints
    HEAD_DIM_Q, HEAD_DIM_K = query.shape[-1], key.shape[-1]
    # when v is in float8_e5m2 it is transposed.
    HEAD_DIM_V = value.shape[-1]
...
def flash_attention_forward(
    ...
    *,
    ...
    logger.debug("GEMS_HYGON FLASH_ATTENTION_FORWARD")
    assert (
...
def flash_attn_varlen_func(
    ...
    if use_c_extension:
        logger.debug("GEMS_HYGON FLASH_ATTN_VARLEN_FUNC(C EXTENSION)")
        ...
        logger.debug("GEMS_HYGON FLASH_ATTN_VARLEN_FUNC")
        assert (
