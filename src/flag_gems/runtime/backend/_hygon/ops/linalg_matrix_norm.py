"""linalg_matrix_norm -- HYGON (DCU/HCU) override.

SVD ords (2/-2/nuc) use the on-device Gram + two-sided Jacobi path
(k ≥ 3), bypassing the nondeterministic Triton Householder QR.  The
7 shared Triton kernels are imported from
...
def _batched_kernel_dispatch(A, dim, ord_val, out_dtype, keepdim):
    ...
    if math.isinf(abs_ord):
        ...
        # HYGON (HCU): a 16-row tile combined with a 32-column tile (mat_N == 32
        # → blk_dim == 32) produces wrong tl.max reductions on HCU Triton — the
        # same [16, 32] tile bug as the 2D non-tiled path.  Clamp the row tile
        # to ≥ 32 so the kernel always reduces over a full warp-sized dim.
        tile_m = 32
        ...
        # HYGON (HCU): clamp the row tile (BLOCK_M) to ≥ 32 — a 16-row tile
        # combined with a 32-column tile (mat_N == 32 → tile_n == 32) hits the
        # same [16, 32] tile bug as the inf-norm path.
        blk_dim = max(triton.next_power_of_2(min(mat_M, 32)), 32)
        init_val = float("inf") if is_min else 0.0
        result = torch.full((batch,), init_val, dtype=torch.float32, device=Ab.device)
        _dummy = torch.empty(1, device=Ab.device)
        _abs_norm_kernel[(batch, grid_dim)](
...
def _ordinf_norm(A, ord_val, dim, keepdim, dtype):
    ...
    if A.ndim == 2 and d0 == 0 and d1 == 1:
        ...
        # HYGON (HCU): when both dims are < 32 the non-TILED path creates
        # pathological masked loads / wrong reductions; fall back to the
        # FlagGems sum/max ops (mirrors the iluvatar CoreX workaround).
        if M <= 2 or N <= 2 or (M < 32 and N < 32):
        ...
        if grid_m * grid_n >= 512:
            ...
            # HYGON (HCU): clamp tile dims ≥ 32 for correctness.  HCU Triton
            # reductions produce wrong results when a tile dimension is < 32
            # elements (same class of bug as iluvatar CoreX).
            BM = max(BM, 32)
            BN = max(BN, 32)
            grid_m = triton.cdiv(M, BM)
            init_val = float("inf") if is_min else 0.0
...
def _svdvals_hybrid(input):
    ...
    # ---- Gram + on-device Jacobi eigensolver (k ≥ 3) -----------------------
    # A single torch fp64 GEMM (IEEE-accurate, see probe_fp64_precision.py)
    # plus the parallel two-sided Jacobi kernels -- the whole
    # eigendecomposition stays on device, no CPU LAPACK involved.  Hygon's
    # _householder_qr_r_kernel is nondeterministic, so the QR/Jacobi/DBDSQR
    # stages are bypassed entirely (odd k uses a dummy column in the
    # Brent-Luk pairing, PAIR_K = k + 1).
    if k >= 3:
