...
@triton.jit
def _fused_mlp_kernel(x_ptr, gate_packed_ptr, up_packed_ptr, gate_scale_ptr,
                      up_scale_ptr, out_ptr, K: tl.constexpr, N: tl.constexpr):
    # Coarse TLE op: gate GEMV + up GEMV + SWIGLU fused in one kernel launch.
    _tle_fused_mlp(x_ptr, gate_packed_ptr, up_packed_ptr, gate_scale_ptr,
                   up_scale_ptr, out_ptr, K, N)
