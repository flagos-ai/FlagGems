...
def _per_token_group_quant_fp8(
    ...
    if scale_ue8m0:
        ...
        # Avoid subnormal intermediate: y_s = _absmax / fp8_max can be subnormal
        # (flushed to 0 on GPU) when fp8_dtype is float32. Rewrite y / y_s as
        # (y / _absmax) * fp8_max which stays in normal float range because
        # |y / _absmax| <= 1.
...
def _per_token_group_quant_fp8_m2(
    ...
    _absmax0 = tl.maximum(max0, eps)
    _absmax1 = tl.maximum(max1, eps)

    y_s0 = _absmax0 / fp8_max
    y_s1 = _absmax1 / fp8_max
...
def _per_token_group_quant_fp8_m4(
    ...
    _absmax0 = tl.maximum(max0, eps)
    _absmax1 = tl.maximum(max1, eps)
    _absmax2 = tl.maximum(max2, eps)
    _absmax3 = tl.maximum(max3, eps)

    y_s0 = _absmax0 / fp8_max
    y_s1 = _absmax1 / fp8_max
    y_s2 = _absmax2 / fp8_max
    y_s3 = _absmax3 / fp8_max
...
def _per_token_group_quant_fp8_m8(
    ...
    _absmax2 = tl.maximum(max2, eps)
    _absmax3 = tl.maximum(max3, eps)
    _absmax4 = tl.maximum(max4, eps)
    _absmax5 = tl.maximum(max5, eps)
    _absmax6 = tl.maximum(max6, eps)
    _absmax7 = tl.maximum(max7, eps)

    y_s0 = _absmax0 / fp8_max
    y_s1 = _absmax1 / fp8_max
    y_s2 = _absmax2 / fp8_max
    y_s3 = _absmax3 / fp8_max
    y_s4 = _absmax4 / fp8_max
    y_s5 = _absmax5 / fp8_max
    y_s6 = _absmax6 / fp8_max
    y_s7 = _absmax7 / fp8_max
...
def per_token_group_quant_fp8(
    ...
    logger.debug("GEMS ILUVATAR PER_TOKEN_GROUP_QUANT_FP8")
