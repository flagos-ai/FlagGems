...
from .flash_api_v3 import mha_varlan_fwd_v3, is_fa3_supported
...
def flash_attn_varlen_func(
    ...
    scheduler_metadata=None, q_descale=None, k_descale=None,
    v_descale=None, s_aux=None, num_splits=0,
    ...
    if use_c_extension:
        ...
        return _generic(
            ...
            scheduler_metadata, q_descale, k_descale, v_descale, s_aux,
            num_splits, cp_world_size, cp_rank, cp_tot_seqused_k, fa_version,
        )
    ...
    real_window_size = (-1, -1) if window_size is None else (window_size[0], window_size[1])

    q, k, v = [_maybe_contiguous(x) for x in (q, k, v)]
    dummy_cu_seqlens_k = torch.empty_like(cu_seqlens_q)
    max_seqlen_q = max_seqlen_q.item() if hasattr(max_seqlen_q, "item") else max_seqlen_q
    max_seqlen_k = max_seqlen_k.item() if hasattr(max_seqlen_k, "item") else max_seqlen_k
    ...
    out, q, k, v, softmax_lse, *_ = _launcher(
        ...
        max_seqlen_q, max_seqlen_k, dropout_p, softmax_scale, False,
        causal, real_window_size[0], real_window_size[1], softcap,
        return_softmax_lse and dropout_p > 0, None,
    )
