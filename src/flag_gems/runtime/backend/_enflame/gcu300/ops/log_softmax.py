...
def keep(conf):
    ...
    if BLOCK_M * BLOCK_N / conf.num_warps > 256 * 512:
        return False
    if BLOCK_M * BLOCK_N / conf.num_warps < 4096:
        return False
    if conf.num_warps > 1:
        return False
    return True
...
@libentry()
@libtuner(
    configs=list(filter(keep, runtime.get_tuned_config("log_softmax"))),
    key=["M", "N"],
)
@triton.jit
def log_softmax_kernel(
    ...
    for tile_k_id in tl.range(pid_k, K, 48):
        # TODO(chenfeiyu): consider float64 add add a utility function to get accumulator type
        zero = tl.full([BLOCK_M, BLOCK_N], value=0.0, dtype=tl.float32)
        one = tl.full([BLOCK_M, BLOCK_N], value=1, dtype=tl.float32)
        n_offset0 = tl.arange(0, BLOCK_N)
        offset0 = m_offset[:, None] * N * K + n_offset0[None, :] * K + tile_k_id
        mask0 = m_offset[:, None] < M and n_offset0[None, :] < N
        ...
        m = m_reduced
        mid = m[:, None] + tl.log(z[:, None])
        for start_n in tl.range(0, N, BLOCK_N, num_stages=num_stages):
            n_offset = start_n + tl.arange(0, BLOCK_N)
            offset = m_offset[:, None] * N * K + n_offset[None, :] * K + tile_k_id
            mask = m_offset[:, None] < M and n_offset[None, :] < N
            input_ptrs = input_ptr + offset
