...
def special_legendre_polynomial_p_forward(x, n):
    ...
    # GCU300: use tl.range (hardware loop), NOT tl.static_range — static
    # unrolling of 254 recurrence steps produces a ~9 MB kernel that the GCU
    # driver cannot load ("loadSegments alloc mem failed"). tl.range keeps the
    # code tiny; masked updates keep P frozen once i > n, so results are
    # unchanged.
    for i in tl.range(2, 256):
        # Only compute if n >= i
        mask = n_val >= i
        p_curr = tl.where(
            mask,
...
def special_legendre_polynomial_p(x: torch.Tensor, n) -> torch.Tensor:
    logger.debug("GEMS_ENFLAME SPECIAL_LEGENDRE_POLYNOMIAL_P")
    ...
    #
    # GCU300: keep n as int32. torch passes n as int64 (scalar -> tensor or an
    # int64 tensor overload); building/keeping it int32 avoids any int64 data in
    # the kernel (degree values are tiny, well within int32). No ENABLE_I64 or
    # TORCH_GCU_ENABLE_INT64_AND_UINT64 needed.
