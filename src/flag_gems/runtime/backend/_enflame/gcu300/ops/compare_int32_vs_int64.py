...
def compare_results(in0, return_inverse=True, return_counts=True, verbose=True):
    ...
    try:
        ...
        results['int32'] = {'out': out32, 'inverse': inv32, 'counts': cnt32, 'error': None}
    except Exception as e:
        results['int32'] = {'out': None, 'inverse': None, 'counts': None, 'error': str(e)}
    ...
    try:
        ...
        results['int64'] = {'out': out64, 'inverse': inv64, 'counts': cnt64, 'error': None}
    except Exception as e:
        results['int64'] = {'out': None, 'inverse': None, 'counts': None, 'error': str(e)}
    ...
    try:
        ...
        results['pytorch'] = {'out': out_pt, 'inverse': inv_pt, 'counts': cnt_pt, 'error': None}
    except Exception as e:
        results['pytorch'] = {'out': None, 'inverse': None, 'counts': None, 'error': str(e)}
...
def analyze_differences(results, in0, verbose=True):
    ...
    if out32.numel() != out_pt.numel():
        issues.append(f"int32 unique 数量不匹配: {out32.numel()} vs pytorch {out_pt.numel()}")
    if out64.numel() != out_pt.numel():
        issues.append(f"int64 unique 数量不匹配: {out64.numel()} vs pytorch {out_pt.numel()}")
    ...
    if not torch.equal(out32, out_pt):
        ...
        if verbose and diff <= 10:
            ...
            for i in idx[:5]:
                print(f"    位置 {i}: int32={out32[i].item()}, pytorch={out_pt[i].item()}")
    ...
    if inv32 is not None and inv_pt is not None:
        ...
        if inv32_flat.min().item() < 0:
            issues.append(f"★ int32 inverse_indices 有负值（可能溢出）: min={inv32_flat.min().item()} ★")
            ...
            if verbose:
                ...
                if len(mismatch_idx) > 0:
                    ...
                    for idx in mismatch_idx[:5]:
                        print(f"      位置 {idx}: int32={inv32_flat[idx].item()}, int64={inv64_flat[idx].item()}")
    ...
    if cnt32 is not None and cnt_pt is not None:
        print(f"  counts 范围:")
        print(f"    int32: [{cnt32.min().item()}, {cnt32.max().item()}], sum={cnt32.sum().item()}")
        if cnt64 is not None:
            print(f"    int64: [{cnt64.min().item()}, {cnt64.max().item()}], sum={cnt64.sum().item()}")
        print(f"    pytorch: [{cnt_pt.min().item()}, {cnt_pt.max().item()}], sum={cnt_pt.sum().item()}")
        
        expected_sum = in0.numel()
        if cnt32.sum().item() != expected_sum:
            issues.append(f"★ int32 counts 总和错误: {cnt32.sum().item()} vs 期望 {expected_sum} ★")
