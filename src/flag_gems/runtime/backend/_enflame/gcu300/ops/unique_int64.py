"""
unique_int64.py - 将所有 int32 改为 int64 的版本
用于对比验证 int32 是否导致结果偏差
"""
...
@libentry()
@triton.jit
def simple_unique_flat_kernel_int64(
...
@triton.jit
def output_counts_flat_impl_int64(
    global_pid,
    idx_ptr: tl.tensor,
...
@libentry()
@triton.jit
def output_counts_flat_kernel_int64(
    idx_ptr: tl.tensor,
    origin_num_tasks: int,  # in
    counts_ptr: tl.tensor,  # out
    num_tasks: int,
...
@triton.jit
def quick_output_flat_impl_int64(
    global_pid,
    sorted_data_ptr: tl.tensor,
    idx_ptr: tl.tensor,
    origin_num_tasks: int,  # in
...
@libentry()
@triton.jit
def quick_output_flat_kernel_int64(
    sorted_data_ptr: tl.tensor,
    idx_ptr: tl.tensor,
    origin_num_tasks: int,  # in
    data_out_ptr: tl.tensor,
...
@triton.jit
def local_quick_unique_flat_impl_int64(
    global_pid,
...
@libentry()
@triton.jit
def local_quick_unique_flat_kernel_int64(
    sorted_data_ptr: tl.tensor,  # in
    local_unique_ptr: tl.tensor,
    origin_idx_ptr: tl.tensor,
    tile_sum_ptr: tl.tensor,  # out
    ...
    for j in range(0, tiles_per_cta):
        global_pid = pid + j * ctas_num
        local_quick_unique_flat_impl_int64(
            global_pid,
            sorted_data_ptr,  # in
            local_unique_ptr,
            origin_idx_ptr,
...
@triton.jit
def global_quick_unique_flat_impl_int64(
    global_pid,
    total,
    local_unique_ptr: tl.tensor,
    origin_idx_ptr: tl.tensor,
...
@libentry()
@triton.jit
def global_quick_unique_flat_kernel_int64(
    local_unique_ptr: tl.tensor,
    origin_idx_ptr: tl.tensor,
    tile_sum_ptr: tl.tensor,  # in
    data_out_ptr: tl.tensor,
    ...
    if one_tile_per_cta:  # monolitic kernel style
        global_quick_unique_flat_impl_int64(
            pid,
            0,
            local_unique_ptr,
            origin_idx_ptr,
        ...
        for j in range(0, tiles_per_cta):
            global_pid = pid + j * ctas_num
            total = global_quick_unique_flat_impl_int64(
                global_pid,
                total,
                local_unique_ptr,
                origin_idx_ptr,
...
def sorted_quick_unique_flat_int64(sorted_data: torch.Tensor, return_counts: bool):
    ...
    if return_counts:
        local_unique = None
        origin_idx = torch.empty_like(sorted_data, dtype=torch.int64)  # int32 -> int64
    ...
    with torch_device_fn.device(sorted_data.device.index):
        local_quick_unique_flat_kernel_int64[grid](
            sorted_data,  # in
            local_unique,
            origin_idx,
            tile_sum,  # out
        ...
        global_quick_unique_flat_kernel_int64[grid](
            local_unique,
            origin_idx,
            tile_sum,  # in
            data_out,
        ...
        if return_counts:
            ...
            quick_output_flat_kernel_int64[grid](
                sorted_data,
                idx,
                num_tasks,  # in
                data_out,
...
@libentry()
@triton.jit
def local_ne_flat_kernel_int64(
    sorted_data_ptr: tl.tensor,  # in
    ne_result_ptr: tl.tensor,
    tile_sum_ptr: tl.tensor,  # out
    global_ctas_num: int,
...
@triton.jit
def global_cumsum_flat_impl_int64(
    global_pid,
    total,
    ne_result_ptr: tl.tensor,
    tile_sum_ptr: tl.tensor,  # in
    ...
    # 关键修改：int32 -> int64
    ne_result = ne_result.to(tl.int64)
    cumsum = tl.cumsum(ne_result)
...
@libentry()
@triton.jit
def global_cumsum_flat_kernel_int64(
    ne_result_ptr: tl.tensor,
    tile_sum_ptr: tl.tensor,  # in
    sorted_data_ptr: tl.tensor,
    sorted_indices_ptr: tl.tensor,  # in
    ...
    if one_tile_per_cta:  # monolitic kernel style
        ...
        for j in range(0, tiles_per_cta):
            global_pid = pid + j * ctas_num
            total = global_cumsum_flat_impl_int64(
                global_pid,
                total,
                ne_result_ptr,
                tile_sum_ptr,  # in
...
def sorted_indices_unique_flat_int64(
    ...
    # launch kernel
    with torch_device_fn.device(sorted_data.device.index):
        local_ne_flat_kernel_int64[grid](
            sorted_data,  # in
            ne_result,
            tile_sum,  # out
            global_ctas_num,
        ...
        global_cumsum_flat_kernel_int64[grid](
            ne_result,
            tile_sum,  # in
            sorted_data,
            sorted_indices,  # in
        ...
        if return_counts:
            ...
            output_counts_flat_kernel_int64[grid](
                idx,
                num_tasks,  # in
                counts,  # out
                out_size,
...
def simple_unique_flat_int64(
    ...
    # launch kernel
    with torch_device_fn.device(sorted_data.device.index):
        simple_unique_flat_kernel_int64[grid](
            sorted_data,
            sorted_indices,  # in
            data_out,
            inverse_indices,
    ...
    return data_out[:out_size], inverse_indices, counts  # 不需要转换，已经是 int64
...
def _unique2_int64(
    ...
    if in0.numel() <= 8192:
        ...
        data_out, inverse_indices, counts = simple_unique_flat_int64(
            sorted_data, sorted_indices, return_inverse, return_counts
        )
    elif return_inverse:
        sorted_data, sorted_indices = torch.sort(in0.ravel())
        data_out, inverse_indices, counts = sorted_indices_unique_flat_int64(
            sorted_data, sorted_indices, return_counts
        )
    else:
        sorted_data, _ = torch.sort(in0.ravel())
        data_out, inverse_indices, counts = sorted_quick_unique_flat_int64(
