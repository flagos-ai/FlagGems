...
def scan_then_fan_col(inp, out, out_indices, n_ele, dtype, use_out_indices=False):
    ...
    if need_partial:
        ...
        partial_min_indices = torch.empty(
            part_num, dtype=torch.int32, device=inp.device
        )
    else:
        partial_min = None
...
def scan_then_fan(inp, out, out_indices, A, B, C, dtype, use_out_indices=False):
    ...
    if need_partial:
        ...
        partial_min_indices = torch.empty(
            A, part_num, C, dtype=torch.int32, device=inp.device
        )
    else:
        partial_min = None
        partial_min_indices = None
...
def scan_part_min_abc_loop_kernel(
    ...
    prev_min_val_idx = tl.full([], 0, dtype=tl.int32)
...
def cummin(
    ...
    *,
    ...
    logger.debug("GEMS_ENFLAME CUMMIN")
    ...
    out_indices = torch.empty_like(input, dtype=torch.int32)
    ...
    return out, out_indices.to(torch.int64)
