...
def isin_by_comparation(
    ...
    if M <= 1024:
        ...
        BLOCK_M, BLOCK_N, num_warps = launch_arg(4, 128, N, 4)
    ctas_num = min(128, triton.cdiv(M, BLOCK_M))
...
def isin_by_search(
    ...
    if unique_in0:
        out = torch.gather(out, 0, unique_order.ravel())
    return out.view_as(in0)
...
def isin(
    ...
    *,
    ...
    logger.debug("GEMS_ENFLAME ISIN")
    ...
    if in0.dtype == torch.int64:
        in0 = in0.to(torch.int32)
    if in1.dtype == torch.int64:
        in1 = in1.to(torch.int32)
