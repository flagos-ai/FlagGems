...
def argmin_kernel(
    ...
    for pid_k in range(pid_k_base, K, GRID_K):
...
def argmin(inp, dim=None, keepdim=False, *, dtype=None):
    ...
    if dim is None:
        ...
        mid_index = torch.empty((mid_size,), dtype=torch.int32, device=inp.device)
        if keepdim:
            shape = list(inp.shape)
            for i in range(0, inp.dim()):
                shape[i] = 1
            out = torch.empty(shape, dtype=torch.int32, device=inp.device)
        else:
            out = torch.empty([], dtype=torch.int32, device=inp.device)
        ...
        return out_index.to(torch.int64)
