...
def gcu_generate_functional_tile_wrapper(
    ...
    with code.indent():
        ...
        code.writeline("for i in range(len(dims_shape)): ")
        with code.indent():
            code.writeline("if dims_shape[i] > 1: ")
            with code.indent():
                code.writeline("insertIdx = i + insertCount")
                code.writeline("in0_shape.insert(insertIdx, 1)")
                code.writeline("reshape_out_shape.insert(insertIdx, dims_shape[i])")
...
def gcu_generate_code(
    ...
    code = gcu_generate_functional_tile_wrapper(
        wrapper_name, destination_passing_func_name, code
    )
    code = gcu_generate_destination_passing_tile_wrapper(
        rank, destination_passing_func_name, kernel_name, code
    )
    code = gcu_generate_tile_kernel(rank, kernel_name, code)
    return code
...
class TileFunction:
    ...
    def __call__(self, x, dims):
        ...
        key = str(ndim)
        dim_diff = ndim - len(dims)
        gcu_generate = False
        key_gcu = "gcu_" + str(ndim)
        broadcast_dim = 0
        broadcast_idx = -1
        for i in range(ndim):
            if len(dims) > i and dims[i] > 1:
                broadcast_dim += 1
                broadcast_idx = i + dim_diff
                key_gcu += "_" + str(i + dim_diff)
