...
def _searchsorted_kernel(
    ...
    # GCU300 does not support 64-bit data types; the host forces USE_INT32_INDEX
    # to True, so the index arithmetic stays in int32 throughout. The int64
    # branches of the upstream kernel are dropped here so no i64 value ever
    # reaches the verifier.
    if IS_1D_SEQUENCE:
        row_offsets = tl.zeros((BLOCK_SIZE,), dtype=tl.int32)
    else:
        row_offsets = ((offsets // values_per_row) * sequence_len).to(tl.int32)
...
def _searchsorted_impl(
    ...
    *,
    ...
    # GCU300: sorter is loaded as data in the kernel; downcast to int32 after validation.
    if sorter_contiguous is not None and sorter_contiguous.dtype == torch.int64:
        sorter_contiguous = sorter_contiguous.to(torch.int32)
    ...
    # GCU300 does not support 64-bit data types; force int32 index arithmetic
    # in the kernel. Values/sequence length are bounded by int32 for any supported
    # shape, and the result index fits in int32 (then cast to int64 if needed).
    use_int32_index = (
...
def searchsorted(
    ...
    *,
    ...
    logger.debug("GEMS_ENFLAME SEARCHSORTED")
...
def searchsorted_out(
    ...
    *,
    ...
    logger.debug("GEMS_ENFLAME SEARCHSORTED OUT")
    return _searchsorted_impl(
        sorted_sequence,
        self,
        out_int32=out_int32,
...
def searchsorted_scalar(
    ...
    *,
    ...
    logger.debug("GEMS_ENFLAME SEARCHSORTED SCALAR")
...
def searchsorted_scalar_out(
    ...
    *,
    ...
    logger.debug("GEMS_ENFLAME SEARCHSORTED SCALAR OUT")
    _check_scalar_values_shape(sorted_sequence)
    values = torch.scalar_tensor(self, device=sorted_sequence.device)
    return _searchsorted_impl(
        sorted_sequence,
