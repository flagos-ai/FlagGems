...
def vstack_kernel(
    ...
    end_idx = local_row * row_stride.to(tl.int32)
    idx = (pid_x * BLOCK_SIZE + col_idx).to(tl.int32)
    offset_mask = idx < end_idx
    in_offset = intensor_ptr + idx
    row_stride_offset = (total_row_offset + base_exc_row_idx) * row_stride.to(tl.int32)
