...
def gcu_generate_functional_repeat_wrapper(
    ...
    with code.indent():
        ...
        code.writeline("is_empty = False")
        code.writeline("out_shape = []")
        code.writeline("reshape_out_shape = []")
        code.writeline("for i in range(len(in0_shape)): ")
        with code.indent():
            code.writeline(
                "assert(sizes_shape[i] >= 0), 'the number of repetitions per dimension out of range (expected to >= 0) \
            ...
            with code.indent():
                code.writeline("is_empty = True")
            code.writeline("out_shape.append(in0_shape[i] * sizes_shape[i])")
            code.writeline("reshape_out_shape.append(in0_shape[i])")
        code.newline()
        code.writeline("insertIdx = -1")
        code.writeline("insertCount = 0")
        code.writeline("for i in range(len(sizes_shape)): ")
        with code.indent():
            code.writeline("if sizes_shape[i] > 1: ")
            with code.indent():
                code.writeline("insertIdx = i + insertCount")
                code.writeline("in0_shape.insert(insertIdx, 1)")
                code.writeline("reshape_out_shape.insert(insertIdx, sizes_shape[i])")
                code.writeline("insertCount += 1")
        ...
        with code.indent():
            ...
            code.writeline("for i in range(insertIdx): ")
            with code.indent():
                code.writeline("if cur_shape * in0_shape[i] > limit:")
                with code.indent():
            ...
            code.writeline("cur_shape = 1")
            code.writeline("for i in range(insertIdx + 1, reshape_len, 1): ")
            with code.indent():
                code.writeline("if cur_shape * in0_shape[i] > limit:")
                with code.indent():
                    code.writeline("in0_merge_shape.append(cur_shape)")
        ...
        code.writeline("if not is_empty: ")
        with code.indent():
            code.writeline("out0 = out0.reshape(reshape_out_shape)")
            # call destination_passing_func
            output_names: str = output_ref_for_wrapper()
            call_str = (
                f"{output_names} = {destination_passing_func_name}"
            ...
            code.writeline(call_str)
            code.writeline("out0 = out0.reshape(out_shape)")
        return_str = "return out0"
        code.writeline(return_str)
        code.newline()
        code.newline()
...
def gcu_generate_destination_passing_repeat_wrapper(
    ...
    with code.indent():
        ...
        if rank > 0:
            ...
            code.writeline("num_warps = 1")
            code.writeline(f"num_ctas = min({max_grid_size}, num_tiles)")
        ...
        if rank > 0:
            code.writeline("# strides of each tensor argument w.r.t the task space")
            code.writeline("in_strides = in0.stride()")
            code.writeline("in0_shape = in0.shape")
            code.writeline("out0_strides = out0.stride()")
        code.newline()
...
def generate_repeat_kernel(
    ...
    with code.indent():
        ...
        if rank > 0:
            ...
            # strides for outputs
            stride_args = ", ".join(f"out0_stride{j}: tl.constexpr" for j in range(rank))
...
def gcu_generate_repeat_kernel(
    ...
    with code.indent():
        ...
        with code.indent():
            ...
            order = _tuple_content(tuple(f"{i}" for i in range(rank)))
            # loads
            code.writeline("# loads")
            in_strides = _tuple_content(tuple(f"in_stride{i}" for i in range(rank)))
            code.writeline(
                f"in0_bptr = tl.make_block_ptr("
...
def gcu_generate_code(
    ...
    # the only runtime determined factor is the rank of the task space
    code = gcu_generate_imports(code)
    code = gcu_generate_functional_repeat_wrapper(
        wrapper_name, destination_passing_func_name, code
    )
    code = gcu_generate_destination_passing_repeat_wrapper(
        rank, destination_passing_func_name, kernel_name, code
    )
    code = gcu_generate_repeat_kernel(rank, kernel_name, code)
    return code
...
class RepeatFunction:
    ...
    def __call__(self, x, sizes):
        ...
        key = str(ndim)
        gcu_generate = False
        key_gcu = "gcu_" + str(ndim)
        broadcast_dim = 0
        broadcast_idx = -1
        for i in range(ndim):
            if len(sizes) > i and sizes[i] > 1:
                broadcast_dim += 1
                broadcast_idx = i
                key_gcu += "_" + str(i)
        gcu_ndim = ndim + broadcast_dim
        if gcu_ndim < 5:
            gcu_generate = True
            key = key_gcu
            ...
            merge_shape = 1
            merged_dim_size = 1
            for i in range(0, broadcast_idx, 1):
                merge_shape *= in_shape[i]
                if merge_shape > limit:
                    merge_shape = in_shape[i]
                    merged_dim_size += 1
            if broadcast_idx > 0:
                merged_dim_size += 1
            merge_shape = 1
            for i in range(broadcast_idx, ndim, 1):
                merge_shape *= in_shape[i]
                if merge_shape > limit:
                    merge_shape = in_shape[i]
                    merged_dim_size += 1
            merged_dim_size += 1
            if merged_dim_size < 4:
                gcu_generate = True
                key = key_gcu
                gcu_ndim = merged_dim_size
