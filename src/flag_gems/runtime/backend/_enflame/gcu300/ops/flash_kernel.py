...
def virtual_to_cache_offset(
    ...
    if boundary_check:
        # ENFLAME(GCU300): generic casts the loaded page index to tl.int64;
        # GCU300 codegen rejects 64-bit in-kernel dtypes. Page indices are
        # small and int32-lossless, so cast to int32 here instead.
        ...
        # ENFLAME(GCU300): int32 in place of the generic tl.int64 (see above).
...
def flash_varlen_fwd_kernel(
    ...
    for step in tl.range(0, n_masking_steps):
        ...
        # ENFLAME(GCU300): pin the P@V accumulator output to fp32 (generic
        # leaves out_dtype implicit).
        acc_ = tl.dot(P, bV, acc_, out_dtype=tl.float32)
        n_block -= 1
