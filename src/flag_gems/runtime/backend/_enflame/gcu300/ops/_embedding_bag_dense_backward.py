...
def _embedding_bag_dense_backward(
    ...
    """
    logger.debug("GEMS_ENFLAME _EMBEDDING_BAG_DENSE_BACKWARD")
    ...
    # pointer args, which GCU rejects. Downcast to int32 before launch. All of
    # them index counts/rows that fit in int32 for practical tensor sizes.
    if indices.dtype == torch.int64:
        indices = indices.to(torch.int32)
    if offset2bag.dtype == torch.int64:
        offset2bag = offset2bag.to(torch.int32)
    if bag_size.dtype == torch.int64:
        bag_size = bag_size.to(torch.int32)
