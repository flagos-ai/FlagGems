...
def topk_stage1_kernel(
    ...
    for i in range(BLOCK_SIZE_M):
        cur_batch = tl.program_id(0) * BLOCK_SIZE_M + i
        if cur_batch < M:
            cur_chunk_idx = tl.program_id(1)
            chunk_num = tl.num_programs(1)

            y_ptr_cur = y_ptr + cur_batch * chunk_num * k + cur_chunk_idx * k
            index_ptr_cur = index_ptr + cur_batch * chunk_num * k + cur_chunk_idx * k

            chunk_offset = cur_chunk_idx * CHUNK_SIZE
            x_ptr_cur = x_ptr + cur_batch * N + chunk_offset
            ...
            for k_idx in range(k):
                ...
                tl.store(y_ptr_cur + k_idx, chunk_select_val)
                tl.store(index_ptr_cur + k_idx, chunk_select_idx + chunk_offset)
...
def topk_stage2_kernel(
    ...
    for i in range(BLOCK_SIZE_M):
        cur_batch = tl.program_id(0) * BLOCK_SIZE_M + i
        if cur_batch < M:
            chunk_x_cur = cur_batch * N + chunk_x
            chunk_index_cur = cur_batch * N + chunk_index
            y_ptr_cur = cur_batch * k + y_ptr
            index_ptr_cur = cur_batch * k + index_ptr
            ...
            tl.store(y_ptr_cur + cols, sorted_chunk_x, mask=cols < k)
            tl.store(index_ptr_cur + cols, sorted_chunk_index, mask=cols < k)
...
def topk(x, k, dim=-1, largest=True, sorted=True):
    ...
    if batch_size > 65535:
        grid_m = MAX_GRID_DIM
        BLOCK_SIZE_M = (batch_size + MAX_GRID_DIM - 1) // MAX_GRID_DIM
