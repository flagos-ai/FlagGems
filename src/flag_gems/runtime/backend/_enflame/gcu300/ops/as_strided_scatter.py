...
def _as_strided_owner_kernel(
    ...
    offsets = tl.program_id(0) * BLOCK_SIZE + tl.arange(0, BLOCK_SIZE)
    mask = offsets < n_elements
    remaining = offsets.to(tl.int32)
    target = tl.full(offsets.shape, storage_offset, tl.int32)
    ...
    tl.atomic_min(owners + target, offsets.to(tl.int32), mask=mask)
...
def as_strided_scatter(
    ...
    logger.debug("GEMS_ENFLAME AS_STRIDED_SCATTER")
    ...
    with torch_device_fn.device(self.device):
        ...
        if expected_numel:
            ...
            if covers_storage:
                ...
                sizes = torch.tensor(size, dtype=torch.int32, device=self.device)
                strides = torch.tensor(stride, dtype=torch.int32, device=self.device)
                owners = torch.full(
                    (storage_numel,),
                    expected_numel,
                    dtype=torch.int32,
