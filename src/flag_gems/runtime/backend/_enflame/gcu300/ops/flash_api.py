...
def mha_varlan_fwd(
    ...
    with torch_device_fn.device(q_device):
        ...
        if p_dropout > 0:
            ...
            is_dropout = False
            # GCU300: never pass an int64-typed buffer to the kernel when dropout
            # is off.  The philox state is only loaded under the `if is_dropout:`
            # constexpr branch, so a None pointer is safe here (same as the
            # varlan_fwd_opt path).
            philox_args = None
...
def mha_fwd(
    ...
    with torch_device_fn.device(q_device):
        ...
        def dispatch(B, H, Q, K, D, params):
            ...
            # ENABLE_I64 must be supplied at launch time (not only as the kernel's
            # default constexpr) so the gcu300 backend enables its i64 path;
            # otherwise int64 kernel inputs are rejected at compile time.
            kernel = kernel(*params.args(), ENABLE_I64=True)
            return kernel
