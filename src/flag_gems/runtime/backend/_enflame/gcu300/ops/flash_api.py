...
def mha_varlan_fwd(
    ...
    with torch_device_fn.device(q_device):
        ...
        if p_dropout > 0:
            ...
            is_dropout = False
            # GCU300 rejects 64-bit types in-kernel; philox is never
            # dereferenced when dropout is off, so pass an int32 dummy to
            # keep the launched kernel free of !tt.ptr<i64> args.
            philox_args = torch.empty((2,), dtype=torch.int32, device=q_device)
