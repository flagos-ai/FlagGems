...
def prod_kernel_mid(
    ...
    if inp.type.element_ty == tl.int64:
        full_val = 0
        target_type = tl.int64
    inp_val = tl.load(inp_ptrs, mask=mask, other=full_val).to(target_type)
...
def prod_kernel(
    ...
    for start_n in range(0, N, BLOCK_N):
        ...
        if inp.type.element_ty == tl.int64:
            full_val = 0
            target_type = tl.int64
        inp_vals = tl.load(inp_ptrs, mask=mask, other=full_val).to(target_type)
        acc *= inp_vals
