...
import torch
import numpy as np
from unique import _unique2, simple_unique_flat, sorted_indices_unique_flat, sorted_quick_unique_flat


def verify_unique_result(in0, sorted_flag=True, return_inverse=True, return_counts=True, verbose=True):
    """
    验证 _unique2 的结果是否与 PyTorch 原生实现一致
    """
    # Triton 实现
    triton_out, triton_inverse, triton_counts = _unique2(
        in0, sorted=sorted_flag, return_inverse=return_inverse, return_counts=return_counts
    )
    ...
    if not torch.equal(triton_out, torch_out):
        ...
        if verbose:
            ...
            for idx in mismatch_idx:
                print(f"    位置 {idx}: triton={triton_out[idx].item()}, torch={torch_out[idx].item()}")
    
    # 比较 inverse_indices
    if return_inverse:
        triton_inverse_flat = triton_inverse.ravel() if triton_inverse is not None else None
        ...
        if triton_inverse_flat is not None:
            if not torch.equal(triton_inverse_flat, torch_inverse_flat):
                ...
                if verbose:
                    mismatch_idx = torch.where(triton_inverse_flat != torch_inverse_flat)[0][:10]
                    print(f"  前10个不匹配位置: {mismatch_idx.tolist()}")
                    for idx in mismatch_idx:
                        print(f"    位置 {idx}: triton={triton_inverse_flat[idx].item()}, torch={torch_inverse_flat[idx].item()}")
                        
                # 检查是否存在溢出的特征（负数或异常大的值）
                if triton_inverse_flat is not None:
                    negative_count = (triton_inverse_flat < 0).sum().item()
                    if negative_count > 0:
                        errors.append(f"  inverse_indices 中有 {negative_count} 个负数值（可能是 int32 溢出）")
                    
                    max_val = triton_inverse_flat.max().item()
                    min_val = triton_inverse_flat.min().item()
                    if max_val > 2**31 - 1 or min_val < -2**31:
                        errors.append(f"  inverse_indices 范围超出 int32: min={min_val}, max={max_val}")
    ...
    if return_counts:
        if triton_counts is not None and not torch.equal(triton_counts, torch_counts):
            ...
            if verbose:
                ...
                print(f"  前10个不匹配位置: {mismatch_idx.tolist()}")
                for idx in mismatch_idx:
                    print(f"    位置 {idx}: triton={triton_counts[idx].item()}, torch={torch_counts[idx].item()}")
            ...
            if triton_sum != expected_sum:
                errors.append(f"  triton counts 总和 {triton_sum} != 输入元素数 {expected_sum}")
...
def test_edge_cases(device='cuda'):
    ...
    test_cases = [
        ...
        ("已排序数据", torch.sort(torch.randint(0, 1000, (10000,), dtype=torch.int64, device=device))[0]),
        ("逆序数据", torch.sort(torch.randint(0, 1000, (10000,), dtype=torch.int64, device=device), descending=True)[0]),
        ("包含负数", torch.randint(-5000, 5000, (10000,), dtype=torch.int64, device=device)),
        ("大数值", torch.randint(2**30, 2**31, (10000,), dtype=torch.int64, device=device)),
    ]
...
def test_int32_overflow_specific(device='cuda'):
    ...
    for total_size, num_unique in sizes_to_test:
        ...
        try:
            ...
            data = torch.randint(0, num_unique, (total_size,), dtype=torch.int64, device=device)
...
def detailed_debug(device='cuda'):
    ...
    print(f"  inverse_indices 范围: [{triton_inverse.min().item()}, {triton_inverse.max().item()}]")
    ...
    print(f"  inverse_indices 范围: [{torch_inverse.min().item()}, {torch_inverse.max().item()}]")
