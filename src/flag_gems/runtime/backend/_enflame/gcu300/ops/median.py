...
from .topk import _get_iinfo_val
...
def median_small_dim_kernel(
    ...
    tl.store(indices + out_offsets, first_match, mask=out_mask)
...
def median_bool_count_kernel(
    ...
    true_count = tl.sum((valid & data).to(tl.int32), axis=0)
...
def median_bool_dim_count_chunks_kernel(
    ...
    true_count = tl.sum(true_mask.to(tl.int32), axis=0)
    ...
    tl.store(first_false + pid, first_false_idx)
    tl.store(first_true + pid, first_true_idx)
...
def median_bool_dim_finish_kernel(
    ...
    false_indices = tl.load(first_false + base, mask=valid, other=2147483647)
    true_indices = tl.load(first_true + base, mask=valid, other=2147483647)
...
def _median_bool_flat(inp):
    ...
    with torch_device_fn.device(inp.device):
        ...
        while counts.numel() > _BOOL_COUNT_REDUCE_BLOCK:
            ...
            reduced = torch.empty(
                (reduced_blocks,), dtype=torch.int32, device=inp.device
            )
...
def _median_bool_dim(inp, dim, output_shape):
    ...
    indices = torch.empty(output_shape, dtype=torch.int32, device=inp.device)
    block = _BOOL_FLAT_BLOCK
    chunks = triton.cdiv(reduction_size, block)
    chunk_shape = (total_outputs, chunks)
    counts = torch.empty(chunk_shape, dtype=torch.int32, device=inp.device)
    first_false = torch.empty(chunk_shape, dtype=torch.int32, device=inp.device)
    first_true = torch.empty(chunk_shape, dtype=torch.int32, device=inp.device)
    ...
    with torch_device_fn.device(inp.device):
        ...
        while chunks > _BOOL_COUNT_REDUCE_BLOCK:
            ...
            reduced_counts = torch.empty(
                reduced_shape, dtype=torch.int32, device=inp.device
            )
...
def _median_lastdim_sort(row_data, output_shape):
    ...
    indices = torch.empty(output_shape, dtype=torch.int32, device=row_data.device)
...
def median_dim(inp, dim=0, keepdim=False):
    ...
    if work.numel() == 0:
        ...
        indices = torch.empty(output_shape, dtype=torch.int32, device=work.device)
    else:
        if work.dtype.is_complex:
            _raise_dim_dtype(work.dtype)
    ...
    # GCU300 kernels produce indices in int32 buffers; restore the canonical
    # int64 dtype required by the torch.median API.
