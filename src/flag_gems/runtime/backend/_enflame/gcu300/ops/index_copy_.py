...
def generate_index_copy_kernel(
    ...
    with code.indent():
        ...
        with code.indent():
            ...
            # index copy (GCU300: int32 arithmetic; int64 index tensors are
            # downcast to int32 in the wrapper so generated code stays int32)
            code.writeline("pre_idx = (src_offset // pre_cal).to(tl.int32)")
            code.writeline(
                "dim_idx = (src_offset % pre_cal // inp_stride_dim).to(tl.int32)"
            )
            code.writeline(
                "src_dim_idx = tl.load(index + dim_idx, mask=mask, other=0).to(tl.int32)"
            )
            code.writeline(
...
def index_copy(inp, dim, index, src):
    logger.debug("GEMS_ENFLAME INDEX_COPY")
...
def index_copy_(inp, dim, index, src):
    logger.debug("GEMS_ENFLAME INDEX_COPY_")
    assert -inp.ndim <= dim < inp.ndim, "Invalid dim"
    dim %= inp.ndim
    assert index.numel() == src.size(
        dim
