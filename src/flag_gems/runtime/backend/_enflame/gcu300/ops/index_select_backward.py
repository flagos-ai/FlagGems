...
def index_select_backward(grad, self_sizes, dim, index):
    ...
    """
    logger.debug("GEMS_ENFLAME INDEX_SELECT_BACKWARD")

    if index.ndim == 0:
        index = index.unsqueeze(0)

    # GCU300: keep the index as int32 (int64_enabled=False). index_select passes
    # int64 indices from the upper layer; the generic kernel upcasts them to
    # int64 which the GCU compiler rejects. Index elements index into a dim of
    # size < 2^31 in practice, so the downcast is lossless.
