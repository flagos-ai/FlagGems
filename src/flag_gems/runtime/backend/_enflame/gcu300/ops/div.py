...
def floor_div_func(x, y):
    ...
    elif torch.is_floating_point(x) & x.type.scalar.is_int():
        return _float_floordiv(x, y.to(torch.float32))
    elif x.type.scalar.is_int() & torch.is_floating_point(y):
        return _float_floordiv(x.to(torch.float32), y)
    else:
