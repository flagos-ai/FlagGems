...
def generate_index_wrapper(
    ...
    with code.indent():
        ...
        code.writeline(f"if input.dtype == torch.int64:")
        code.writeline(f"  if isinstance(input, StridedBuffer):")
        code.writeline(f"    input.convert_to_int32()")
        ...
        for i in range(indices_len):
            code.writeline(f"if indices[{i}].dtype == torch.int64:")
            with code.indent():
