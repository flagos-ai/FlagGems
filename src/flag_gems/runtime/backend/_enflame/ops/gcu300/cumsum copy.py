...
def scan_part_sum_abc_kernel(
    ...
    # Loop over all A rows assigned to this CTA
    for a_idx in range(pid_a, A, num_ctas_a):
...
def scan_then_fan(inp, out, A, B, C, dtype):
    ...
    num_ctas_a = min(A, MAX_GRID_DIM)
    grid = (num_ctas_a, part_num, C)
    with torch_device_fn.device(inp.device):
        scan_part_sum_abc_kernel[grid](
            inp, out, partial_sum, A, B, C, num_ctas_a, part_num, BLOCK_SIZE
        )
