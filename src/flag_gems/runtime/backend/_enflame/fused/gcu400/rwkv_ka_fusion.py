...
def rwkv_ka_fusion(
    ...
    kt = torch.ops.aten.mul.Tensor(k, kk)
    kt_f = kt.float().view(T, H, N)
    norms = torch.ops.aten.linalg_vector_norm(kt_f, 2, [-1], True)
    norms = torch.ops.aten.clamp_min(norms, 1e-6)
    okk_f = torch.ops.aten.div.Tensor(kt_f, norms)
    o_kk = okk_f.to(dtype).view(T, C)

    if dtype == torch.float32:
        o_k = torch.ops.aten.mul.Tensor(
            k, 1 + torch.ops.aten.mul.Tensor(a - 1, ka)
        )
    else:
        coeff = (1 + torch.ops.aten.mul.Tensor(
            a.float() - 1, ka.float()
        )).to(dtype)
        o_k = torch.ops.aten.mul.Tensor(k, coeff)

    o_kka = torch.ops.aten.mul.Tensor(o_kk, a)
