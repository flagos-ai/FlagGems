...
def moe_align_block_size_stage1(
    ...
    for e in range(num_experts):
        count = tl.sum(((expert_id == e) & mask).to(tl.int32), axis=0)
        tl.store(tokens_cnts_ptr + off_c + e, count)
...
def moe_align_block_size_stage4(
    ...
    mask = offset < numel
    expert_id = tl.load(topk_ids_ptr + offset, mask=mask, other=0)
    for e in range(num_experts):
        is_expert_e = (expert_id == e) & mask
        is_expert_e_int = is_expert_e.to(tl.int32)
        inclusive_cumsum = tl.cumsum(is_expert_e_int, axis=0)
        exclusive_cumsum = inclusive_cumsum - is_expert_e_int
        base = tl.load(tokens_cnts_ptr + off_t + e)
        expert_start = tl.load(cumsum_ptr + e)
        rank_post_pad = base + exclusive_cumsum + expert_start
        tl.store(sorted_token_ids_ptr + rank_post_pad, offset, mask=is_expert_e)
