...
def fused_moe_kernel_gptq_awq(
    ...
    offs_token = tl.load(sorted_token_ids_ptr + offs_token_id).to(tl.int32)
...
def fused_moe_kernel(
    ...
    # Create pointers for first blocks of A and B
    offs = tl.arange(0, BLOCK_SIZE_M).to(tl.int32)
    num_tokens_post_padded = tl.load(num_tokens_post_padded_ptr)
    if pid_m * BLOCK_SIZE_M >= num_tokens_post_padded:
        return
    ...
    offs_token = offs_token.to(tl.int32)  # prevent int32 overflow
    ...
    if FUSE_SILU and PAIR_GATE_UP_DOT:
        ...
        offs_pair = tl.arange(0, BLOCK_SIZE_N * 2).to(tl.int32)
...
def fused_experts_impl(
    ...
    # GCU300: int64_enabled=False. torch.topk returns int64 topk_ids, and the
    # MoE kernels read topk_ids / expert_ids (= curr_topk_ids.view(-1)) via the
    # pointer; an int64 pointer arg is rejected ("64-bit data type not supported
    # on GCU300"). Downcast to int32 before any kernel launch; expert ids and
    # token counts are all well under 2^31 in practice. The in-kernel offset
    # arithmetic was already rewritten to int32 in this gcu300 copy.
