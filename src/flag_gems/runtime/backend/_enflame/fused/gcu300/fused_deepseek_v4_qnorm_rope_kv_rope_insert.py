...
def fused_deepseek_v4_qnorm_rope_kv_rope_insert(
    ...
    """
    logger.debug("GEMS_ENFLAME FUSED_DEEPSEEK_V4_QNORM_ROPE_KV_ROPE_INSERT")

    # GCU300: int64_enabled=False. vLLM/upper layers pass slot_mapping and
    # position_ids as int64; the kernel reads them via int64 pointer args, which
    # GCU rejects ("64-bit data type not supported on GCU300"). Both index
    # plain, sub-2^31 ranges in practice, so downcast to int32 before launch.
    # Mirrors the established pattern in gcu300 concat_and_cache_mla.
    ...
    # Grid capped at MAX_GRID_DIM for large workloads
    grid = min(total, MAX_GRID_DIM)
