...
@libentry()
@triton.jit(do_not_specialize=["K", "M", "N", "philox_seed", "philox_offset"])
def multinomial_with_replacement(
    cdf_ptr, out_ptr, K, M, N, philox_seed, philox_offset, NBLOCK: tl.constexpr = 128
):
    ...
    num_prog = tl.num_programs(1)
    iter_num = tl.cdiv(M, num_prog)
    for i in range(0, iter_num):
        pid_m = i * num_prog + tl.program_id(1)
        # Mask to skip processing if pid_m exceeds the number of distributions
        valid_dist = pid_m < M
        ...
        # Use a temporary pointer for CDF to avoid modifying the original pointer
        cdf_ptr_m = cdf_ptr + pid_m * K
        ...
        for _ in range(steps):
            ...
            x = tl.load(cdf_ptr_m + mid, mask=n < N and valid_dist)
        ...
        # Store results only for valid distributions and valid samples
        tl.store(out_ptr + y_off + n, start, mask=n < N and valid_dist)
...
def multinomial(prob, n_samples, with_replacement=False, *, gen=None):
    ...
    grid = lambda META: (triton.cdiv(n_samples, META["NBLOCK"]), min(n_dist, MAX_GRID_DIM))
    multinomial_with_replacement[grid](
        cum_prob, out, n_categories, n_dist, n_samples, philox_seed, philox_offset
    )
