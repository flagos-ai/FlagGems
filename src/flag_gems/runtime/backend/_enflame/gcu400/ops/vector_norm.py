...
def l2_norm_kernel(X, Out, M, N, BLOCK_M: tl.constexpr, BLOCK_N: tl.constexpr):
    ...
    num_progs_m = tl.num_programs(0)

    for k in range(0, tl.cdiv(M, num_progs_m * BLOCK_M)):
        row_block_id = k * num_progs_m + pid_m
        m_offset = row_block_id * BLOCK_M + tl.arange(0, BLOCK_M)[:, None]

        row_mask = m_offset < M
        X_offset = X + m_offset * N
        Out_offset = Out + m_offset
        ...
        for off in range(0, N, BLOCK_N):
            ...
            a = tl.load(X_offset + cols, mask, other=0.0).to(tl.float32)
            _sum += a * a

        sum = tl.sum(_sum, axis=1)
        out = tl.sqrt(sum)[:, None]
        tl.store(Out_offset, out, row_mask)
...
def min_norm_kernel(X, Out, M, N, BLOCK_M: tl.constexpr, BLOCK_N: tl.constexpr):
    ...
    for k in range(0, tl.cdiv(M, num_progs_m * BLOCK_M)):
        ...
        for off in range(0, N, BLOCK_N):
            ...
            a = tl.load(X_offset + cols, mask, other=float("inf")).to(tl.float32)
...
def l0_norm_kernel(X, Out, M, N, BLOCK_M: tl.constexpr, BLOCK_N: tl.constexpr):
    ...
    for k in range(0, tl.cdiv(M, num_progs_m * BLOCK_M)):
        ...
        for off in range(0, N, BLOCK_N):
            ...
            a = tl.load(X_offset + cols, mask, other=0).to(tl.float32)
...
def vector_norm(x, ord=2, dim=None, keepdim=False, dtype=None):
    ...
    with torch_device_fn.device(x.device):
        if (not dim) or len(dim) == x.ndim:
            ...
            M = x.numel() // N
            out = torch.empty(shape, dtype=dtype, device=x.device)
            grid = lambda META: (min(triton.cdiv(M, META["BLOCK_M"]), MAX_GRID_DIM),)
            if ord == 2:
