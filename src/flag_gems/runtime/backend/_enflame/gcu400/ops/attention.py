...
configs = [
    ...
    for BM in [64, 128]
    for BN in [128]
    for PLV in [True, False]
    for s in [3]
    for w in [2, 4]
]


def _attn_fwd_prune_configs(configs, named_args, **kwargs):
    head_dim = named_args.get("HEAD_DIM", 128)
    return [c for c in configs if c.kwargs.get("BLOCK_N", 128) <= head_dim]
...
@libtuner(
    configs=configs,
    key=["KV_CTX", "HEAD_DIM"],
    prune_configs_by={"early_config_prune": _attn_fwd_prune_configs},
)
@triton.jit
def _attn_fwd(
    Q,
