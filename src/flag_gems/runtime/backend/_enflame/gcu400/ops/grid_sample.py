...
def grid_sample(
    ...
    """
    logger.debug("GEMS_ENFLAME GRID_SAMPLE")
    # Validate inputs
    ...
    if not is_3d:
        ...
        if (
            ...
            grid_y = num_h_blocks * num_w_blocks

            # grid_y must not exceed 255 (hardware limit)
            if grid_y > 255:
                # Increase tile size to reduce grid_y within the limit
                while grid_y > 255:
                    block_h *= 2
                    block_w *= 2
                    num_h_blocks = (H_out + block_h - 1) // block_h
            ...
            # grid_x must not exceed 65535 (hardware limit)
            if grid_x <= 65535:
                ...
                # Split along batch (N) dimension to keep grid_x within limit
                # Align to N boundary so that batch_N * C == nc_count
                max_n_per_batch = max(1, 65535 // C)
                for n_start in range(0, N, max_n_per_batch):
                    n_end = min(n_start + max_n_per_batch, N)
                    batch_N = n_end - n_start
                    batch_input = input[n_start:n_end]
                    batch_output = output[n_start:n_end]
                    batch_grid = grid[n_start:n_end]
                    grid_size = (batch_N * C, grid_y)
                    kernel[grid_size](
                        batch_output,
                        batch_input,
                        batch_grid,
            ...
            if grid_x <= 65535:
                ...
                # Split along batch (N) dimension to keep grid_x within limit
                # Each batch slice must satisfy: batch_N * C * H_out * W_out <= 65535
                pixels_per_sample = C * H_out * W_out
                max_n_per_batch = max(1, 65535 // max(1, pixels_per_sample))
                if pixels_per_sample <= 65535:
                    # Can split by N dimension
                    for n_start in range(0, N, max_n_per_batch):
                        n_end = min(n_start + max_n_per_batch, N)
                        batch_N = n_end - n_start
                        batch_input = input[n_start:n_end]
                        batch_output = output[n_start:n_end]
                        batch_grid = grid[n_start:n_end]
                        batch_grid_x = batch_N * C * H_out * W_out
                        grid_size = (batch_grid_x,)
                        kernel[grid_size](
                            batch_output,
                            batch_input,
                            batch_grid,
                    ...
                    # Single sample exceeds limit (C * H_out * W_out > 65535)
                    # Split by N, channel groups, and row groups
                    hw_pixels = H_out * W_out
                    max_c_per_group = max(1, 65535 // max(1, hw_pixels))
                    if hw_pixels <= 65535:
                        for ni in range(N):
                            for c_start in range(0, C, max_c_per_group):
                                c_end = min(c_start + max_c_per_group, C)
                                batch_C = c_end - c_start
                                sample_input = input[ni : ni + 1, c_start:c_end]
                                sample_output = output[ni : ni + 1, c_start:c_end]
                                sample_grid = grid[ni : ni + 1]
                                sample_grid_x = batch_C * H_out * W_out
                        ...
                        # H_out * W_out > 65535, split by row groups per channel
                        max_h_per_group = max(1, 65535 // max(1, W_out))
                        for ni in range(N):
    ...
    # Handle 5D inputs (N, C, D_in, H_in, W_in)
        ...
        if (
            ...
            num_h_blocks = (H_out + block_h - 1) // block_h
            num_w_blocks = (W_out + block_w - 1) // block_w
            grid_y = num_d_blocks * num_h_blocks * num_w_blocks
            ...
            if grid_x <= 65535:
                ...
                # Split along batch (N) dimension to keep grid_x within limit
                voxels_per_sample = C * D_out * H_out * W_out
                max_n_per_batch = max(1, 65535 // max(1, voxels_per_sample))
                if voxels_per_sample <= 65535:
                    for n_start in range(0, N, max_n_per_batch):
                        n_end = min(n_start + max_n_per_batch, N)
                        batch_N = n_end - n_start
                        batch_input = input[n_start:n_end]
                        batch_output = output[n_start:n_end]
                        batch_grid = grid[n_start:n_end]
                        batch_grid_x = batch_N * C * D_out * H_out * W_out
                        grid_size = (batch_grid_x,)
                        kernel[grid_size](
                            batch_output,
                            batch_input,
                    ...
                    # Single sample exceeds limit, split by N and channel groups
                    dhw_voxels = D_out * H_out * W_out
                    max_c_per_group = max(1, 65535 // max(1, dhw_voxels))
                    for ni in range(N):
                        for c_start in range(0, C, max_c_per_group):
                            c_end = min(c_start + max_c_per_group, C)
                            batch_C = c_end - c_start
                            ...
                            sample_output = output[ni : ni + 1, c_start:c_end]
                            sample_grid = grid[ni : ni + 1]
                            sample_grid_x = batch_C * D_out * H_out * W_out
                            grid_size = (sample_grid_x,)
                            kernel[grid_size](
                                sample_output,
                                sample_input,
