...
def sum_kernel_1(
    ...
    for work_id in range(pid, mid_size, num_ctas):
        offset = work_id * BLOCK_SIZE + tl.arange(0, BLOCK_SIZE)
        inp_ptrs = inp + offset
        mask = offset < M
...
def sum(inp, *, dtype=None):
    ...
    grid_x = min(mid_size, MAX_GRID_X)
    with torch_device_fn.device(inp.device):
        sum_kernel_1[(grid_x, 1, 1)](inp, mid, M, mid_size, block_size)
...
def sum_dim_kernel_non_inner(
    ...
    num_ctas_x = tl.num_programs(0)

    k_offsets = pid_k * TILE_K + tl.arange(0, TILE_K)[None, :]

    for pid_m in range(pid_x, M, num_ctas_x):
        if ONE_TILE_PER_CTA:
            ...
            _acc = tl.zeros([TILE_N, TILE_K], dtype=cdtype)
            ...
            for start_n in range(0, N, TILE_N):
                ...
                _acc += inp
            out = tl.sum(_acc, axis=0, keep_dims=True)
            out_offset = pid_m * K + k_offsets
            output_ptrs = output_ptr + out_offset
            tl.store(output_ptrs, out, mask=k_offsets < K)
...
def sum_dim_kernel(
    ...
    for block_id in range(pid_x, total_blocks, num_ctas_x):
        # Map block_id to the rows this CTA should compute
        pid = block_id * BLOCK_M + tl.arange(0, BLOCK_M)[:, None]
        inp_ptrs = inp + pid * N
        out_ptrs = out + pid
        row_mask = pid < M
        ...
        tl.store(out_ptrs, sum, row_mask)
...
def sum_dim_comm(inp, dim=None, keepdim=False, *, dtype=None, out=None):
    ...
    if len(dim) == 1:
        ...
        grid = lambda meta: (
            min(triton.cdiv(M, meta["BLOCK_M"]), MAX_GRID_X),
        )
        with torch_device_fn.device(inp.device):
