...
def avg_pool2d_forward_kernel(
    ...
    divisor_override,
    total_nc,
    NC_PER_PROGRAM: tl.constexpr,
    BLOCK_H: tl.constexpr,
    BLOCK_W: tl.constexpr,
):
    pid_nc_base = tl.program_id(0)
    ...
    for nc_i in range(NC_PER_PROGRAM):
        pid_nc = pid_nc_base * NC_PER_PROGRAM + nc_i
        if pid_nc < total_nc:
            n_idx = pid_nc // in_c
            c_idx = pid_nc % in_c
...
def avg_pool2d(
    ...
    logger.debug("Enflame GEMS AVG_POOL2D FORWARD")
    ...
    NC_PER_PROGRAM = 1
    if total_nc >= 65536:
        grid_nc = MAX_GRID_DIM
        NC_PER_PROGRAM = (total_nc + MAX_GRID_DIM - 1) // MAX_GRID_DIM
...
def avg_pool2d_backward(
    ...
    logger.debug("Enflame GEMS AVG_POOL2D BACKWARD")
