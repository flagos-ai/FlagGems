    ...
    if ONE_TILE:
        for m_start in tl.range(pid * BLOCK_M, M, num_programs * BLOCK_M):
            out_block = tl.make_block_ptr(
                ...
                offsets=(m_start, 0), block_shape=(BLOCK_M, BLOCK_N), order=(1, 0),
            )
            grad_block = tl.make_block_ptr(
                base=out_grad_ptr, shape=(M, N), strides=(N, 1),
                offsets=(m_start, 0), block_shape=(BLOCK_M, BLOCK_N), order=(1, 0),
            )
            out_tile = tl.load(out_block, boundary_check=(0, 1), padding_option="zero").to(tl.float32)
            grad_tile = tl.load(grad_block, boundary_check=(0, 1), padding_option="zero").to(tl.float32)
            scale = tl.sum(out_tile * grad_tile, axis=1)
            in_grad_tile = out_tile * (grad_tile - scale[:, None])
            in_grad_block = tl.make_block_ptr(
                base=in_grad_ptr, shape=(M, N), strides=(N, 1),
                offsets=(m_start, 0), block_shape=(BLOCK_M, BLOCK_N), order=(1, 0),
            )
            tl.store(in_grad_block, in_grad_tile.to(in_grad_ptr.dtype.element_ty), boundary_check=(0, 1))
    else:
        for m_start in tl.range(pid * BLOCK_M, M, num_programs * BLOCK_M):
...
def softmax_backward_scale_kernel(
    ...
    for n_start in tl.range(pid * BLOCK_N, N, num_programs * BLOCK_N):
        ...
        grad_val = tl.load(out_grad_ptr + n_offsets, mask=mask, other=0.0).to(tl.float32)
...
def softmax_backward_grad_kernel(
    ...
    for n_start in tl.range(pid * BLOCK_N, N, num_programs * BLOCK_N):
        ...
        tl.store(in_grad_ptr + n_offsets, in_grad_val.to(in_grad_ptr.dtype.element_ty), mask=mask)
    ...
    for k_start in tl.range(pid_k * BLOCK_K, K, num_k_programs * BLOCK_K):
        ...
        for n_start in tl.range(0, N, BLOCK_N):
            out_block = tl.make_block_ptr(
                ...
                shape=(N, K), strides=(K, 1),
                offsets=(n_start, k_start), block_shape=(BLOCK_N, BLOCK_K), order=(1, 0),
            )
            grad_block = tl.make_block_ptr(
    ...
    with torch_device_fn.device(in_grad.device):
        if K > 1:
            ...
            softmax_backward_kernel_inner[grid](
                output, grad_output, in_grad, M, N,
                BLOCK_M=BLOCK_M, BLOCK_N=BLOCK_N, ONE_TILE=ONE_TILE, num_warps=1,
            )
    return in_grad
