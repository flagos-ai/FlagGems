...
from _enflame.utils.gcu300.codegen_config_utils import CodeGenConfig, get_codegen_config
...
class KernelGenerator:
    ...
    def gen_signature(self, code, with_block_pointer=False):
        ...
        with code.indent():
            ...
            if ndim > 0:
                ...
                # number of tiles, used to compute task
                code.writeline("num_tiles: int,")
    ...
    def gen_body_gsl_without_bptr(self, code):
        code.writeline("num_ctas = tl.num_programs(0)")
        code.writeline("for j in tl.range(pid, num_tiles, num_ctas, num_stages=2):")
        with code.indent():
            code.writeline("tile_id = j")
            self.gen_body_one_tile_per_cta_with_bptr(code)
    ...
    def gen_body_gsl_1d_tile(self, code):
        code.writeline("num_ctas = tl.num_programs(0)")
        code.writeline("for j tl.range(pid, num_tiles, num_ctas, num_stages=2):")
        with code.indent():
            code.writeline("tile_id = j")
            self.gen_body_one_tile_per_cta_with_bptr(code)
            ...
            with code.indent():
                ...
                with code.indent():
                    ...
                    if ndim > 0:
                        ...
                        code.writeline("num_tiles, # num tiles")
...
class ModuleGenerator:
    ...
    def generate_imports(code: IndentedBuffer) -> IndentedBuffer:
        ...
        code.writeline("from flag_gems.runtime.backend._enflame.utils.gcu300.shape_utils import (")
...
class PointwiseDynamicFunction:
    ...
    def __init__(self, op_desc: FunctionSchema, scalar_fn: JITFunction, config=None):
        ...
        # print(f"[pointwise_dynamic]config: {self.config}")
    ...
    def instantiate(self, ndim):
        ...
        # print(f"[pointwise_dynamic]kernel_name: {kernel_name}")
        # print(f"[pointwise_dynamic]scalar_fn_name: {scalar_fn_name}")
        # print(f"[pointwise_dynamic]code: {code.getvalue()}")
        ...
        file_name = (
            f"pointwise_dynamic_{kernel_name}_"
        ...
        # print(f"[bql]file_path: {file_path}")
