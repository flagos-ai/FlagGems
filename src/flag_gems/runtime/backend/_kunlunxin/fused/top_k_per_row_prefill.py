...
def _chunk_counts_kernel(
    ...
    if row < num_rows:
        ...
        count_gt = tl.full((), 0, tl.int32)
        count_eq = tl.full((), 0, tl.int32)
        mask_gt = tl.full((), 0, tl.uint32)
        mask_eq = tl.full((), 0, tl.uint32)
        for lane in tl.static_range(0, BLOCK):
            position = chunk * BLOCK + lane
            if position < row_len:
                column = (start + position).to(tl.int64) * stride1
                key = _ordered_key(tl.load(logits_ptr + logits_row_offset + column))
                is_gt = key > threshold
                is_eq = key == threshold
                count_gt += is_gt.to(tl.int32)
                count_eq += is_eq.to(tl.int32)
                mask_gt = mask_gt | (is_gt.to(tl.uint32) << lane)
                mask_eq = mask_eq | (is_eq.to(tl.uint32) << lane)
...
def top_k_per_row_prefill(
    ...
    if num_rows > max_rows_per_launch:
        workspaces = [] if _workspaces is None else _workspaces
    ...
    if _workspaces is not None:
        _workspaces.extend((counts, masks, chunk_prefix))
