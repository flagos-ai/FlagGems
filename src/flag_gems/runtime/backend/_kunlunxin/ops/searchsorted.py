...
def _searchsorted_kernel(
    ...
    for _ in tl.static_range(LOG_SEQUENCE_LEN):
        ...
        if HAS_SORTER:
            ...
            valid_sorter = (sorted_index >= 0) & (sorted_index < sequence_len)
            tl.device_assert(~active | valid_sorter, "sorter index out of range")
            sorted_offsets = row_offsets + tl.where(valid_sorter, sorted_index, 0)
