...
@triton.jit
def _grid_sample_3d_kunlunxin_kernel(output_ptr, input_ptr, grid_ptr, total, C: tl.constexpr, D_IN: tl.constexpr, H_IN: tl.constexpr, W_IN: tl.constexpr, D_OUT: tl.constexpr, H_OUT: tl.constexpr, W_OUT: tl.constexpr, IN_SN: tl.constexpr, IN_SC: tl.constexpr, IN_SD: tl.constexpr, IN_SH: tl.constexpr, IN_SW: tl.constexpr, GR_SN: tl.constexpr, GR_SD: tl.constexpr, GR_SH: tl.constexpr, GR_SW: tl.constexpr, GR_SC: tl.constexpr, MODE: tl.constexpr, PADDING: tl.constexpr, ALIGN: tl.constexpr, BLOCK: tl.constexpr):
    offsets = tl.program_id(0) * BLOCK + tl.arange(0, BLOCK)
    mask = offsets < total
    ox = offsets % W_OUT
    q = offsets // W_OUT
    ...
    gx = tl.load(grid_ptr + base, mask=mask, other=0.).to(tl.float32)
    gy = tl.load(grid_ptr + base + GR_SC, mask=mask, other=0.).to(tl.float32)
    gz = tl.load(grid_ptr + base + 2 * GR_SC, mask=mask, other=0.).to(tl.float32)
    nx = ~(tl.abs(gx) <= 3.4e38)
    ny = ~(tl.abs(gy) <= 3.4e38)
    nz = ~(tl.abs(gz) <= 3.4e38)
    nan = nx | ny | nz
    gx, gy, gz = tl.where(nx, -1., gx), tl.where(ny, -1., gy), tl.where(nz, -1., gz)
    if ALIGN:
        x = (gx + 1.) * (W_IN - 1) * .5
        y = (gy + 1.) * (H_IN - 1) * .5
        z = (gz + 1.) * (D_IN - 1) * .5
        ...
        z = ((gz + 1.) * D_IN - 1.) * .5
    if PADDING == 1:
        x, y, z = tl.maximum(0., tl.minimum(x, W_IN - 1.)), tl.maximum(0., tl.minimum(y, H_IN - 1.)), tl.maximum(0., tl.minimum(z, D_IN - 1.))
    elif PADDING == 2:
        if ALIGN:
            lo, sx, sy, sz = 0., tl.maximum(W_IN - 1., 1.), tl.maximum(H_IN - 1., 1.), tl.maximum(D_IN - 1., 1.)
        else:
            lo, sx, sy, sz = -.5, W_IN * 1., H_IN * 1., D_IN * 1.
        dx, dy, dz = tl.abs(x - lo), tl.abs(y - lo), tl.abs(z - lo)
        fx, fy, fz = tl.floor(dx / sx).to(tl.int32), tl.floor(dy / sy).to(tl.int32), tl.floor(dz / sz).to(tl.int32)
        ex, ey, ez = dx - fx.to(tl.float32) * sx, dy - fy.to(tl.float32) * sy, dz - fz.to(tl.float32) * sz
    ...
    if MODE == 0:
        ...
        xi += ((x - xf > .5) | ((x - xf == .5) & ((xi & 1) != 0))).to(tl.int32)
        yi += ((y - yf > .5) | ((y - yf == .5) & ((yi & 1) != 0))).to(tl.int32)
        zi += ((z - zf > .5) | ((z - zf == .5) & ((zi & 1) != 0))).to(tl.int32)
        valid = mask & ((PADDING != 0) | ~nan) & (xi >= 0) & (xi < W_IN) & (yi >= 0) & (yi < H_IN) & (zi >= 0) & (zi < D_IN)
        xi, yi, zi = tl.maximum(0, tl.minimum(xi, W_IN - 1)), tl.maximum(0, tl.minimum(yi, H_IN - 1)), tl.maximum(0, tl.minimum(zi, D_IN - 1))
        value = tl.load(input_ptr + base + zi * IN_SD + yi * IN_SH + xi * IN_SW, mask=mask, other=0.)
        value = tl.where(valid, value, 0.)
    else:
        x0, y0, z0 = tl.floor(x).to(tl.int32), tl.floor(y).to(tl.int32), tl.floor(z).to(tl.int32)
        ...
        for iz in tl.static_range(2):
            for iy in tl.static_range(2):
                for ix in tl.static_range(2):
                    ...
                    valid = mask & ((PADDING != 0) | ~nan) & (xx >= 0) & (xx < W_IN) & (yy >= 0) & (yy < H_IN) & (zz >= 0) & (zz < D_IN)
                    xx, yy, zz = tl.maximum(0, tl.minimum(xx, W_IN - 1)), tl.maximum(0, tl.minimum(yy, H_IN - 1)), tl.maximum(0, tl.minimum(zz, D_IN - 1))
                    v = tl.load(input_ptr + base + zz * IN_SD + yy * IN_SH + xx * IN_SW, mask=mask, other=0.).to(tl.float32)
                    value += tl.where(valid, v, 0.) * tl.where(ix == 0, 1. - wx, wx) * tl.where(iy == 0, 1. - wy, wy) * tl.where(iz == 0, 1. - wz, wz)
    tl.store(output_ptr + offsets, value, mask=mask)
...
def grid_sample(
    ...
    if input.dim() == 5:
        ...
        _grid_sample_3d_kunlunxin_kernel[(triton.cdiv(total, 64),)](
            output, input, grid, total,
            C=c, D_IN=input.shape[2], H_IN=input.shape[3], W_IN=input.shape[4],
            D_OUT=d_out, H_OUT=h_out, W_OUT=w_out,
            IN_SN=input.stride(0), IN_SC=input.stride(1), IN_SD=input.stride(2),
            IN_SH=input.stride(3), IN_SW=input.stride(4),
            GR_SN=grid.stride(0), GR_SD=grid.stride(1), GR_SH=grid.stride(2),
            GR_SW=grid.stride(3), GR_SC=grid.stride(4),
            MODE={"nearest": 0, "bilinear": 1}[mode],
            PADDING={"zeros": 0, "border": 1, "reflection": 2}[padding_mode],
            ALIGN=align_corners, BLOCK=64, num_warps=4, num_stages=1,
            isCloseVectorization=True, buffer_size_limit=2048, unroll_num=8,
        )
        return output
