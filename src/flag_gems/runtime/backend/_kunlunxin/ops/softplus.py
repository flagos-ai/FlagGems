...
# beta==1 fast path: stable identity (x+|x|)/2 + log(1+exp(-|x|)).
@pointwise_dynamic(
    is_tensor=[True, False, False],
    promotion_methods=[(0, "DEFAULT")],
    config=config_,
...
# (numel_upper_bound, BLOCK_SIZE, num_warps).
_TIERS = (
    (2048, 1024, 4),
    (16384, 2048, 4),
    (262144, 8192, 8),
...
def softplus(self, beta=1.0, threshold=20.0):
    ...
    # beta==1 && threshold>=17: guard is redundant, use unguarded fast path.
