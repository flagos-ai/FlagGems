...
class Conv2d(torch.autograd.Function):
    ...
    def forward(ctx, input, weight, bias, stride, padding, dilation, groups):
        ...
        # v2.5: bf16 hybrid by GROUPS — xpudnn conv2d_fusion<bf16> covers
        # groups=1 at all sizes (0.79-1.00x, incl. 12x12/18x18 small inputs);
        # groups>1 hits a ~0.08ms slow path (0.22-0.26x) while the fp32 cast
        # path gets 0.44-0.50x there. fp16 api:: path is immune (g2 0.78-0.83x).
        use_python_fp32 = (input.dtype == torch.bfloat16) and (groups != 1)
