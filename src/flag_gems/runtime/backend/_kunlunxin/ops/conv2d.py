...
@triton.jit
def _forward(
    x, w, b, y, n, hin, win, cout, hout, wout, cpg, opg, kh, kw,
    sh, sw, ph, pw, dh, dw, xsn, xsc, xsh, xsw, wso, wsi, wsh, wsw,
    ysn, ysc, ysh, ysw, HAS_BIAS: tl.constexpr, CPG: tl.constexpr,
    OPG: tl.constexpr, KH: tl.constexpr, KW: tl.constexpr, BLOCK: tl.constexpr,
):
    ...
    for r in range(KH):
        ...
        for s in range(KW):
            ...
            for ci in range(CPG):
                xv = tl.load(
                    x + ni * xsn + (group * CPG + ci) * xsc
                    + safe_ih * xsh + safe_iw * xsw,
                    mask=m < n * cout * plane, other=0.0,
                )
...
def _forward_spatial_channels4(
    x, w, b, y, n, hin, win, cout, hout, wout, cpg, kh, kw,
    sh, sw, ph, pw, dh, dw, xsn, xsc, xsh, xsw, wso, wsi, wsh, wsw,
    ysn, ysc, ysh, ysw, HAS_BIAS: tl.constexpr, CPG: tl.constexpr,
    KH: tl.constexpr, KW: tl.constexpr, BLOCK: tl.constexpr,
):
    p = tl.program_id(0) * BLOCK + tl.arange(0, BLOCK)
    ...
    for r in range(KH):
        ...
        for s in range(KW):
            ...
            for ci in range(CPG):
                ...
                acc0 += xv * tl.load(w + oc * wso + ci * wsi + r * wsh + s * wsw, mask=m0, other=0.0).to(tl.float32)
                acc1 += xv * tl.load(w + (oc + 1) * wso + ci * wsi + r * wsh + s * wsw, mask=m1, other=0.0).to(tl.float32)
                acc2 += xv * tl.load(w + (oc + 2) * wso + ci * wsi + r * wsh + s * wsw, mask=m2, other=0.0).to(tl.float32)
                acc3 += xv * tl.load(w + (oc + 3) * wso + ci * wsi + r * wsh + s * wsw, mask=m3, other=0.0).to(tl.float32)
    if HAS_BIAS:
    ...
    for r in range(KH):
        ...
        for s in range(KW):
            ...
            for ci in range(CPG):
                ...
                xv = tl.where(valid, xv, 0.0).to(tl.float32)
                acc0 += xv * tl.load(w + oc * wso + ci * wsi + r * wsh + s * wsw).to(tl.float32)
                acc1 += xv * tl.load(w + (oc + 1) * wso + ci * wsi + r * wsh + s * wsw).to(tl.float32)
                acc2 += xv * tl.load(w + (oc + 2) * wso + ci * wsi + r * wsh + s * wsw).to(tl.float32)
                acc3 += xv * tl.load(w + (oc + 3) * wso + ci * wsi + r * wsh + s * wsw).to(tl.float32)
                acc4 += xv * tl.load(w + (oc + 4) * wso + ci * wsi + r * wsh + s * wsw).to(tl.float32)
                acc5 += xv * tl.load(w + (oc + 5) * wso + ci * wsi + r * wsh + s * wsw).to(tl.float32)
                acc6 += xv * tl.load(w + (oc + 6) * wso + ci * wsi + r * wsh + s * wsw).to(tl.float32)
                acc7 += xv * tl.load(w + (oc + 7) * wso + ci * wsi + r * wsh + s * wsw).to(tl.float32)
    if HAS_BIAS:
...
def _forward_spatial_channels16(
    ...
    for r in range(KH):
        ...
        for s in range(KW):
            ...
            safe_iw = tl.where(valid, iw, 0)
            for ci in range(CPG):
                xv = tl.load(x + ni * xsn + ci * xsc + safe_ih * xsh + safe_iw * xsw, mask=pmask, other=0.0)
                xv = tl.where(valid, xv, 0.0).to(tl.float32)
                acc0 += xv * tl.load(w + oc * wso + ci * wsi + r * wsh + s * wsw).to(tl.float32)
                acc1 += xv * tl.load(w + (oc + 1) * wso + ci * wsi + r * wsh + s * wsw).to(tl.float32)
                acc2 += xv * tl.load(w + (oc + 2) * wso + ci * wsi + r * wsh + s * wsw).to(tl.float32)
                ...
                acc6 += xv * tl.load(w + (oc + 6) * wso + ci * wsi + r * wsh + s * wsw).to(tl.float32)
                acc7 += xv * tl.load(w + (oc + 7) * wso + ci * wsi + r * wsh + s * wsw).to(tl.float32)
                acc8 += xv * tl.load(w + (oc + 8) * wso + ci * wsi + r * wsh + s * wsw).to(tl.float32)
                acc9 += xv * tl.load(w + (oc + 9) * wso + ci * wsi + r * wsh + s * wsw).to(tl.float32)
                acc10 += xv * tl.load(w + (oc + 10) * wso + ci * wsi + r * wsh + s * wsw).to(tl.float32)
                acc11 += xv * tl.load(w + (oc + 11) * wso + ci * wsi + r * wsh + s * wsw).to(tl.float32)
                acc12 += xv * tl.load(w + (oc + 12) * wso + ci * wsi + r * wsh + s * wsw).to(tl.float32)
                acc13 += xv * tl.load(w + (oc + 13) * wso + ci * wsi + r * wsh + s * wsw).to(tl.float32)
                acc14 += xv * tl.load(w + (oc + 14) * wso + ci * wsi + r * wsh + s * wsw).to(tl.float32)
                acc15 += xv * tl.load(w + (oc + 15) * wso + ci * wsi + r * wsh + s * wsw).to(tl.float32)
    if HAS_BIAS:
        acc0 += tl.load(b + oc).to(tl.float32)
        acc1 += tl.load(b + oc + 1).to(tl.float32)
        acc2 += tl.load(b + oc + 2).to(tl.float32)
...
@triton.jit
def _input_grad(
    dy, w, dx, n, cin, hin, win, hout, wout, cpg, opg, kh, kw,
    sh, sw, ph, pw, dh, dw, dysn, dysc, dysh, dysw, wso, wsi, wsh, wsw,
    dxsn, dxsc, dxsh, dxsw, BM: tl.constexpr, BC: tl.constexpr,
    BO: tl.constexpr,
):
    ...
    for r in range(0, kh):
        ...
        for s in range(0, kw):
            ...
            for obase in range(0, opg, BO):
                ...
                gy = tl.load(
                    dy + ni[:, None, None] * dysn
                    + (g[None, :, None] * opg + o[None, None, :]) * dysc
                    + oh[:, None, None] * dysh + ow[:, None, None] * dysw,
                    mask=(ni[:, None, None] < n) & (c[None, :, None] < cin)
                    & (o[None, None, :] < opg) & hvalid[:, None, None]
                    & wvalid[:, None, None], other=0.0,
                )
                ww = tl.load(
                    w + (g[:, None] * opg + o[None, :]) * wso + lc[:, None] * wsi
                    + r * wsh + s * wsw,
                    mask=(c[:, None] < cin) & (o[None, :] < opg), other=0.0,
                )
                acc += tl.sum(gy * ww[None, :, :], axis=2)
    tl.store(dx + ni[:, None] * dxsn + c[None, :] * dxsc + ih[:, None] * dxsh + iw[:, None] * dxsw,
             acc, mask=(ni[:, None] < n) & (c[None, :] < cin))


@libentry()
@triton.jit
def _weight_grad(
    x, dy, grad_w, n, hin, win, cout, hout, wout, cpg, opg, kh, kw,
    sh, sw, ph, pw, dh, dw, xsn, xsc, xsh, xsw, dysn, dysc, dysh, dysw,
    gws0, gws1, gws2, gws3, BP: tl.constexpr, BK: tl.constexpr,
):
    ...
    for pbase in range(0, total, BP):
        ...
        xv = tl.load(
            x + ni[:, None] * xsn + (g[None, :] * cpg + ci[None, :]) * xsc
            + ih * xsh + iw * xsw,
            mask=(p[:, None] < total) & (oc[None, :] < cout)
            & (ih >= 0) & (ih < hin) & (iw >= 0) & (iw < win), other=0.0,
        )
        gy = tl.load(dy + ni[:, None] * dysn + oc[None, :] * dysc
                     + oh[:, None] * dysh + ow[:, None] * dysw,
                     mask=(p[:, None] < total) & (oc[None, :] < cout), other=0.0)
...
@libentry()
@triton.jit
def _bias_grad(dy, grad_b, n, cout, hout, wout, dysn, dysc, dysh, dysw,
               BP: tl.constexpr, BO: tl.constexpr):
    ...
    for pbase in range(0, total, BP):
        ...
        q = p % (hout * wout)
        oh, ow = q // wout, q % wout
        value = tl.load(dy + ni[:, None] * dysn + o[None, :] * dysc
                        + oh[:, None] * dysh + ow[:, None] * dysw,
                        mask=(p[:, None] < total) & (o[None, :] < cout), other=0.0)
...
def _output_shape(padding, kh, kw, dh, dw, sh, sw, hin, win):
    if isinstance(padding, str):
        if padding == "valid":
            return 0, 0, (hin - dh * (kh - 1) - 1) // sh + 1, (win - dw * (kw - 1) - 1) // sw + 1
    ...
    return ph, pw, (hin + 2 * ph - dh * (kh - 1) - 1) // sh + 1, (win + 2 * pw - dw * (kw - 1) - 1) // sw + 1
...
class Conv2d(torch.autograd.Function):
    ...
    def forward(ctx, input, weight, bias, stride, padding, dilation, groups):
        ...
        if groups <= 0 or input.shape[1] % groups or weight.shape[0] % groups or weight.shape[1] * groups != input.shape[1]:
            raise RuntimeError("conv2d input, weight, and groups have incompatible channels")
        ...
        output = torch.empty((n, cout, hout, wout), device=input.device, dtype=input.dtype)
        opg = cout // groups
        reduction = cpg * kh * kw
        ...
        if use_channels16:
            block = 128
            _forward_spatial_channels16[(triton.cdiv(n * hout * wout, block), cout // 16)](
                input, weight, bias, output, n, hin, win, cout, hout, wout, cpg, kh, kw,
                sh, sw, ph, pw, dh, dw, *input.stride(), *weight.stride(), *output.stride(),
                HAS_BIAS=bias is not None, CPG=cpg, KH=kh, KW=kw, BLOCK=block,
                num_warps=4)
        elif use_channels8:
            block = 128
            _forward_spatial_channels8[(triton.cdiv(n * hout * wout, block), cout // 8)](
                input, weight, bias, output, n, hin, win, cout, hout, wout, cpg, kh, kw,
                sh, sw, ph, pw, dh, dw, *input.stride(), *weight.stride(), *output.stride(),
                HAS_BIAS=bias is not None, CPG=cpg, KH=kh, KW=kw, BLOCK=block,
                num_warps=4)
            ...
            block = 128
            _forward_spatial_channels4[(triton.cdiv(n * hout * wout, block), triton.cdiv(cout, 4))](
                input, weight, bias, output, n, hin, win, cout, hout, wout, cpg, kh, kw,
                sh, sw, ph, pw, dh, dw, *input.stride(), *weight.stride(), *output.stride(),
                HAS_BIAS=bias is not None, CPG=cpg, KH=kh, KW=kw, BLOCK=block,
                num_warps=4)
            ...
            grid = (triton.cdiv(n * hout * wout, block), cout) if use_spatial_tile else (triton.cdiv(output_elements, block),)
            kernel[grid](
                input, weight, bias, output, n, hin, win, cout, hout, wout, cpg, opg, kh, kw,
                sh, sw, ph, pw, dh, dw, *input.stride(), *weight.stride(), *output.stride(),
                HAS_BIAS=bias is not None, CPG=cpg, OPG=opg, KH=kh, KW=kw, BLOCK=block,
                num_warps=4)
        ctx.save_for_backward(input, weight)
        ctx.args = (stride, padding, dilation, groups, ph, pw, hout, wout, bias is not None)
        return output
    ...
    def backward(ctx, out_grad):
        ...
        if need_x:
            ...
            _input_grad[(triton.cdiv(n * hin * win, 32), triton.cdiv(cin, 32))](
                out_grad, weight, grad_x, n, cin, hin, win, hout, wout, cpg, cout // groups, kh, kw,
                sh, sw, ph, pw, dh, dw, *out_grad.stride(), *weight.stride(), *grad_x.stride(), BM=32, BC=32, BO=32)
        if need_w:
            grad_w = torch.empty_like(weight)
            _weight_grad[(triton.cdiv(weight.numel(), 64),)](
                input, out_grad, grad_w, n, hin, win, cout, hout, wout, cpg, cout // groups, kh, kw,
                sh, sw, ph, pw, dh, dw, *input.stride(), *out_grad.stride(), *grad_w.stride(), BP=64, BK=64)
        if has_bias and need_b:
            grad_b = torch.empty((cout,), device=out_grad.device, dtype=out_grad.dtype)
            _bias_grad[(triton.cdiv(cout, 64),)](out_grad, grad_b, n, cout, hout, wout, *out_grad.stride(), BP=128, BO=64)
