...
# Triton implementation of fused_recurrent_gated_delta_rule_fwd for
# Kunlunxin. Kernel structure follows
# flag_gems/fused/FLA/fused_recurrent.py (flash-linear-attention, MIT);
# only the strided-address (Qwen3-Next) kernel variant is used here, since
# the other variants (contiguous-packed and large-T) fail to compile on the
# XPU Triton backend (OutOfResources: uni_sram). The strided kernel is
# layout-agnostic and handles all T. Unlike the generic implementation, the
# "no ssm_state_indices" path is fully supported: initial/final states are
# indexed by the sequence id (i_n), matching the semantics of the previous
...
def fused_recurrent_gated_delta_rule_fwd_kernel(
    ...
    if USE_INITIAL_STATE:
        # Without ssm_state_indices, the initial state is indexed by the
        # sequence id (matching the previous Python-loop implementation).
