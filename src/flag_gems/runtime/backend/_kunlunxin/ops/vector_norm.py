...
def l1_norm_rows_kernel_3(
    ...
    for offset in tl.range(0, full_size, 8):
        values = tl.load(
            Next + row * NEXT_SIZE + offset + tl.arange(0, 8)
        ).to(tl.float32)
        total += tl.sum(values)
...
def _l2_flat(x, out):
    ...
    while mid_cnt > _L2_BLOCK:
        ...
        nxt = torch.empty_strided((next_cnt,), (1,), dtype=torch.float32, device=x.device)
        if rows2:
...
if _TLE_GPU_OK:
    ...
    def _tle_norm_row_kernel(
        ...
        if SQ_ONLY:
            ...
            c_dtype = DTYPE
        a_lmem = tle.gpu.alloc([XBLOCK, YBLOCK], dtype=DTYPE,
                               layout=None, scope=tle.gpu.lmem)
        c_lmem = tle.gpu.alloc([XBLOCK], dtype=c_dtype,
                               layout=None, scope=tle.gpu.lmem)
...
def _tle_flat(x, out, dtype):
    ...
    if r:
        yb_t = 1 << (int(r).bit_length() - 1)  # pow2 floor, <= r: zero-fill covers the rest
...
def vector_norm(x, ord=2, dim=None, keepdim=False, dtype=None):
    ...
    with torch_device_fn.device(x.device):
        if len(dim) == x.ndim:
            ...
            if ord == 2 and M > 0:
                out = torch.empty_strided(shape, [1] * len(shape), dtype=dtype, device=x.device)
            ...
            mid = torch.empty_strided([BLOCK_MID], [1], dtype=torch.float32, device=x.device)
            ...
            if ord == 2:
                ...
                mid = torch.empty_strided((M, MID_SIZE), (MID_SIZE, 1), dtype=torch.float32, device=x.device)
                ...
                if MID_SIZE <= 1024:
                    ...
                    next_mid = torch.empty_strided(
                        (M, NEXT_SIZE), (NEXT_SIZE, 1), dtype=torch.float32, device=x.device
                    )
