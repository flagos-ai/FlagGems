...
from ._scaled_dot_product_fused_attention_overrideable import _scaled_dot_product_fused_attention_overrideable
...
# [opsns 2026-09-17] Mirror the two routed attention ops into flag_gems.ops as
# well: official benchmark files access flag_gems.ops.<op> directly, while the
# test suite uses the flag_gems.<op> alias (backend-replaced at SpecOpRegistrar
# time, which is when this module is first imported by the loader).
