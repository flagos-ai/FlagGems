...
def _validate_lengths(data, lengths, axis, unsafe):
    ...
    valid_lengths = torch.all(
        lengths_detached.sum(dim=-1) == data.size(axis)
    ).item()
    ...
    if segment_length > 0:
        ...
        if IS_SUM or IS_MEAN:
            ...
            for block_idx in tl.static_range(MAX_BLOCKS):
                ...
                match = tl.where(
                    output_is_nan, values != values, values == output_value
                ) & mask
            ...
            for block_idx in tl.static_range(MAX_BLOCKS):
                ...
                product_values = tl.where(nan_mask | zero_mask | ~mask, 1.0, values)
                product *= tl.reduce(
                    product_values, axis=0, combine_fn=_multiply
                )
...
def _segment_reduce_backward_element_kernel(
    ...
    output_idx = (
        (outer_idx * SEGMENT_COUNT + safe_segment_idx) * inner_size + inner_idx
    )
    ...
    with torch_device_fn.device(data.device):
        if kernel_reduce in ("max", "min", "prod"):
            ...
            _segment_reduce_backward_element_kernel[grid](
                ...
                LOG_SEGMENT_COUNT=max(
                    1, math.ceil(math.log2(segment_count + 1))
                ),
                MAX_BLOCKS=triton.cdiv(data_size_axis, block_size),
                BLOCK_SIZE=block_size,
            )
