...
def _avg_pool3d_backward_gather_kernel(
    ...
    for kd in tl.static_range(0, KD):
        ...
        d_matches = (
            (output_d * SD + kd == input_d + PD) & (output_d >= 0) & (output_d < OUT_D)
        )
        start_d = output_d * SD - PD
        for kh in tl.static_range(0, KH):
            output_h = (input_h + PH - kh) // SH
            h_matches = (
                (output_h * SH + kh == input_h + PH)
                & (output_h >= 0)
                & (output_h < OUT_H)
            )
            ...
            for kw in tl.static_range(0, KW):
                ...
                value = tl.load(
                    grad_output_ptr + output_offset, mask=matches, other=0.0
                ).to(tl.float32)
