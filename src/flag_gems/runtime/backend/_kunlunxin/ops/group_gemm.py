...
# Kunlunxin backend override of flag_gems/ops/group_gemm.py.
# Same kernels as the generic module, with two autotuning fixes:
#   1. decorator order: @libtuner outermost (the generic module stacks
#      @libentry() outside @libtuner(), and libentry's kernel-cache key
#      intentionally drops do_not_specialize values (M/N/K keep only the
#      type class), so only the first shape ever entered the autotuner and
#      every later shape silently reused the first shape's config);
#   2. two extra BLOCK_M=512 candidates for large-M/small-K shapes.
# Imported by _kunlunxin/ops/grouped_mm.py, which dispatches here for the
# large-M / small-K regime; no __init__ change needed.
import logging
...
# Persistent grouped_mm kernel launches NUM_SMS * _GMM_GRID_MULT programs: the
# default one-program-per-core scheme under-hides memory latency for large-M /
# small-K shapes (grid x4 measured ~25% faster there, no regression elsewhere).
_GMM_GRID_MULT = 4
...
def get_autotune_config(pre_hook=None):
    return [
        ...
        # Large BLOCK_M pays off on large-M / small-K shapes; num_warps=16 was
        # measured bimodal on this backend (5.19 / 6.53 ms on the same shape),
        # so only the stable num_warps=8 variant is kept.
        triton.Config(
            {"BLOCK_M": 512, "BLOCK_N": 128, "BLOCK_K": 64, "GROUP_M": 8},
            num_stages=3,
            num_warps=8,
            pre_hook=pre_hook,
        ),
...
def group_mm(A: torch.Tensor, B: torch.Tensor, offs: torch.Tensor) -> torch.Tensor:
    ...
    if _support_host_tensor_descriptor and supports_tma():
        ...
        grouped_mm_kernel[(NUM_SMS * _GMM_GRID_MULT,)](
            A,
            B,
            C,
            offs,
