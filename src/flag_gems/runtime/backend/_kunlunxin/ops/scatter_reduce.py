...
def _scatter_reduce_2d_kernel(
    inp, index, src, out, input_size0, input_size1, index_size0, index_size1,
    src_size1, DIM: tl.constexpr, REDUCE: tl.constexpr,
    INCLUDE_SELF: tl.constexpr, BLOCK: tl.constexpr,
):
    output_offset = tl.program_id(0)
    ...
    selected = valid & (indices == destination)
    values = tl.load(src + src_base + offsets * src_stride, mask=selected, other=0.0).to(tl.float32)
    self_value = tl.load(inp + output_offset).to(tl.float32)
    selected_count = tl.sum(selected.to(tl.int32), axis=0)
    if REDUCE == 0:
        reduced = tl.sum(tl.where(selected, values, 0.0), axis=0)
        ...
        for offset in tl.static_range(BLOCK):
            valid_offset = valid_base & (offset < dim_size)
            index_value = tl.load(index + index_base + offset * index_stride, mask=valid_offset, other=-1)
            value = tl.load(src + src_base + offset * src_stride, mask=valid_offset, other=1.0).to(tl.float32)
...
def scatter_reduce(inp, dim, index, src, reduce, *, include_self=True):
    ...
    if reduce not in _REDUCTIONS:
        raise RuntimeError(f"reduce argument must be either sum, prod, mean, amax or amin, got {reduce}")
    if inp.ndim == 0:
        raise RuntimeError("scatter_reduce(): Expected self to have non-zero dimensionality")
    ...
    for axis, size in enumerate(index.shape):
        if size > src.shape[axis] or (axis != dim and size > inp.shape[axis]):
            raise RuntimeError("index must not be larger than src or self outside the scatter dimension")
    ...
    if block > 65536:
        raise RuntimeError("Kunlunxin scatter_reduce supports at most 65536 source elements along dim")
