    ...
    if A.dtype == torch.bool:
        ...
        if a32 is not None:
            return bitwise_or_func_scalar(
                a32, _pack_scalar_bool_to_i32(B)
            ).view(torch.bool).reshape(A.shape)
        return bitwise_or_func_scalar(A.view(torch.int8), int(B)).view(torch.bool)
    if A.dtype == torch.int16:
        a32 = _try_view_i32(A)
        if a32 is not None:
            return bitwise_or_func_scalar(
                a32, _pack_scalar_i16_to_i32(B)
            ).view(torch.int16).reshape(A.shape)
...
def bitwise_or_scalar_tensor(A, B):
    ...
    if B.dtype == torch.bool:
        ...
        if b32 is not None:
            return bitwise_or_func_scalar(
                b32, _pack_scalar_bool_to_i32(A)
            ).view(torch.bool).reshape(B.shape)
        return bitwise_or_func_scalar(B.view(torch.int8), int(A)).view(torch.bool)
    if B.dtype == torch.int16:
        b32 = _try_view_i32(B)
        if b32 is not None:
            return bitwise_or_func_scalar(
                b32, _pack_scalar_i16_to_i32(A)
            ).view(torch.int16).reshape(B.shape)
