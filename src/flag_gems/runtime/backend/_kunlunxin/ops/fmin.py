...
def _pick_block_size(n_elements):
    # BLOCK=16384 is the measured memory-bound sweet spot on XPU for n >= 2*16384;
    # below that a single oversized masked tile costs more launch floor than the
    # original BLOCK=1024 kernel (tiny-shape bench cells would otherwise regress).
    return 16384 if n_elements >= 32768 else 1024
