...
def _validate(inp, dim, index, src):
    ...
    assert inp.ndim == src.ndim, "Self and source should have the same number of dimensions"
    assert all(
        (inp.size(i) == src.size(i)) or i == dim for i in range(inp.ndim)
    ), "src.size(d) == self.size(d) for all dimensions d != dim"
    assert bool(((0 <= index) & (index < inp.size(dim))).all()), (
...
def index_copy_(inp, dim, index, src):
    ...
    if inp.ndim == 1:
        _index_copy_rank1[grid](
            inp, index, src, n_elements, inp.stride(0), src.stride(0), BLOCK=block, num_warps=8
        )
    elif inp.ndim == 2:
