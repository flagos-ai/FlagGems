...
def celoss_indices_kernel(
    ...
    offset_d = (pid_d * BLOCK_D).to(tl.int64) + tl.arange(0, BLOCK_D)

    tgt_ptrs = tgt_ptr + (pid_n * D).to(tl.int64) + offset_d
    tgt_mask = offset_d < D
    ...
    for off in range(0, C, BLOCK_C):
        ...
        inp_ptrs = inp_ptr + (
            (pid_n * C * D).to(tl.int64)
            + (offset_c[:, None] * D).to(tl.int64)
            + offset_d[None, :]
        ).to(tl.int64)
    ...
    inp_tgt_ptrs = inp_ptr + (pid_n * C * D).to(tl.int64) + tgt * D + offset_d
    inp_tgt = tl.load(inp_tgt_ptrs, mask=tgt_mask, other=-float("inf")).to(tl.float32)

    out = final_sum + final_max - inp_tgt
    w_tgt_ptrs = w_tgt_ptr + (pid_n * D).to(tl.int64) + offset_d
    ...
    out *= w_tgt
    out_ptrs = out_ptr + (pid_n * D).to(tl.int64) + offset_d
...
def celoss_indices_bwd(
    ...
    tgt_mask = offset_d < D
    tgt = tl.load(tgt_ptrs, mask=tgt_mask, other=0)
    out_grad_ptrs = out_grad_ptr + (pid_n * D).to(tl.int64) + offset_d
