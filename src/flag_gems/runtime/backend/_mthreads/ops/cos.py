...
from flag_gems.ops.cos import cos as default_cos
from flag_gems.ops.cos import cos_ as default_cos_
...
def cos_kernel(
    ...
    # tl.cos only supports fp32/fp64, convert to fp32 first
    x_fp32 = x.to(tl.float32)
    out_fp32 = tl.cos(x_fp32)
...
def _launch_cos(x: torch.Tensor, out: torch.Tensor, dtype_size: int):
    n_elements = out.numel()
    grid = lambda meta: (triton.cdiv(n_elements, meta["BLOCK_SIZE"]),)
    with torch_device_fn.device(out.device):
        cos_kernel[grid](x, out, n_elements, dtype_size)
    return out
...
def cos(x):
    logger.debug("GEMS_MTHREADS COS")
    use_triton, dtype_size = _use_triton_kernel(x)
    if not use_triton:
        return default_cos(x)

    out = torch.empty_like(x)
    return _launch_cos(x, out, dtype_size)


def cos_(x):
    logger.debug("GEMS_MTHREADS COS_")
    use_triton, dtype_size = _use_triton_kernel(x)
    if not use_triton:
        return default_cos_(x)

    return _launch_cos_inplace(x)
