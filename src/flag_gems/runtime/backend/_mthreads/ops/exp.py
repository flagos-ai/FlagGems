...
from flag_gems.ops.exp import exp as default_exp
from flag_gems.ops.exp import exp_ as default_exp_
...
def exp_kernel(
    ...
    # tl.exp only supports fp32/fp64, convert to fp32 first
    x_fp32 = x.to(tl.float32)
    out_fp32 = tl.exp(x_fp32)
...
def _launch_exp(x: torch.Tensor, out: torch.Tensor, dtype_size: int):
    n_elements = out.numel()
    grid = lambda meta: (triton.cdiv(n_elements, meta["BLOCK_SIZE"]),)
    with torch_device_fn.device(out.device):
        exp_kernel[grid](x, out, n_elements, dtype_size)
    return out
...
def exp(x):
    logger.debug("GEMS_MTHREADS EXP")
    use_triton, dtype_size = _use_triton_kernel(x)
    if not use_triton:
        return default_exp(x)

    out = torch.empty_like(x)
    return _launch_exp(x, out, dtype_size)


def exp_(x):
    logger.debug("GEMS_MTHREADS EXP_")
    use_triton, dtype_size = _use_triton_kernel(x)
    if not use_triton:
        return default_exp_(x)

    return _launch_exp_inplace(x)
