...
def repeat_kernel_2d(
    ...
    # Load from input
    inp_ptrs = inp_ptr + inp_offs_m[:, None] * inp_stride0 + inp_offs_n[None, :] * inp_stride1
...
def repeat_kernel_3d(
    ...
    inp_ptrs = inp_ptr + inp_m * inp_stride0 + inp_n[:, None] * inp_stride1 + inp_k[None, :] * inp_stride2
    data = tl.load(inp_ptrs, mask=mask, other=0.0)

    out_ptrs = out_ptr + m_idx * out_stride0 + offs_n[:, None] * out_stride1 + offs_k[None, :] * out_stride2
    tl.store(out_ptrs, data, mask=mask)
...
def repeat_kernel_4d(
    ...
    inp_ptrs = (inp_ptr + inp_m * inp_stride0 + inp_n * inp_stride1 +
                inp_k[:, None] * inp_stride2 + inp_l[None, :] * inp_stride3)
    data = tl.load(inp_ptrs, mask=mask, other=0.0)

    out_ptrs = (out_ptr + m_idx * out_stride0 + n_idx * out_stride1 +
                offs_k[:, None] * out_stride2 + offs_l[None, :] * out_stride3)
    tl.store(out_ptrs, data, mask=mask)
...
def repeat_kernel_nd_flat(
    ...
    out_ptr,
    num_tasks,
    inp_shape0, inp_shape1, inp_shape2, inp_shape3, inp_shape4,
    out_shape0, out_shape1, out_shape2, out_shape3, out_shape4,
    inp_stride0, inp_stride1, inp_stride2, inp_stride3, inp_stride4,
    ...
    for idx in range(pid * BLOCK_SIZE, num_tasks, num_ctas * BLOCK_SIZE):
        ...
        inp_offset = (inp_idx0 * inp_stride0 + inp_idx1 * inp_stride1 +
                      inp_idx2 * inp_stride2 + inp_idx3 * inp_stride3 + inp_idx4 * inp_stride4)
        out_offset = (out_idx0 * out_stride0 + out_idx1 * out_stride1 +
                      out_idx2 * out_stride2 + out_idx3 * out_stride3 + out_idx4 * out_stride4)
...
def repeat(inp: torch.Tensor, sizes) -> torch.Tensor:
    ...
    for i in range(len(in0_shape)):
        assert sizes_shape[i] >= 0, f'the number of repetitions per dimension out of range (expected to >= 0) but got {sizes_shape[i]}'
        if sizes_shape[i] == 0:
            is_empty = True
    ...
    with torch_device_fn.device(inp.device.index):
        if rank == 1:
            ...
            repeat_kernel_1d[grid](
                ...
                in0_shape[0], out_shape[0],
            )
        elif rank == 2:
            # 2D case - use 2D blocking with autotune
            grid = lambda META: (triton.cdiv(out_shape[0], META["BLOCK_M"]), triton.cdiv(out_shape[1], META["BLOCK_N"]))
            repeat_kernel_2d[grid](
                inp, out,
                inp_strides[0], inp_strides[1],
                out_strides[0], out_strides[1],
                in0_shape[0], in0_shape[1],
                out_shape[0], out_shape[1],
            )
        elif rank == 3:
            # 3D case
            grid = lambda META: (out_shape[0], triton.cdiv(out_shape[1], META["BLOCK_N"]) * triton.cdiv(out_shape[2], META["BLOCK_K"]))
            repeat_kernel_3d[grid](
                inp, out,
                inp_strides[0], inp_strides[1], inp_strides[2],
                out_strides[0], out_strides[1], out_strides[2],
                in0_shape[0], in0_shape[1], in0_shape[2],
                out_shape[0], out_shape[1], out_shape[2],
            )
        elif rank == 4:
            # 4D case - use 2D grid kernel
            num_mn = out_shape[0] * out_shape[1]
            grid = lambda META: (num_mn, triton.cdiv(out_shape[2], META["BLOCK_K"]) * triton.cdiv(out_shape[3], META["BLOCK_L"]))
            ...
            grid = lambda META: (min(65535, triton.cdiv(num_tasks, META["BLOCK_SIZE"])),)
            repeat_kernel_nd_flat[grid](
                inp, out,
                num_tasks,
                in0_shape_padded[0], in0_shape_padded[1], in0_shape_padded[2], in0_shape_padded[3], in0_shape_padded[4],
                out_shape_padded[0], out_shape_padded[1], out_shape_padded[2], out_shape_padded[3], out_shape_padded[4],
                inp_strides_padded[0], inp_strides_padded[1], inp_strides_padded[2], inp_strides_padded[3], inp_strides_padded[4],
                out_strides_padded[0], out_strides_padded[1], out_strides_padded[2], out_strides_padded[3], out_strides_padded[4],
                rank=rank,
            )
