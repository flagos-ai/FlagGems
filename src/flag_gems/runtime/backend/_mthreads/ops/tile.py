...
def tile(inp: torch.Tensor, dims) -> torch.Tensor:
    ...
    for i in range(len(in0_shape)):
        assert dims_shape[i] >= 0, f'the number of repetitions per dimension out of range (expected to >= 0) but got {dims_shape[i]}'
        if dims_shape[i] == 0:
            is_empty = True
    ...
    with torch_device_fn.device(inp.device.index):
        if rank == 1:
            ...
            tile_kernel_nd_flat[grid](
                inp, out,
                num_tasks,
                in0_shape[0], in0_shape[1], in0_shape[2], in0_shape[3], in0_shape[4],
                out_shape[0], out_shape[1], out_shape[2], out_shape[3], out_shape[4],
                inp_strides[0], inp_strides[1], inp_strides[2], inp_strides[3], inp_strides[4],
