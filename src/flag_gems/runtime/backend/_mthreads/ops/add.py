...
from flag_gems.ops.add import add as default_add
from flag_gems.ops.add import add_ as default_add_
...
@libentry()
@triton.jit
def add_tensor_tensor_inplace_no_alpha_kernel(
    x_ptr,
    y_ptr,
    n_elements,
    BLOCK_SIZE: tl.constexpr,
...
@libentry()
@triton.jit
def add_tensor_tensor_inplace_kernel(
    x_ptr,
    y_ptr,
    alpha,
    n_elements,
...
@libentry()
@triton.jit
def add_tensor_scalar_inplace_no_alpha_kernel(
    x_ptr,
    scalar,
    n_elements,
    BLOCK_SIZE: tl.constexpr,
...
@libentry()
@triton.jit
def add_tensor_scalar_inplace_kernel(
    x_ptr,
    scalar,
    alpha,
    n_elements,
...
def add(A, B, *, alpha=1):
    ...
    if isinstance(A, torch.Tensor) and isinstance(B, torch.Tensor):
        ...
        with torch_device_fn.device(out.device):
            add_tensor_tensor_kernel[grid](A, B, out, alpha, n_elements)
        return out
        ...
        grid = lambda meta: (triton.cdiv(n_elements, meta["BLOCK_SIZE"]),)
        with torch_device_fn.device(out.device):
            add_tensor_scalar_kernel[grid](A, out, B, alpha, n_elements)
        return out
...
def add_(A, B, *, alpha=1):
    logger.debug("GEMS_MTHREADS ADD_")
    if isinstance(A, torch.Tensor) and isinstance(B, torch.Tensor):
        if not (_use_triton_kernel(A) and _use_triton_kernel(B) and A.shape == B.shape):
            return default_add_(A, B, alpha=alpha)
        ...
        with torch_device_fn.device(A.device):
            if alpha == 1:
                add_tensor_tensor_inplace_no_alpha_kernel[grid](
                    A, B, n_elements, BLOCK_SIZE=BLOCK_SIZE, num_warps=num_warps
                )
            else:
                add_tensor_tensor_inplace_kernel[grid](
        ...
        with torch_device_fn.device(A.device):
            if alpha == 1:
                add_tensor_scalar_inplace_no_alpha_kernel[grid](
                    A, B, n_elements, BLOCK_SIZE=BLOCK_SIZE, num_warps=num_warps
                )
            else:
                add_tensor_scalar_inplace_kernel[grid](
                    A, B, alpha, n_elements, BLOCK_SIZE=BLOCK_SIZE, num_warps=num_warps
                )
        return A
    else:
