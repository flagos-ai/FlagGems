...
def feature_dropout(input, p, train=True):
    ...
    # On mthreads, the test reference computes `channel * scale` where `channel` is a
    # per-channel 1-D slice of size `spatial_size`. torch_musa's elementwise
    # `tensor * python_float` rounds the scalar to the tensor dtype when the operand
    # has more than one element, but keeps full precision for a single-element
    # operand. Round `scale` to the input dtype to match the reference's per-channel
    # rounding when there is a spatial extent; keep full precision for the
    # single-element (spatial_size == 1) case so the result matches the nearest
    # rounding the reference uses there.
    if spatial_size >= 2 and input.dtype in (
        torch.float16,
        torch.bfloat16,
    ):
        scale = torch.scalar_tensor(scale, dtype=input.dtype).item()
