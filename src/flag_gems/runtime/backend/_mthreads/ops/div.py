...
def true_divide(A, B):
    logger.debug("GEMS MTHREADS TRUE_DIVIDE")
...
def true_divide_out(A, B, out):
    logger.debug("GEMS MTHREADS TRUE_DIVIDE OUT")
    if isinstance(A, torch.Tensor) and isinstance(B, torch.Tensor):
...
def true_divide_(A, B):
    logger.debug("GEMS MTHREADS TRUE_DIVIDE_")
...
# NOTE: torch's trunc-division is the correctly-rounded (round-to-nearest)
# quotient truncated toward zero. mthreads' plain `x / y` and `div_rz` can both
# drift ~1 ULP from the correctly-rounded result (and div_rz additionally
# rounds toward zero before trunc), so neither matches torch on fp32. `div_rn`
# maps to the round-to-nearest division intrinsic, and trunc() of that matches
# torch's trunc-division exactly on fp32.
...
def trunc_divide(A, B):
    logger.debug("GEMS MTHREADS TRUNC_DIVIDE")
...
def trunc_divide_(A, B):
    logger.debug("GEMS MTHREADS TRUNC_DIVIDE_")
    # Integer types: use dedicated int kernels (Triton // is C-style truncation)
...
def floor_divide(A, B):
    logger.debug("GEMS MTHREADS FLOOR_DIVIDE")
    if isinstance(A, torch.Tensor) and isinstance(B, torch.Tensor):
...
def floor_divide_(A, B):
    logger.debug("GEMS MTHREADS FLOOR_DIVIDE_")
    if isinstance(B, torch.Tensor):
...
def div_mode(A, B, rounding_mode=None):
    logger.debug("GEMS MTHREADS DIV_MODE")
...
def div_mode_(A, B, rounding_mode=None):
    logger.debug("GEMS MTHREADS DIV_MODE_")
    if rounding_mode is None:
