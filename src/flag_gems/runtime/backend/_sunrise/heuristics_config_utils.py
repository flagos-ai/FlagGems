...
def argmax_heur_block_m(args):
    return 1 if args["M"] < 4096 else 8
...
def gather_heur_block_m(args):
    return min(1, triton.next_power_of_2(triton.cdiv(args["N"], 2048)))
...
def batch_norm_heur_block_m(args):
    return min(256, triton.next_power_of_2(args["batch_dim"]))
...
HEURISTICS_CONFIGS = {
    ...
    "sum_inner": {
        "TILE_N": sum_heur_tile_n_inner,
        "ONE_TILE_PER_CTA": sum_heur_one_tile_per_cta,
        "num_warps": sum_heur_num_warps_inner,
    },
    "sum_non_inner": {
        "TILE_K": sum_heur_tile_k,
        "TILE_N": sum_heur_tile_n_non_inner,
        "ONE_TILE_PER_CTA": softmax_heur_one_tile_per_cta,
        "num_warps": softmax_heur_num_warps_non_inner,
    },
}
