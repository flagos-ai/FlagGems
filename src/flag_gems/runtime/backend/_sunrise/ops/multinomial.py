...
def multinomial(prob, n_samples, with_replacement=False, *, gen=None):
    logger.debug("GEMS_SUNRISE MULTINOMIAL")
    ...
    if (not with_replacement) or n_samples == 1:
        # In case of with_replacement, sampling is approximated by selecting
        # the top k indices over sorted probabilities with an exponential perturbation
        ...
        if n_samples == 1:
            ...
            _, indices = torch.topk(s.cpu(), n_samples, dim=-1)
            return indices.to(prob.device).to(torch.int64)
