...
def scatter_reduce_prod_2d_kernel(
    ...
    for i in range(LOOP):
        ...
        block_stop = False
        out_ptr_u32 = (out_ptr + out_offsets).to(
            tl.pointer_type(tl.uint32, 1), bitcast=True
        )
        while not block_stop:
            cur_bits = tl.load(out_ptr_u32, mask=mask, other=0)
            ...
            new_bits = new_val.to(tl.uint32, bitcast=True)
            # Sunrise/PTPU is more stable when product CAS operates on the raw
            # float32 bit pattern instead of a floating-pointer CAS.
            cas_res = tl.atomic_cas(out_ptr_u32, cur_bits, new_bits, sem="acq_rel")
            stop |= (cur_bits == cas_res) | is_nan
