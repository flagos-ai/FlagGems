...
def sorted_quick_unique_flat(sorted_data: torch.Tensor, return_counts: bool):
    ...
    ctas_num = global_ctas_num if global_ctas_num < 32768 else 1024
    tiles_per_cta = triton.cdiv(num_tasks, tile_size * ctas_num)
    num_warps = 8 if tiles_per_cta == 1 else 16
    grid = (ctas_num, 1, 1)
...
def sorted_indices_unique_flat(
    ...
    if global_ctas_num <= 4096:
        ...
        global_ctas_num = triton.cdiv(num_tasks, tile_size)
    next_power_global_ctas_num = triton.next_power_of_2(global_ctas_num)
    ctas_num = global_ctas_num if global_ctas_num < 32768 else 4096
    tiles_per_cta = triton.cdiv(num_tasks, tile_size * ctas_num)
    num_warps = 8 if tiles_per_cta == 1 else 16
    grid = (ctas_num, 1, 1)
...
def _unique2(
    ...
    if in0.numel() <= 8192:
        ...
        sorted_data, sorted_indices = torch.sort(in0.ravel().cpu())
        data_out, inverse_indices, counts = sorted_indices_unique_flat(
            sorted_data.to(in0.device), sorted_indices.to(in0.device), return_counts
        )
    else:
        sorted_data, _ = torch.sort(in0.ravel().cpu())
        data_out, inverse_indices, counts = sorted_quick_unique_flat(
            sorted_data.to(in0.device), return_counts
        )
    return (
        data_out,
