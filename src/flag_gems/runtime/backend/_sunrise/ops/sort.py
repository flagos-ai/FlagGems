...
def compute_global_hist_kernel(
    ...
    dsize = cta_n_start + CTA_TILE_N
    cta_n_end = tl.where(dsize < n, dsize, n)

    arr_partial_ptr = arr_ptr + pid_m * n
    range_tile_r = tl.arange(0, TILE_R)
    range_tile_n = tl.arange(0, TILE_N)
    acc_type = tl.int32
    ...
    for p in range(0, num_passes):  # parallel
        ...
        for r_start in range(0, r, TILE_R):  # parallel
            bin_indices = r_start + range_tile_r
            acc = tl.zeros((TILE_R, TILE_N), dtype=acc_type)
            for n_start in range(cta_n_start, cta_n_end, TILE_N):  # sequantial
                n_offsets = n_start + range_tile_n  # (TILE_N, )
                mask = n_offsets < cta_n_end
                arr = tl.load(arr_partial_ptr + n_offsets, mask=mask)
                arr = convert_to_uint_preverse_order(arr, descending)
                key = (arr >> bit_offset) & bfe_mask  # (TILE_N, )
                matches = tl.where(
                    mask, (bin_indices[:, None] == key).to(acc_type), 0
                )  # (TILE_R, TILE_N)
                acc += matches
            local_sum = tl.sum(acc, axis=1).to(tl.int32)
            tl.atomic_add(
                out_ptr + (pid_m * num_passes + p) * r + bin_indices,
                local_sum,
                sem="relaxed",
            )
...
def sweep(
    ...
    for bin_index in range(cta_r_start, cta_r_end):
        ...
        local_sum = tl.sum(matches.to(dtype=dt), axis=0).to(tl.uint32)
        pack0 = aggregate_mask | local_sum
        status_offset = (pid_m * r + bin_index) * OUT_N + pid_n
        ...
        ex_cumsum_bins = tl.load(
            excumsum_bins_ptr + (pid_m * n_passes + pass_id) * r + bin_index
        )  # scalar
...
def radix_sort(arr, k_bits=8, descending=False):
    ...
    with torch_device_fn.device(arr.device):
        ...
        indices_in = (
            torch.arange(0, n, dtype=torch.int32, device=arr.device)
            .broadcast_to(arr.shape)
            .contiguous()
        )
        arr_out = torch.empty_like(arr)
        indices_out = torch.empty(
            indices_in.shape, device=indices_in.device
        ).as_strided(indices_in.shape, indices_in.stride())

        TILE_R = 2 if n > 2048 else num_bins
        grid_r = triton.cdiv(num_bins, TILE_R)
        TILE_N = 2048
        if n > 32768:
            TILE_N = 2048
        elif m > 2048 and n <= 128:
            TILE_N = 128
        elif m < 32 and n > 8096:
            TILE_N = 256
...
def sort_stable(inp, *, stable, dim=-1, descending=False):
    ...
    if sort_elem_cnt == 1:
        return inp, torch.zeros(inp.shape, dtype=torch.int64, device=inp.device)

    if dim < 0:
        dim = dim + inp.ndim
    if dim != inp.ndim - 1:
        inp = torch.movedim(inp.cpu(), dim, -1).contiguous().to(device=device)
    else:
        inp = inp.contiguous()

    dtype = inp.dtype
    num_bits_per_pass = 1 if dtype == torch.bool else 2
