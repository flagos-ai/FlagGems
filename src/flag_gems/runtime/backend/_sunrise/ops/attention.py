...
from flag_gems.runtime.backend._sunrise.ops.flash_api import mha_fwd, mha_varlan_fwd
from flag_gems.runtime.backend._sunrise.ops.flash_kernel import keep
...
class ScaleDotProductAttention(torch.autograd.Function):
    ...
    def forward(
        ...
        # [sunrise fix] padding for unsupported head dims, since auto lower unsupported.
        head_size = key.shape[-1]
        supported_head_dims = {16, 32, 64, 128, 256}
        if head_size not in supported_head_dims:
            ...
            ), f"Unsupported head dim {head_size}, max supported is {supported_head_dims[-1]}"
            pad = padded_head_dim - head_size
