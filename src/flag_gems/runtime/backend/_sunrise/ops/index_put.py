...
def generate_index_put_kernel(
    ...
    with code.indent():
        ...
        with code.indent():
            code.writeline("tmp_out = tl.load(input_ptr + input_offset, mask=mask)")
            code.writeline("output = tmp_out + cur_value")
            code.writeline("tl.store(input_ptr + input_offset, output, mask=mask)")
        code.writeline("else:")
        with code.indent():
