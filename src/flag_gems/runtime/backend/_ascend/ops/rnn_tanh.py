...
"""Ascend Triton implementation of both ``aten::rnn_tanh`` overloads.
...
_COMPILED_RUNNER_TAKES_CONSTEXPRS = tuple(
    int(part) for part in triton.__version__.split("+", 1)[0].split(".")[:2]
) >= (3, 5)
...
def rnn_tanh_forward_dot_kernel(
    ...
    for step in range(seq_len):
        ...
        # Masked padding makes arbitrary feature sizes safe. Keep reduced-
        # precision inputs in storage dtype while accumulation remains fp32.
        input_linear = tl.dot(x.to(weight_ih.dtype), tl.trans(weight_ih)).to(tl.float32)
...
def rnn_tanh_input_linear_kernel(
    ...
    for k_start in range(0, input_size, BLOCK_K):
        ...
        acc += tl.dot(x.to(weight.dtype), weight)
...
def rnn_tanh_packed_forward_step_dot_kernel(
    ...
    acc = tl.dot(x.to(w_ih.dtype), tl.trans(w_ih)).to(tl.float32)
    acc += tl.dot(hidden.to(w_hh.dtype), tl.trans(w_hh)).to(tl.float32)
    if HAS_BIAS:
        bias = tl.load(
...
def _launch_forward(
    ...
    for layer in range(num_layers):
        ...
        for direction in range(directions):
            ...
            use_ascend_chunked = (
                matrix_shape and input.dtype == torch.bfloat16 and hidden_size == 128
            )
            use_ascend_tiled = matrix_shape and not use_ascend_chunked
            if use_ascend_chunked:
                rows = seq_len * batch_size
                input_linear = _empty(
                    (seq_len, batch_size, hidden_size), input, dtype=torch.float32
                )
                block_m = 16
                ...
                for chunk_start in range(0, seq_len, chunk_size):
                    ...
                    if compiled_chunk is not None and not final_chunk:
                        if _COMPILED_RUNNER_TAKES_CONSTEXPRS:
                            compiled_chunk(
                                hx,
                                weight_hh,
                                input_linear,
    ...
    for layer in range(num_layers - 1, -1, -1):
        ...
        for direction in range(directions):
            ...
            for time_index in time_indices:
                active_batch = batch_sizes_host[time_index]
                row_offset = row_offsets[time_index]
                rnn_tanh_bptt_step_kernel[(active_batch, triton.cdiv(hidden_size, 64))](
                    current_grad,
                    grad_hidden_work,
                    layer_outputs[layer],
                    dpre,
