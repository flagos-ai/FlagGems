...
def _causal_conv1d_fwd_kernel(  # continuous batching
    ...
    if IS_APC_ENABLED:
        ...
        # If the sequence without the sequence_offset_index is stride_cache_chunk-aligned,
        # then the last full chunk is the second-to-last one
        if seq_end_offset == 0:
    ...
    if chunk_offset == 0:
        ...
        if (
            ...
            if load_init_state:
                ...
                # tl.debug_barrier()
                # need this due to the bug in tl.where not enforcing this
                # when data is the result of another tl.load
        ...
        if KERNEL_WIDTH == 5:
            # ruff: noqa: F841
            conv_states_ptrs = prior_tokens  # [BLOCK_N]
            col3 = tl.load(conv_states_ptrs, mask_w, 0.0)  # noqa: F841
            conv_states_ptrs = prior_tokens - 1 * stride_x_token  # [BLOCK_N]
            col2 = tl.load(conv_states_ptrs, mask_w, 0.0)
            conv_states_ptrs = prior_tokens - 2 * stride_x_token  # [BLOCK_N]
            col1 = tl.load(conv_states_ptrs, mask_w, 0.0)
        ...
        # For example:
        # If n_block_to_fill = 0,
        # then only the state at the sequence end is cached and the process below is not involved.
    ...
    for idx_token in range(segment_len):
        ...
        matrix_w = tle.dsa.extract_slice(
            w_tile, offsets=(0 * BLOCK_N,), sizes=(BLOCK_N,), strides=(1,)
        )
        matrix_x = col0
        for j in tl.static_range(KERNEL_WIDTH):
            if KERNEL_WIDTH == 2:
                if j == 1:  # KERNEL_WIDTH-1:
                    matrix_w = tle.dsa.extract_slice(
                        w_tile, offsets=(1 * BLOCK_N,), sizes=(BLOCK_N,), strides=(1,)
                    )
                ...
                if j == 1:
                    ...
                    matrix_w = tle.dsa.extract_slice(
                        w_tile, offsets=(2 * BLOCK_N,), sizes=(BLOCK_N,), strides=(1,)
                    )
                    x_ptrs_1d = x_base_1d + idx_token * stride_x_token  # [BLOCK_N]
                    matrix_x = tl.load(x_ptrs_1d, mask=mask_x_1d)
            elif KERNEL_WIDTH == 4:
                if j == 1:
                    ...
                    matrix_w = tle.dsa.extract_slice(
                        w_tile, offsets=(3 * BLOCK_N,), sizes=(BLOCK_N,), strides=(1,)
                    )
                    x_ptrs_1d = x_base_1d + idx_token * stride_x_token  # [BLOCK_N]
                    matrix_x = tl.load(x_ptrs_1d, mask=mask_x_1d)
...
def causal_conv1d_fn(
    ...
    if not HAS_TLE or tle is None:
        raise RuntimeError("causal_conv1d_fn requires Triton TLE support")
