...
def moe_align_block_size_stage4(
    ...
    for i in range(tokens_per_thread):
        ...
        if offset < numel:
            expert_id = tl.load(topk_ids_ptr + offset)
            tl.atomic_add(tokens_cnts_ptr + off_t + expert_id, 1)
            token_idx_in_expert = tl.load(tokens_cnts_ptr + off_t + expert_id)
            rank_post_pad = token_idx_in_expert + tl.load(cumsum_ptr + expert_id) - 1
