...
@triton.jit
def i0_kernel(x_ptr, out_ptr, n_elements, BLOCK_SIZE: tl.constexpr, M: tl.constexpr):
...
def _launch_i0(out: torch.Tensor, x: torch.Tensor):
    ...
    i0_kernel[(grid,)](x_contig, out_contig, n_elements, BLOCK_SIZE=BLOCK_SIZE, M=M)
...
def i0(x: torch.Tensor):
    logger.debug("GEMS_CAMBRICON I0")
...
def i0_out(x: torch.Tensor, out: torch.Tensor):
    logger.debug("GEMS_CAMBRICON I0_OUT")
    if x.device.type != flag_gems.device or out.device.type != flag_gems.device:
        raise ValueError(
