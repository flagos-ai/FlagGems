...
def _fused_backward_kernel(
    ...
    num_col_tiles = tl.cdiv(W_in, BLOCK_IW)
    pid_row = tile_id // num_col_tiles  # nc * H_in + ih
    pid_col = tile_id % num_col_tiles  # iw tile
...
def _precompute_weights_kernel(
    ...
    for oi in range(pid, output_size, num_jobs):
        center = scale * (oi + 0.5)
        ...
        tl.store(
            weight_ptr + oi.to(tl.int64) * MAX_KSIZE + offs,
            weights,
            mask=offs_mask,
        )
...
def _pass1_w_gather_nchw_kernel(
    ...
    for tile_id in range(pid, total_tiles, num_jobs):
        pid_row = tile_id // num_col_tiles
        pid_col = tile_id % num_col_tiles
...
def _upsample_bicubic2d_aa_backward(
    ...
    logger.debug("GEMS_CAMBRICON UPSAMPLE BICUBIC2D AA BACKWARD")
    ...
    total_tiles = NC * H_in * triton.cdiv(W_in, BLOCK_IW)
    use_fused = (
        total_elems <= _FUSE_THRESHOLD
        and total_tiles <= TOTAL_CORE_NUM
        and h_scale >= 1.0
        and w_scale >= 1.0
    )
    ...
    with torch_device_fn.device(grad_output.device):
        if use_fused:
            ...
            if H_out > 0:
                _precompute_weights_kernel[(min(H_out, TOTAL_CORE_NUM),)](
                    wy,
                    wy_start,
                    H_out,
                    H_in,
            ...
            if W_out > 0:
                _precompute_weights_kernel[(min(W_out, TOTAL_CORE_NUM),)](
                    wx,
                    wx_start,
                    W_out,
                    W_in,
            ...
            total_tiles1 = NC * H_out * triton.cdiv(W_in, BLOCK_IW)
            grid1 = (min(total_tiles1, TOTAL_CORE_NUM),)
            ...
            total_tiles2 = NC * H_in * triton.cdiv(W_in, BLOCK_IW)
            grid2 = (min(total_tiles2, TOTAL_CORE_NUM),)
