...
def prune_var_welford_config(configs, named_args, **kwargs):
    pruned_configs = []
...
@libentry()
@triton.autotune(
    configs=runtime.get_tuned_config("var_mean"),
    prune_configs_by={"early_config_prune": prune_var_welford_config},
    key=["M", "N"],
)
...
def var_welford_kernel(
    ...
    for i in range(0, iter_num):
        pid = (i * num_prog + tle.program_id(0)) * BLOCK_M + tl.arange(0, BLOCK_M)[
            :, None
        ]
        X_ptr = X + pid * N
        Var_ptr = Var + pid
...
@libentry()
@triton.heuristics(runtime.get_heuristic_config("var"))
@triton.jit(do_not_specialize=["correction"])
def var_kernel_2(
    Acc,
    Average,
...
def var(x, dim=None, *, correction=None, keepdim=False):
    logger.debug("GEMS_CAMBRICON VAR")
