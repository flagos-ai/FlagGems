...
@triton.jit
def elu_forward_kernel(x, alpha, scale, input_scale, inplace):
    return tl.where(
        x > 0,
        scale * input_scale * x,
...
def elu(A, alpha=1.0, scale=1.0, input_scale=1.0):
    logger.debug("GEMS_CAMBRICON ELU")
    return elu_forward_kernel(A, alpha, scale, input_scale, False)


def elu_(A, alpha=1.0, scale=1.0, input_scale=1.0):
    logger.debug("GEMS_CAMBRICON ELU_")
    return elu_forward_kernel(A, alpha, scale, input_scale, True, out0=A)
