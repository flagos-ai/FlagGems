...
def div_complex_kernel(ar, ai, br, bi):
    ...
    # Promote to fp32 for intermediate calculations to avoid fp16 precision loss
    ar_f = ar.to(tl.float32)
    ai_f = ai.to(tl.float32)
    br_f = br.to(tl.float32)
    bi_f = bi.to(tl.float32)
    ...
    ratio1 = tl.where(br_f == 0, 0.0, bi_f / br_f)
    denom1 = br_f + bi_f * ratio1
    real1 = (ar_f + ai_f * ratio1) / denom1
    imag1 = (ai_f - ar_f * ratio1) / denom1

    # When |bi| > |br|: ratio = br/bi, denom = bi + br*ratio
    ratio2 = tl.where(bi_f == 0, 0.0, br_f / bi_f)
    denom2 = bi_f + br_f * ratio2
    real2 = (ar_f * ratio2 + ai_f) / denom2
    imag2 = (ai_f * ratio2 - ar_f) / denom2

    real = tl.where(use_br, real1, real2)
    imag = tl.where(use_br, imag1, imag2)

    # Cast back to original dtype
    return real.to(ar.dtype), imag.to(ar.dtype)
