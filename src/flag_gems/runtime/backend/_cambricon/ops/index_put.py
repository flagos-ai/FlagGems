...
def generate_index_put_kernel(
    ...
    with code.indent():
        ...
        args += [
            ...
            "CHECK_LAST_WRITE: tl.constexpr = False,",
            "DUP_BLOCK: tl.constexpr = 1,",
        ]
    ...
    with code.indent():
        ...
        code.writeline("if CHECK_LAST_WRITE:")
        with code.indent():
            code.writeline("dup_offsets = tl.arange(0, DUP_BLOCK)[None, :]")
            code.writeline("dup_valid = (dup_offsets > offset0) & (dup_offsets < M)")
            code.writeline("dup_cur_idx = dup_offsets")
            for j in range(index_rank - 1, -1, -1):
                code.writeline(f"dup_indices_idx{j} = dup_cur_idx % indices0_shape{j}")
                code.writeline(f"dup_cur_idx = dup_cur_idx // indices0_shape{j}")
            code.writeline("has_later = dup_valid")
            for i in range(indices_len):
                comp = [
                    f"dup_indices_idx{j} * indices{i}_stride{j}"
                    for j in range(index_rank)
                ]
                code.writeline(
                    f"dup_index{i} = tl.load("
                    f"indices{i}_ptr + {' + '.join(comp)}, "
                    "mask=dup_offsets < M, other=-1)"
                )
                code.writeline(
                    f"has_later = has_later & (dup_index{i} == cur_index{i})"
                )
            code.writeline(
                "has_later = tl.max(has_later.to(tl.int32), axis=1)[:, None] != 0"
            )
            code.writeline("mask = mask & (~has_later)")
        code.newline()
        ...
        code.newline()
        code.writeline(
            "cur_value = tl.load(values_ptr + values_offset, mask=mask, other=0.0)"
        )
...
def generate_index_put_wrapper(
    ...
    with code.indent():
        ...
        with code.indent():
            ...
            code.writelines(args)
            code.writeline("CHECK_LAST_WRITE=(not accumulate) and M <= 1024,")
            code.writeline("DUP_BLOCK=triton.next_power_of_2(M) if M > 0 else 1,")
        code.writeline(")")
...
def index_put_(inp, indices, values, accumulate=False):
    ...
    if accumulate and inp_view.dtype == torch.float16:
        # Match the CPU reference path used by the accuracy tests, which upcasts.
        accum_inp = inp_view.to(torch.float32)
        _index_put_func(accum_inp, tensors, values.to(torch.float32), accumulate)
        inp_view.copy_(accum_inp)
    else:
...
def _index_put_impl_(inp, indices, values, accumulate=False, unsafe=False):
    logger.debug("GEMS_CAMBRICON _INDEX_PUT_IMPL_")
    ...
    if accumulate and inp.dtype == torch.float16:
        # Match the CPU reference path used by the accuracy tests, which upcasts.
        accum_inp = inp.to(torch.float32)
        _index_put_func(accum_inp, tensor_indices, values.to(torch.float32), accumulate)
        inp.copy_(accum_inp)
    else:
