...
def generate_imports(code: IndentedBuffer) -> IndentedBuffer:
    ...
    code.writeline(f"TOTAL_CORE_NUM = {TOTAL_CORE_NUM}")
...
def generate_index_kernel(
    ...
    with code.indent():
        ...
        code.writeline("total_blocks0 = (M + BLOCK_SIZE0 - 1) // BLOCK_SIZE0")
        code.writeline("total_blocks1 = (N + BLOCK_SIZE1 - 1) // BLOCK_SIZE1")
        code.writeline("total_blocks = total_blocks0 * total_blocks1")
        code.writeline("for flat_pid in range(pid, total_blocks, num_jobs):")
        with code.indent():
            code.writeline("pid0 = flat_pid // total_blocks1")
            code.writeline("pid1 = flat_pid % total_blocks1")
            code.writeline(
            ...
            code.newline()
            code.writeline(
                "cur_value = tl.load(input_ptr + input_offset , mask = mask)"
            )
...
def generate_index_wrapper(
    ...
    with code.indent():
        ...
        with code.indent():
            code.writeline(
                "min(triton.cdiv(M, meta['BLOCK_SIZE0']) * triton.cdiv(N, meta['BLOCK_SIZE1']), TOTAL_CORE_NUM),"
            )
        code.writeline(")")
        code.newline()
        code.writeline("with torch_device_fn.device(input.device):")
        with code.indent():
...
def index(inp, indices):
    logger.debug("GEMS_CAMBRICON INDEX")
