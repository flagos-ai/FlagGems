...
def apply_softcap(S, softcap, is_softcap: tl.constexpr):
    if is_softcap:
        # Use a fp32 inline tanh form on MLU; libdevice tanh shows concurrent softcap jitter.
        x = tl.minimum(tl.maximum(S * softcap, -10.0), 10.0)
        exp_2x = tl.math.exp2(2.0 * x * 1.4426950408889634)
        S = (exp_2x - 1.0) / (exp_2x + 1.0)
...
def flash_fwd_kernel(
    ...
    invalid_rowsum = (rowsum_ == 0) | (rowsum_ != rowsum_)
    lse = tl.where(
        invalid_rowsum,
        float("inf"),
        rowmax_ * scale_softmax + tl.log(rowsum_),
    )
    inv_sum = tl.where(invalid_rowsum, 1.0, 1.0 / rowsum_)
...
def flash_varlen_fwd_kernel(
    ...
    if is_cu_seqlens_k:
        ...
        v_offset = k_bos * v_row_stride
    else:
        k_len_cache = seqlen_k
        k_offset = bid * k_batch_stride
        v_offset = bid * v_batch_stride
    ...
    for step in tl.range(0, n_masking_steps):
        ...
        if is_paged:
            ...
            k_ptr_seq = k_ptr_base + k_offset
            v_ptr_seq = v_ptr_base + v_offset
