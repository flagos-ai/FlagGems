...
def grid_sample_2d_nearest_zeros_kernel(
    ...
    total_blocks = N * C * H_out * W_out
    for pid in range(pid_start, total_blocks, num_jobs):
        nc = pid // (H_out * W_out)
        hw = pid % (H_out * W_out)
...
def grid_sample_3d_nearest_zeros_kernel(
    ...
    pid_start = tl.program_id(0)
    num_jobs = tl.num_programs(0)
    total_blocks = N * C * D_out * H_out * W_out
    for pid in range(pid_start, total_blocks, num_jobs):
...
def grid_sample_3d_nearest_zeros_tiled_kernel(
    ...
    num_spatial_blocks = num_d_blocks * num_h_blocks * num_w_blocks
    total_blocks = N * C * num_spatial_blocks
    for flat_pid in range(pid_start, total_blocks, num_jobs):
        pid_dhw = flat_pid % num_spatial_blocks
        pid_nc = flat_pid // num_spatial_blocks
...
def grid_sample_2d_nearest_zeros_tiled_kernel(
    ...
    total_blocks = N * C * num_spatial_blocks
    for flat_pid in range(pid_start, total_blocks, num_jobs):
        pid_hw = flat_pid % num_spatial_blocks
        pid_nc = flat_pid // num_spatial_blocks
...
def grid_sample(
    ...
    """
    logger.debug("GEMS_CAMBRICON GRID_SAMPLE")

    # Validate inputs
    _validate_grid_sample_input(input, grid, mode, padding_mode)

    if (
        runtime.device.vendor_name == "cambricon"
        and input.dim() == 4
        and mode == "bicubic"
    ):
        padding_mode_enum = {"zeros": 0, "border": 1, "reflection": 2}[padding_mode]
        return torch.ops.aten.grid_sampler_2d(
            input, grid, 2, padding_mode_enum, align_corners
        )
    ...
    if not is_3d:
        ...
        if mode == "nearest":
            ...
            logger.debug(f"GEMS_CAMBRICON GRID_SAMPLE mode '{mode}' not supported")
        ...
        if use_tiled and mode in ["nearest", "bilinear"]:
            ...
            num_h_blocks = (H_out + block_h - 1) // block_h
            num_w_blocks = (W_out + block_w - 1) // block_w
            total_blocks = N * C * num_h_blocks * num_w_blocks
            grid_size = (min(total_blocks, TOTAL_CORE_NUM),)
        else:
    ...
    # Handle 5D inputs (N, C, D_in, H_in, W_in)
        ...
        if mode == "nearest":
            ...
            logger.debug(
                f"GEMS_CAMBRICON GRID_SAMPLE mode '{mode}' not supported for 5D input"
            )
        ...
        if (
            ...
            num_h_blocks = (H_out + block_h - 1) // block_h
            num_w_blocks = (W_out + block_w - 1) // block_w
            total_blocks = N * C * num_d_blocks * num_h_blocks * num_w_blocks
            grid_size = (min(total_blocks, TOTAL_CORE_NUM),)
        else:
