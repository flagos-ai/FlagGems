...
def histc(inp, bins=100, min=0, max=0):
    ...
    """
    logger.debug("GEMS_CAMBRICON HISTC")
    ...
    # Limit launched programs to physical cores; the kernel loops over remaining tiles.
    grid_size = triton.cdiv(n_elements, BLOCK_SIZE)
    if grid_size > TOTAL_CORE_NUM:
        grid_size = TOTAL_CORE_NUM
    grid = (grid_size,)
