...
def _needs_cas_fallback():
    ...
    """
    return flag_gems.vendor_name in ["cambricon", "iluvatar"]
...
def scatter_reduce_sum_2d_kernel(
    ...
    base_offset = pid * BLOCK * LOOP
    grid_stride = num_jobs * BLOCK * LOOP
    arange = tl.arange(0, BLOCK)

    while base_offset < N:
        for i in range(LOOP):
            offsets = (base_offset + i * BLOCK + arange).to(tl.int64)
            mask = offsets < N
...
def scatter_reduce_mean_2d_kernel(
    ...
    while base_offset < N:
        for i in range(LOOP):
            ...
            tl.atomic_add(count_ptr + out_offsets, ones_i, mask=mask, sem="relaxed")
...
@triton.heuristics({"BLOCK": heur_block, "LOOP": heur_loop})
@triton.jit(do_not_specialize=["N"])
def scatter_reduce_count_2d_kernel(
    index_ptr,
    count_ptr,
    N,
    src_ncols,
    ...
    while base_offset < N:
        for i in range(LOOP):
            ...
            ones = tl.full((BLOCK,), 1, dtype=tl.int32)
            tl.atomic_add(count_ptr + out_offsets, ones, mask=mask, sem="relaxed")
        base_offset += grid_stride
...
def scatter_reduce(inp, dim, index, src, reduce, *, include_self=True):
    ...
    """
    logger.debug("GEMS_CAMBRICON SCATTER_REDUCE")
    ...
    if reduce == "mean":
        if include_self:
            count = torch.ones(out.shape, dtype=torch.int32, device=inp.device)
        else:
            count = torch.zeros(out.shape, dtype=torch.int32, device=inp.device)
    ...
    # Cambricon MLU can lose updates when a kernel performs two atomics for the
    # same scatter position. Launch contribution counting/marking separately.
    mask_ptr = dummy_mask
    reduce_use_mask = False
    ...
    with torch_device_fn.device(inp.device):
        if reduce == "sum":
            ...
            if use_2d:
                scatter_reduce_count_2d_kernel[grid](
                    index,
                    count,
                    N,
                    src.shape[1],
                ...
                scatter_reduce_count_kernel[grid](
                    index,
                    count,
                    N,
                    padded_dim,
...
def scatter_reduce_(inp, dim, index, src, reduce, *, include_self=True):
    ...
    logger.debug("GEMS_CAMBRICON SCATTER_REDUCE_")
...
def scatter_reduce_out(inp, dim, index, src, reduce, *, include_self=True, out=None):
    ...
    logger.debug("GEMS_CAMBRICON SCATTER_REDUCE_OUT")
