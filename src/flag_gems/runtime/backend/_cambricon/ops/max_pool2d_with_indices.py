...
def limit_grid(grid_0, grid_1):
    grid_0_ub = MAX_GRID_SIZE_X // 4
    grid_1_ub = MAX_GRID_SIZE_Y
...
def max_pool2d_forward_kernel(
    ...
    out_h,
    out_w,
    # Total number of tasks on axis 0
    task_num_0,
    # Pooling parameters
    ...
    task_num_1 = tl.cdiv(out_h, BLOCK_H) * tl.cdiv(out_w, BLOCK_W)
    grid_0 = tl.num_programs(0)
    grid_1 = tl.num_programs(1)
    pid_nc = tl.program_id(0)
    while pid_nc < task_num_0:
        pid_hw = tl.program_id(1)
        while pid_hw < task_num_1:
...
def max_pool2d_backward_kernel(
    ...
    task_num_1 = tl.cdiv(in_h, BLOCK_IN_H) * tl.cdiv(in_w, BLOCK_IN_W)
    grid_0 = tl.num_programs(0)
    grid_1 = tl.num_programs(1)
    nc_idx = tl.program_id(0)
    while nc_idx < task_num_0:
        pid_hw = tl.program_id(1)
        while pid_hw < task_num_1:
...
def max_pool2d_with_indices(
    ...
    def grid(meta):
        grid_0 = in_n * in_c
        grid_1 = triton.cdiv(out_h, meta["BLOCK_H"]) * triton.cdiv(
            out_w, meta["BLOCK_W"]
        )
        return limit_grid(grid_0, grid_1)

    task_num_0 = in_n * in_c
...
def max_pool2d_backward(
    ...
    def grid(meta):
        grid_0 = in_n * in_c
        grid_1 = triton.cdiv(in_h, meta["BLOCK_IN_H"]) * triton.cdiv(
            in_w, meta["BLOCK_IN_W"]
        )
        return limit_grid(grid_0, grid_1)
