...
@triton.jit
def log1p_kernel_(x_ptr, n_elements, BLOCK_SIZE: tl.constexpr, M: tl.constexpr):
...
def log1p_(*args, **kwargs):
    logger.debug("GEMS_CAMBRICON LOG1P_")
    x = None
    if len(args) > 0:
        x = args[0]
    else:
    ...
    with torch_device_fn.device(x.device):
        log1p_kernel_[(grid,)](x, n_elements, BLOCK_SIZE=1024, M=M)
    return x
