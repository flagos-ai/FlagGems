...
def _soft_margin_loss_elementwise_kernel(
    x_ptr, y_ptr, out_ptr, n_elements, BLOCK_SIZE: tl.constexpr, M: tl.constexpr
):
...
def soft_margin_loss(input: torch.Tensor, target: torch.Tensor, reduction="mean"):
    logger.debug("GEMS_CAMBRICON SOFT_MARGIN_LOSS")
    ...
    if red == 0:
        ...
        _soft_margin_loss_elementwise_kernel[(grid,)](
            input, target, out, n_elements, BLOCK_SIZE=BLOCK_SIZE, M=M
        )
        return out
    else:
        ...
        _soft_margin_loss_sum_kernel[(grid,)](
            input, target, tmp_sum, n_elements, BLOCK_SIZE=BLOCK_SIZE, M=M
        )
        if red == 2:
            # sum
...
def soft_margin_loss_out(
    ...
    logger.debug("GEMS_CAMBRICON SOFT_MARGIN_LOSS_OUT")
    input, target = _check_tensors(input, target)
    red = _normalize_reduction(reduction)
    n_elements = input.numel()
