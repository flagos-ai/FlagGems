...
def generate_index_copy_kernel(
    ...
    with code.indent():
        ...
        with code.indent():
            ...
            code.writeline("num_jobs = tl.num_programs(axis=0)")
            code.writeline("base_offsets = pid * BLOCK_SIZE")
            code.writeline("grid_stride = num_jobs * BLOCK_SIZE")
            code.writeline("while base_offsets < N:")
            with code.indent():
                code.writeline("offsets = base_offsets + tl.arange(0, BLOCK_SIZE)")
...
def index_copy(inp, dim, index, src):
    logger.debug("GEMS_CAMBRICON INDEX_COPY")
...
def index_copy_(inp, dim, index, src):
    logger.debug("GEMS_CAMBRICON INDEX_COPY_")
    assert ((0 <= index) * (index < inp.size(dim))).equal(
        torch.ones(tuple(index.shape), dtype=torch.bool, device=inp.device)
    ), "0 <= index < self.size(dim)"
    assert dim >= -inp.ndim and dim < inp.ndim, "Invalid dim"
