...
def _ctz(x):
    ux = x.to(tl.uint64)
    lowbit = ux & (0 - ux)
    return libdevice.popc(lowbit - 1).to(tl.int32)
...
def gcd_kernel_i16(x_ptr, y_ptr, lut_ptr, out_ptr, n_elements, BLOCK: tl.constexpr):
    ...
    while pid * BLOCK < n_elements:
        offsets = pid * BLOCK + arange
...
def _launch_gcd(lhs, rhs, out):
    ...
    grid = (min(triton.cdiv(numel, block), MAX_GRID_SIZE_X // num_warps),)
...
def gcd(self, other, *, out=None):
    logger.debug("GEMS_CAMBRICON GCD")
...
def gcd_out(self, other, *, out=None):
    logger.debug("GEMS_CAMBRICON GCD_OUT")
    if out is None:
