...
@libentry()
@triton.autotune(
    configs=[triton.Config({"BLOCK_H": 16, "BLOCK_W": 16}, num_stages=4, num_warps=4)],
    ...
    for kd in range(0, kernel_d):
        ...
        for kh in range(0, kernel_h):
            for kw in range(0, kernel_w):
                ...
                safe_d_in = tl.where(d_valid, d_in, 0)
                safe_h_in = tl.where((h_in >= 0) & (h_in < in_h), h_in, 0)
                safe_w_in = tl.where((w_in >= 0) & (w_in < in_w), w_in, 0)
                input_offset = (
...
def avg_pool3d_backward_kernel(
    ...
    # Output-centric backward: each program owns a grad_output tile and scatters
    # its contribution to the covered grad_input window via atomic_add.
    pid_nc = tl.program_id(0)
    pid_dhw = tl.program_id(1)
    ...
    safe_h_out = tl.where(h_out_offsets[:, None] < out_h, h_out_offsets[:, None], 0)
    safe_w_out = tl.where(w_out_offsets[None, :] < out_w, w_out_offsets[None, :], 0)
    grad_out_ptr = (
        grad_output_base
        + d_out * out_stride_d
    ...
    grad_out_val = tl.load(grad_out_ptr, mask=out_mask, other=0.0).to(tl.float32)

    if divisor_override != 0:
        divisor = tl.full((BLOCK_H, BLOCK_W), divisor_override, dtype=tl.float32)
    elif COUNT_INCLUDE_PAD:
        d_start_bwd = d_out * stride_d - padding_d
        d_pc = tl.minimum(d_start_bwd + kernel_d, in_d + padding_d) - tl.maximum(
            d_start_bwd, -padding_d
        )
        d_pc = tl.maximum(d_pc, 0)

        h_start_bwd = h_out_offsets[:, None] * stride_h - padding_h
        h_pc = tl.minimum(h_start_bwd + kernel_h, in_h + padding_h) - tl.maximum(
            h_start_bwd, -padding_h
        )
        h_pc = tl.maximum(h_pc, 0)

        w_start_bwd = w_out_offsets[None, :] * stride_w - padding_w
        w_pc = tl.minimum(w_start_bwd + kernel_w, in_w + padding_w) - tl.maximum(
            w_start_bwd, -padding_w
        )
        w_pc = tl.maximum(w_pc, 0)
        divisor = (d_pc * h_pc * w_pc).to(tl.float32)
    else:
        d_start = d_out * stride_d - padding_d
        d_count = tl.minimum(d_start + kernel_d, in_d) - tl.maximum(d_start, 0)
        d_count = tl.maximum(d_count, 0)

        h_start = h_out_offsets[:, None] * stride_h - padding_h
        h_count = tl.minimum(h_start + kernel_h, in_h) - tl.maximum(h_start, 0)
        h_count = tl.maximum(h_count, 0)

        w_start = w_out_offsets[None, :] * stride_w - padding_w
        w_count = tl.minimum(w_start + kernel_w, in_w) - tl.maximum(w_start, 0)
        w_count = tl.maximum(w_count, 0)
        divisor = (d_count * h_count * w_count).to(tl.float32)

    grad_to_add = grad_out_val / tl.where(divisor == 0, 1.0, divisor)
    ...
    for kd in range(kernel_d):
        ...
        for kh in range(kernel_h):
            h_in = h_out_offsets[:, None] * stride_h - padding_h + kh
            h_valid = (h_in >= 0) & (h_in < in_h)
            safe_h_in = tl.where(h_valid, h_in, 0)
            for kw in range(kernel_w):
                w_in = w_out_offsets[None, :] * stride_w - padding_w + kw
                w_valid = (w_in >= 0) & (w_in < in_w)
                safe_w_in = tl.where(w_valid, w_in, 0)
                in_mask = out_mask & d_valid & h_valid & w_valid
                grad_input_ptrs = (
                    grad_input_base
                    + safe_d_in * in_stride_d
                    + safe_h_in * in_stride_h
                ...
                tl.atomic_add(
                    grad_input_ptrs,
                    grad_to_add.to(grad_input_ptr.type.element_ty),
                    sem="relaxed",
                    mask=in_mask,
                )
...
def avg_pool3d(
    ...
    """
    logger.debug("GEMS_CAMBRICON AVG_POOL3D FORWARD")
...
def avg_pool3d_backward(
    ...
    """
    logger.debug("GEMS_CAMBRICON AVG_POOL3D BACKWARD")
