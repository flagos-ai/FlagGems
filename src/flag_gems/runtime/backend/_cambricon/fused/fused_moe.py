...
def get_embedded_moe_configs():
    ...
    with open(config_path, "r") as f:
        # YAML keys are strings, values are dicts where keys are M and values are configs.
...
def _get_device_name() -> str:
    """Return the normalized Cambricon device name for config lookup."""
    try:
...
def get_moe_configs(
    ...
    for the current Cambricon device. Returns None if no matching config is found.
    """
...
def try_get_optimal_moe_config(
    ...
    # First try to load optimal config from the Cambricon config file.
    E, _, N = w2_shape
...
def _get_config_quant_dtype(
    ...
    """Map Cambricon-supported activation quantization flags."""
    if ocp_mx_scheme is not None:
        raise NotImplementedError(
            f"Cambricon fused MoE does not support OCP MX quantization: {ocp_mx_scheme}"
        )
    if use_fp8_w8a8:
...
def _cambricon_supports_fp8() -> bool:
    try:
        major, _ = torch.mlu.get_device_capability()
    except (AttributeError, RuntimeError):
        return False
    return major >= 6


def _check_cambricon_quant_args(
    use_fp8_w8a8: bool,
    use_int8_w8a8: bool,
    use_int8_w8a16: bool,
    use_int4_w4a16: bool,
    ...
    if sum(bool(x) for x in enabled) > 1:
        raise ValueError("Only one fused MoE quantization mode can be enabled")
    if ocp_mx_scheme is not None:
        raise NotImplementedError(
            f"Cambricon fused MoE does not support OCP MX quantization: {ocp_mx_scheme}"
        )
    if use_fp8_w8a8:
        if not _cambricon_supports_fp8():
            raise NotImplementedError(
                "Cambricon fused MoE fp8_w8a8 requires MLU 6xx or newer"
            )
        assert block_shape is not None and len(block_shape) == 2
        assert w1_scale is not None and w2_scale is not None
        assert w1.dtype in (torch.float8_e4m3fn, torch.float8_e5m2)
        assert w2.dtype in (torch.float8_e4m3fn, torch.float8_e5m2)
    if use_int8_w8a8 or use_int8_w8a16:
        assert w1.dtype == torch.int8 and w2.dtype == torch.int8
        assert w1_scale is not None and w2_scale is not None
    if use_int4_w4a16:
        # Cambricon currently supports the unpacked INT4-in-INT8 path by
        # dequantizing weights before launching the MoE kernels.
        assert w1.dtype == torch.int8 and w2.dtype == torch.int8
        assert w1_scale is not None and w2_scale is not None
...
def get_default_config(
    ...
    Heuristic selection adapted for Cambricon from the vLLM defaults.
...
def _get_config_dtype_str(
    ...
    """Return dtype string for Cambricon kernel config lookup."""
    if use_fp8_w8a8:
        return "fp8_w8a8"
    if use_int8_w8a16:
        return "int8_w8a16"
...
def fused_moe_kernel_gptq_awq(
    ...
    # Map physical programs to virtual C blocks (grouped ordering for L2 reuse).
    num_pid_m = tl.cdiv(EM, BLOCK_SIZE_M)
    num_pid_n = tl.cdiv(N, BLOCK_SIZE_N)
    num_tiles = num_pid_m * num_pid_n
    for pid in range(tl.program_id(axis=0), num_tiles, tl.num_programs(axis=0)):
                ...
                if off_experts == -1:
                    ...
                    if use_fp8_w8a8 or use_int8_w8a8:
                        if group_k > 0 and group_n > 0:  # block-wise
                            ...
                            # Use scalar scale load for hardware broadcast when block size fits
                            # within quantization group.
                            if BLOCK_SIZE_N <= group_n:
                        ...
                        if use_int8_w8a16:
                            ...
                            if group_k > 0 and group_n > 0:
                                ...
                                if SWAP_AB:
                                    ...
                                    # Pre-compute combined scale to reduce arithmetic overhead via
...
def fused_experts_impl(
    ...
    logger.debug("GEMS_CAMBRICON FUSED_MOE")
    _check_cambricon_quant_args(
        use_fp8_w8a8,
        use_int8_w8a8,
        use_int8_w8a16,
        use_int4_w4a16,
