...
def triton_sparse_mla_fwd(
    ...
    for ck in range(NK):
        ...
        if tl.max(mask_ids, axis=0) > 0:
            ...
            exp_qk = tl.math.exp2(
                qk.to(tl.float32) - new_max[:, None].to(tl.float32)
            ).to(tl.float16)
            # exp_qk = tl.math.exp(qk - new_max[:, None]).to(tl.float16)
            sum_qk = tl.sum(exp_qk, axis=1)
            alpha = tl.math.exp2(max_log.to(tl.float32) - new_max.to(tl.float32)).to(
                tl.float16
            )
...
def triton_sparse_mla_fwd_interface(
    ...
    logger.debug("GEMS_CAMBRICON SPARSE_MLA_FWD_INTERFACE")
    is_causal = True
