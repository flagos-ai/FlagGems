...
def fused_recurrent_gated_delta_rule_fwd(
    ...
    # Separate read-only initial states from in-place writes to avoid cross-grid
    # races when continuous batching reuses the same state slot.
    h0 = initial_state.clone() if ssm_state_indices is not None else initial_state
    ...
    if qkv_contiguous:
        ...
        logger.debug(
            "GEMS_CAMBRICON fused_recurrent_gated_delta_rule_fwd, "
