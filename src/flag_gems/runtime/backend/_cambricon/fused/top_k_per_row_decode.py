...
def top_k_per_row_decode(
    ...
    if vocab_size <= _SINGLE_BLOCK_LIMIT // 2:
        ...
        # Large vocab: keep cross-program synchronization within one core wave.
        core_num = torch.mlu.get_device_properties(device).multi_processor_count
        large_block_size = max(
            _LARGE_BLOCK_SIZE,
            triton.next_power_of_2(triton.cdiv(vocab_size, core_num)),
        )
        n_blocks = (vocab_size + large_block_size - 1) // large_block_size
        dev_idx = device.index if device.index is not None else 0
        key = (dev_idx, "large")
        if key not in _cache:
            max_nb = 64
            pb_size = max_nb * 256
            pb_hist = torch.zeros(pb_size, dtype=torch.int32, device=device)
        ...
        pb_hist, buf_val, buf_idx = _cache[key]
        sync = torch.zeros(2, dtype=torch.int32, device=device)
        counter = torch.zeros(2, dtype=torch.int32, device=device)
