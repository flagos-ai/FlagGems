...
def index_put(inp, indices, values, accumulate=False):
    ...
    if has_none:
        if not has_tensor:
            raise ValueError("At least one non-None index tensor is required")
        return _handle_mixed_none_indices(inp, indices, values, accumulate, inplace=False)
...
def index_put_(inp, indices, values, accumulate=False):
    logger.debug("GEMS_METAX INDEX_PUT_")
    ...
    if has_none:
        if not has_tensor:
            raise ValueError("At least one non-None index tensor is required")
        return _handle_mixed_none_indices(inp, indices, values, accumulate, inplace=True)
