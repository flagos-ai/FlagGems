...
def fractional_max_pool2d(
    ...
    Args:
        input: Input tensor of shape (N, C, H, W) or (C, H, W).
        ...
        _random_samples: Optional tensor of shape (N, C, 2) or (C, 2) with values
            in [0, 1).
    """
    logger.debug("GEMS METAX FRACTIONAL_MAX_POOL2D")
    assert input.dim() in (3, 4), f"Expected 3D or 4D input, got {input.dim()}D"

    # Support unbatched (C, H, W) input by temporarily adding a batch dim,
    # matching torch.nn.functional.fractional_max_pool2d semantics.
    is_batched = input.dim() == 4
    if not is_batched:
        input = input.unsqueeze(0)
        if _random_samples is not None and _random_samples.dim() == 2:
...
def fractional_max_pool2d_backward(
    ...
    Scatters gradients back to input positions using the indices from forward.
    """
    logger.debug("GEMS METAX FRACTIONAL_MAX_POOL2D_BACKWARD")
    if isinstance(kernel_size, int):
        kernel_h, kernel_w = kernel_size, kernel_size
    else:
        kernel_h, kernel_w = kernel_size
    ...
    if grad_input.numel() == 0:
        ...
        return grad_input.squeeze(0) if not is_batched else grad_input
