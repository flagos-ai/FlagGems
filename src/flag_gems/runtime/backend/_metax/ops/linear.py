...
def _disable_aabs_for_small_dims(M, N, K):
    """Temporarily disable FlagTree AABS when a dimension is smaller than a tile.

    AABS shrinks a tl.load-driven BLOCK to next_power_of_2(shape), down to 1,
    whenever the shape is smaller than the block. For M/N/K below 16 it lowers
    the tiles feeding tl.dot below the tensor-core minimum, and the MetaX
    backend then fails to compile the kernel (assertion "tn and tk not meet
    conditon" in MACAMmaEncodingAttr::composeSharedLayoutForOperand). Keep the
    configured block sizes for such shapes.
    """
    if min(M, N, K) >= 16 or not hasattr(_autotuning_knobs, "adjust_block_size"):
        yield
        return
...
def linear(input, weight, bias=None):
    ...
    with torch_device_fn.device(input.device), _disable_aabs_for_small_dims(M, N, K):
        linear_kernel[grid](
            input_flat,
            weight,
