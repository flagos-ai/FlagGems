...
def fused_repeat_and_index_select_kernel(
    ...
    inp_row_offset = pid * M
    for m in range(0, tl.cdiv(M, BLOCK_SIZE)):
        cols_offsets = m * BLOCK_SIZE + tl.arange(0, BLOCK_SIZE)
        mask = cols_offsets < M
        cur_inp = tl.load(inp + inp_row_offset + cols_offsets, mask=mask, other=0.0)

        for cur_row_in_pid in range(0, num_of_rows):
            output_row_index = start_row_idx + cur_row_in_pid
            output_row_offsets = output_row_index * M + cols_offsets
            tl.store(out + output_row_offsets, cur_inp, mask=mask)
...
def fused_repeat_and_index_select(inp, repeats, dim):
    ...
    repeats = repeats.cumsum(axis=0)
    index_len = repeats[-1].item()
    ...
    fused_repeat_and_index_select_kernel[grid](
        inp, out, M, N, repeats, BLOCK_SIZE=BLOCK_SIZE
    )
    return out
...
def repeat_interleave_self_tensor(inp, repeats, dim=None, *, output_size=None):
    ...
    if inp.ndim == 2 and dim == 0:
        res = fused_repeat_and_index_select(inp, repeats, dim)
    else:
