...
def special_legendre_polynomial_p_forward(x, n):
    ...
    # We need to handle this as a loop since we don't know n at compile time
    for i in range(2, 256):  # Max 256 to cover most use cases
        # Only compute if n >= i
        mask = n_val >= i
...
def special_legendre_polynomial_p(x: torch.Tensor, n: torch.Tensor) -> torch.Tensor:
    logger.debug("GEMS_VENDOR SPECIAL_LEGENDRE_POLYNOMIAL_P")
