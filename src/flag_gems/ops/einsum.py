...
def _general_einsum_one_operand(
    equation: str,
    A: torch.Tensor
) -> torch.Tensor:
    """
    ...
    for idx in repeated:
        ...
        if len(positions) >= 2:
            ...
            new_indices = [c for i, c in enumerate(current_indices)
                          if i not in [dim1, dim2]] + [idx]
...
def einsum(equation: str, *operands, path: Optional[List[int]] = None):
    ...
    operands = tuple(op.contiguous() if op.is_floating_point() else op for op in operands)
