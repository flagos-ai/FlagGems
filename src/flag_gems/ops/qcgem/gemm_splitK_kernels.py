...
# QC-GEM: GEMM Split-K kernels for FlagGems
...
from .utils import (
    dequantize, init_to_zero, next_power_of_2, is_hip, is_divisible, linear_tile, swizzle_tile,
...
def kernel_config_pruner(configs, nargs, **kwargs):
    ...
    for config in configs:
        ...
        if load_scales_as_block:
            ...
            block_size_k = min(max(block_size_k, 32), g)
    ...
    if not used:
        ...
        block_size_k = min(k, 256) if load_scales_as_block else min(k, g)
        block_size_k = next_power_of_2(block_size_k)
        block_size_k = max(block_size_k, 32)
        while block_size_k > 32 and not is_divisible(k, block_size_k):
            block_size_k //= 2
        block_size_k = max(block_size_k, 32)
...
def gemm_splitK_MX_kernel(
    ...
    for k in tl.range(num_pid_k, num_stages=NUM_STAGES):
        ...
        k_m = (k * SPLIT_K + pid_k) * BLOCK_SIZE_K_S
