...
@libentry()
@triton.jit
def reduce_prod_kernel_result(mid, out, mid_size, BLOCK_MID: tl.constexpr):
    offset = tl.arange(0, BLOCK_MID)
    mid_ptrs = mid + offset
    mask = offset < mid_size
    mid_val = tl.load(mid_ptrs, mask=mask, other=1.0).to(tl.float32)
    reduce_prod_val = tl.reduce(mid_val, axis=0, combine_fn=reduce_mul)
    tl.store(out, reduce_prod_val)


def reduce_prod(inp, *, dtype=None):
    ...
    with torch_device_fn.device(inp.device):
        reduce_prod_kernel_mid[(mid_size, 1, 1)](inp, mid, M, block_size)
        reduce_prod_kernel_result[(1, 1, 1)](mid, out, mid_size, block_mid)
    return out
...
def reduce_prod_dim(inp, dim=None, keepdim=False, *, dtype=None):
    ...
    with torch_device_fn.device(inp.device):
        reduce_prod_kernel[grid](inp, out, M, N)
