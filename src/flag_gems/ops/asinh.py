...
# Operator  : asinh
# Interface : torch.asinh(input) -> Tensor
...
def asinh_fwd_kernel(
    ...
    # Cast to fp32 — tl.log and tl.sqrt require fp32 or fp64
    x_f32   = x.to(tl.float32)
    # asinh(x) = ln(x + sqrt(x^2 + 1))
    # Numerically stable for all real inputs including large values
    out_f32 = tl.log(x_f32 + tl.sqrt(x_f32 * x_f32 + 1.0))
    tl.store(out_ptr + offsets, out_f32.to(x.dtype), mask=mask)


def asinh(input: torch.Tensor) -> torch.Tensor:
    """
    Computes inverse hyperbolic sine element-wise.
    Matches torch.asinh interface exactly.

    Uses FlagGems Triton kernel for large tensors (>4M elements).
    Falls back to PyTorch native for smaller tensors.

    The identity ln(x + sqrt(x^2 + 1)) is numerically stable
    for all real inputs including large values, matching
    torch.asinh behavior.
    ...
    n   = x.numel()
    asinh_fwd_kernel[(triton.cdiv(n, BLOCK_SIZE),)](
