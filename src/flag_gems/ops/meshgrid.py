...
def meshgrid(tensors, indexing='ij'):
    ...
    if ndim == 2:
        ...
        if indexing == 'ij':
            return (x.view(-1, 1).expand(x.size(0), y.size(0)),
                    y.view(1, -1).expand(x.size(0), y.size(0)))
        else:  # xy
            return (x.view(1, -1).expand(y.size(0), x.size(0)),
    ...
    if ndim == 3:
        ...
        if indexing == 'ij':
            return (x.view(-1, 1, 1).expand(x.size(0), y.size(0), z.size(0)),
                    y.view(1, -1, 1).expand(x.size(0), y.size(0), z.size(0)),
                    z.view(1, 1, -1).expand(x.size(0), y.size(0), z.size(0)))
        else:  # xy
            return (x.view(1, -1, 1).expand(y.size(0), x.size(0), z.size(0)),
