...
def conv_transpose1d_forward_kernel(
    ...
    # Pointers setup
    input_base = input_pointer + (input_n_stride * batch_idx)[:, None] + (
        input_c_stride * pid_group * in_channels_per_group
    )
    weight_base = weight_pointer + (
...
def conv_transpose1d(
    ...
    out_width = conv_transpose1d_output_size(
        input_width, kernel_width, stride_width, padding_width, output_padding_width, dilation_width
    )
