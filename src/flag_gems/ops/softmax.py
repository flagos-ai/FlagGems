...
def _forward(x: torch.Tensor, dim: int) -> torch.Tensor:
    ...
    # Handle zero-size last dimension
    if N == 0:
        return x.reshape(orig_shape).to(orig_dtype)

    out = torch.empty_like(x2d)

    # Cap BLOCK_SIZE at 65536 to avoid 32-bit memory overflow on some hardware
    ...
    softmax_kernel[(num_rows,)](
        out, x2d, x2d.stride(0), out.stride(0), N,
...
def _backward(dy: torch.Tensor, y: torch.Tensor, dim: int) -> torch.Tensor:
    ...
    if N == 0:
        return dy.reshape(orig_shape).to(orig_dtype)

    dx = torch.empty_like(dy2d)

    # Cap BLOCK_SIZE at 65536 to avoid 32-bit memory overflow
    BLOCK_SIZE = min(triton.next_power_of_2(N), 65536)
    num_warps = 2 if BLOCK_SIZE <= 256 else 4 if BLOCK_SIZE <= 1024 else 8

    softmax_backward_kernel[(num_rows,)](
        dx, dy2d, y2d, dy2d.stride(0), dx.stride(0), N,
        BLOCK_SIZE=BLOCK_SIZE, num_warps=num_warps,
    )
    dx = dx.view(dy.shape)
    if dim not in (-1, len(orig_shape) - 1):
...
def softmax(x: torch.Tensor, dim: int = -1, half_to_float: bool = False) -> torch.Tensor:
...
def softmax_backward(
    dy: torch.Tensor, y: torch.Tensor, dim: int = -1
) -> torch.Tensor:
