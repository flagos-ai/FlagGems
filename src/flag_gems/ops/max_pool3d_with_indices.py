...
@triton.autotune(
    configs=[
        ...
        triton.Config({"BLOCK_D": 4, "BLOCK_H": 16, "BLOCK_W": 8}, num_stages=3, num_warps=4),
        triton.Config({"BLOCK_D": 4, "BLOCK_H": 8, "BLOCK_W": 16}, num_stages=3, num_warps=4),
        triton.Config({"BLOCK_D": 4, "BLOCK_H": 16, "BLOCK_W": 16}, num_stages=2, num_warps=8),
        triton.Config({"BLOCK_D": 8, "BLOCK_H": 8, "BLOCK_W": 8}, num_stages=3, num_warps=4),
        triton.Config({"BLOCK_D": 8, "BLOCK_H": 16, "BLOCK_W": 8}, num_stages=2, num_warps=4),
        triton.Config({"BLOCK_D": 8, "BLOCK_H": 8, "BLOCK_W": 16}, num_stages=2, num_warps=4),
        triton.Config({"BLOCK_D": 2, "BLOCK_H": 16, "BLOCK_W": 16}, num_stages=4, num_warps=4),
        triton.Config({"BLOCK_D": 2, "BLOCK_H": 32, "BLOCK_W": 16}, num_stages=3, num_warps=8),
        triton.Config({"BLOCK_D": 2, "BLOCK_H": 16, "BLOCK_W": 32}, num_stages=3, num_warps=8),
        triton.Config({"BLOCK_D": 1, "BLOCK_H": 32, "BLOCK_W": 32}, num_stages=3, num_warps=8),
        triton.Config({"BLOCK_D": 1, "BLOCK_H": 16, "BLOCK_W": 32}, num_stages=4, num_warps=4),
    ],
...
def max_pool3d_forward_kernel(
    ...
    hw_idx = pid_dhw % (num_h_blocks * num_w_blocks)
    h_block_idx = hw_idx // num_w_blocks
    w_block_idx = hw_idx % num_w_blocks
    ...
    d_out = d_block_idx * BLOCK_D + tl.arange(0, BLOCK_D)
    h_out = h_block_idx * BLOCK_H + tl.arange(0, BLOCK_H)
    w_out = w_block_idx * BLOCK_W + tl.arange(0, BLOCK_W)

    output_offset = (
        d_out[:, None, None] * out_h * out_w
        + h_out[None, :, None] * out_w
        + w_out[None, None, :]
    )
    out_mask = (
        (d_out[:, None, None] < out_d)
        & (h_out[None, :, None] < out_h)
        & (w_out[None, None, :] < out_w)
    )
    tl.store(out_base_ptr + output_offset, max_val_acc, mask=out_mask)
    tl.store(indices_base_ptr + output_offset, max_idx_acc, mask=out_mask)
...
@triton.autotune(
    configs=[
        ...
        triton.Config({"BLOCK_D": 4, "BLOCK_H": 16, "BLOCK_W": 8}, num_warps=4),
        triton.Config({"BLOCK_D": 4, "BLOCK_H": 8, "BLOCK_W": 16}, num_warps=4),
        triton.Config({"BLOCK_D": 8, "BLOCK_H": 8, "BLOCK_W": 8}, num_warps=4),
        triton.Config({"BLOCK_D": 8, "BLOCK_H": 8, "BLOCK_W": 16}, num_warps=8),
        triton.Config({"BLOCK_D": 2, "BLOCK_H": 16, "BLOCK_W": 16}, num_warps=4),
        triton.Config({"BLOCK_D": 2, "BLOCK_H": 32, "BLOCK_W": 16}, num_warps=8),
        triton.Config({"BLOCK_D": 1, "BLOCK_H": 16, "BLOCK_W": 32}, num_warps=4),
        triton.Config({"BLOCK_D": 1, "BLOCK_H": 32, "BLOCK_W": 16}, num_warps=4),
    ],
...
def max_pool3d_backward_kernel(
    ...
    out_h,
    out_w,
    # Strides for grad_output/indices (per nc)
    out_stride_nc,
    out_stride_d,
    out_stride_h,
    # Pooling parameters
    ...
    num_d_blocks = tl.cdiv(in_d, BLOCK_D)
...
def max_pool3d_with_indices(
    ...
    # Support both 4D (C, D, H, W) and 5D (N, C, D, H, W) input
    if input.dim() == 4:
