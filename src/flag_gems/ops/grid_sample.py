...
def grid_sampler_2d_fwd_kernel(
    x_ptr, grid_ptr, y_ptr,
    stride_x_n, stride_x_c, stride_x_h, stride_x_w,
    stride_grid_n, stride_grid_h, stride_grid_w, stride_grid_c,
    ...
    for c in range(0, c_in, BLOCK_SIZE):
        ...
        # Output: (N, C, H, W)
        base_y_ptr = y_ptr + n_idx * stride_y_n + c_offs * stride_y_c + h_idx * stride_y_h + w_idx * stride_y_w
        ...
        if interpolation_mode == INTERPOLATION_BILINEAR:
            ...
            if padding_mode == PADDING_ZEROS:
                if within_bounds(ix_nw, iy_nw, h_in, w_in):
                    off = iy_nw.to(tl.int32) * stride_x_h + ix_nw.to(tl.int32) * stride_x_w
                    val_nw = tl.load(base_x_ptr + off, mask=mask_c, other=0.0).to(tl.float32)
                if within_bounds(ix_ne, iy_ne, h_in, w_in):
                    off = iy_ne.to(tl.int32) * stride_x_h + ix_ne.to(tl.int32) * stride_x_w
                    val_ne = tl.load(base_x_ptr + off, mask=mask_c, other=0.0).to(tl.float32)
                if within_bounds(ix_sw, iy_sw, h_in, w_in):
                    off = iy_sw.to(tl.int32) * stride_x_h + ix_sw.to(tl.int32) * stride_x_w
                    val_sw = tl.load(base_x_ptr + off, mask=mask_c, other=0.0).to(tl.float32)
                if within_bounds(ix_se, iy_se, h_in, w_in):
                    off = iy_se.to(tl.int32) * stride_x_h + ix_se.to(tl.int32) * stride_x_w
                    val_se = tl.load(base_x_ptr + off, mask=mask_c, other=0.0).to(tl.float32)
            else:
                ...
                off_nw = iy_nw_safe.to(tl.int32) * stride_x_h + ix_nw_safe.to(tl.int32) * stride_x_w
                val_nw = tl.load(base_x_ptr + off_nw, mask=mask_c, other=0.0).to(tl.float32)
                
                off_ne = iy_ne_safe.to(tl.int32) * stride_x_h + ix_ne_safe.to(tl.int32) * stride_x_w
                val_ne = tl.load(base_x_ptr + off_ne, mask=mask_c, other=0.0).to(tl.float32)
                
                off_sw = iy_sw_safe.to(tl.int32) * stride_x_h + ix_sw_safe.to(tl.int32) * stride_x_w
                val_sw = tl.load(base_x_ptr + off_sw, mask=mask_c, other=0.0).to(tl.float32)
                
                off_se = iy_se_safe.to(tl.int32) * stride_x_h + ix_se_safe.to(tl.int32) * stride_x_w
                val_se = tl.load(base_x_ptr + off_se, mask=mask_c, other=0.0).to(tl.float32)
            ...
            if padding_mode == PADDING_ZEROS:
                if within_bounds(ix_nearest, iy_nearest, h_in, w_in):
                    off = iy_nearest.to(tl.int32) * stride_x_h + ix_nearest.to(tl.int32) * stride_x_w
                    out_val = tl.load(base_x_ptr + off, mask=mask_c, other=0.0).to(tl.float32)
            else:
                ...
                off = iy_nearest_safe.to(tl.int32) * stride_x_h + ix_nearest_safe.to(tl.int32) * stride_x_w
                out_val = tl.load(base_x_ptr + off, mask=mask_c, other=0.0).to(tl.float32)
...
def grid_sampler_2d_bwd_kernel(
    grad_output_ptr, input_ptr, grid_ptr,
    grad_input_ptr, grad_grid_ptr,
    stride_go_n, stride_go_c, stride_go_h, stride_go_w,
    stride_in_n, stride_in_c, stride_in_h, stride_in_w,
    stride_grid_n, stride_grid_h, stride_grid_w, stride_grid_c,
    stride_gi_n, stride_gi_c, stride_gi_h, stride_gi_w,
    stride_gg_n, stride_gg_h, stride_gg_w, stride_gg_c,
    n_in, c_in, h_in, w_in,
    h_out, w_out,
    interpolation_mode: tl.constexpr,
    padding_mode: tl.constexpr,
    ...
    for c in range(0, c_in, BLOCK_SIZE):
        ...
        # Grad Output pointer
        base_go_ptr = grad_output_ptr + n_idx * stride_go_n + c_offs * stride_go_c + h_idx * stride_go_h + w_idx * stride_go_w
        ...
        if interpolation_mode == INTERPOLATION_BILINEAR:
            ...
            if output_mask_input:
                ...
                # NW
                if padding_mode != PADDING_ZEROS or within_bounds(ix_nw, iy_nw, h_in, w_in):
                    # For Border/Reflection, coords are clamped valid, so we write to clamped idx
                    off = iy_nw_safe.to(tl.int32) * stride_gi_h + ix_nw_safe.to(tl.int32) * stride_gi_w
                    tl.atomic_add(base_gi_ptr + off, g_out * nw, mask=mask_c)
                
                # NE
                if padding_mode != PADDING_ZEROS or within_bounds(ix_ne, iy_ne, h_in, w_in):
                    off = iy_ne_safe.to(tl.int32) * stride_gi_h + ix_ne_safe.to(tl.int32) * stride_gi_w
                    tl.atomic_add(base_gi_ptr + off, g_out * ne, mask=mask_c)

                # SW
                if padding_mode != PADDING_ZEROS or within_bounds(ix_sw, iy_sw, h_in, w_in):
                    off = iy_sw_safe.to(tl.int32) * stride_gi_h + ix_sw_safe.to(tl.int32) * stride_gi_w
                    tl.atomic_add(base_gi_ptr + off, g_out * sw, mask=mask_c)

                # SE
                if padding_mode != PADDING_ZEROS or within_bounds(ix_se, iy_se, h_in, w_in):
                    off = iy_se_safe.to(tl.int32) * stride_gi_h + ix_se_safe.to(tl.int32) * stride_gi_w
            ...
            if output_mask_grid:
                ...
                if padding_mode == PADDING_ZEROS:
                    if within_bounds(ix_nw, iy_nw, h_in, w_in):
                        val_nw = tl.load(base_in_ptr + iy_nw.to(tl.int32)*stride_in_h + ix_nw.to(tl.int32)*stride_in_w, mask=mask_c, other=0.0).to(tl.float32)
                    if within_bounds(ix_ne, iy_ne, h_in, w_in):
                        val_ne = tl.load(base_in_ptr + iy_ne.to(tl.int32)*stride_in_h + ix_ne.to(tl.int32)*stride_in_w, mask=mask_c, other=0.0).to(tl.float32)
                    if within_bounds(ix_sw, iy_sw, h_in, w_in):
                        val_sw = tl.load(base_in_ptr + iy_sw.to(tl.int32)*stride_in_h + ix_sw.to(tl.int32)*stride_in_w, mask=mask_c, other=0.0).to(tl.float32)
                    if within_bounds(ix_se, iy_se, h_in, w_in):
                        val_se = tl.load(base_in_ptr + iy_se.to(tl.int32)*stride_in_h + ix_se.to(tl.int32)*stride_in_w, mask=mask_c, other=0.0).to(tl.float32)
                else:
                    val_nw = tl.load(base_in_ptr + iy_nw_safe.to(tl.int32)*stride_in_h + ix_nw_safe.to(tl.int32)*stride_in_w, mask=mask_c, other=0.0).to(tl.float32)
                    val_ne = tl.load(base_in_ptr + iy_ne_safe.to(tl.int32)*stride_in_h + ix_ne_safe.to(tl.int32)*stride_in_w, mask=mask_c, other=0.0).to(tl.float32)
                    val_sw = tl.load(base_in_ptr + iy_sw_safe.to(tl.int32)*stride_in_h + ix_sw_safe.to(tl.int32)*stride_in_w, mask=mask_c, other=0.0).to(tl.float32)
                    val_se = tl.load(base_in_ptr + iy_se_safe.to(tl.int32)*stride_in_h + ix_se_safe.to(tl.int32)*stride_in_w, mask=mask_c, other=0.0).to(tl.float32)
            ...
            if output_mask_input:
                ...
                if valid:
                    ...
                    ix_nearest_safe = safe_clamp(ix_nearest, 0.0, w_in - 1.0)
                    iy_nearest_safe = safe_clamp(iy_nearest, 0.0, h_in - 1.0)
                    off = iy_nearest_safe.to(tl.int32) * stride_gi_h + ix_nearest_safe.to(tl.int32) * stride_gi_w
    ...
    if output_mask_grid:
        base_gg_ptr = grad_grid_ptr + n_idx * stride_gg_n + h_idx * stride_gg_h + w_idx * stride_gg_w
        # Store x
...
def grid_sampler_2d(input, grid, interpolation_mode=0, padding_mode=0, align_corners=False):
    """
    Args:
        input: (N, C, H_in, W_in)
        grid: (N, H_out, W_out, 2)
        ...
        align_corners: bool
    """
    return GridSampler2d.apply(input, grid, interpolation_mode, padding_mode, align_corners)
