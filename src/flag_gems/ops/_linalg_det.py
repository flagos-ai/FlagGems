...
def _det_kernel(
    ...
    writes back the in-place LU factorization. The determinant is the signed
    product of the pivots: sign(from row swaps) * prod(diagonal of U).
    """
...
def _linalg_det(A):
    ...
    # to recover the determinant, so a zero-initialized buffer is returned.
    ...
    stride_det = result.stride(-1) if result.dim() > 0 else 0
