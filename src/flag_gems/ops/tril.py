...
def tril(A, diagonal=0):
    ...
    # When diagonal is large enough, all elements are in the lower triangle
    if diagonal >= N - 1:
        return A_input.clone()

    # Effective column bound: beyond column M+diagonal-1, all elements are upper triangle
    ...
    # Use copy-lower path only when the tensor is large enough for the column
    # reduction to outweigh the torch.zeros overhead (~5us kernel launch cost).
    total_bytes = A_input.numel() * A_input.element_size()
    if N_eff < N and total_bytes >= 1024 * 1024:
        # Right columns are entirely upper triangle — pre-zero output, copy lower only
...
def tril_(A, diagonal=0):
    ...
    # For inplace, only rows where some elements are above the diagonal need zeroing
    # Row i has upper-triangle elements iff i + diagonal < N - 1
    ...
    if not can_use_directly:
        ...
        with torch_device_fn.device(A.device):
            # Zero-upper kernel (write-only, no loads) is faster for fp32+ and batch
            # tensors. For fp16/bf16 non-batch, the read+write pattern benefits from
            # L2 cache priming, making the full kernel faster.
