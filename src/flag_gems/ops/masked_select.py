...
def masked_select(inp, mask):
    ...
    prefix_sum_ = mask_flattened.cumsum(axis=0)
    reff = prefix_sum_.to("cpu")
    prefix_sum = reff[-1].item()
    out = torch.empty(prefix_sum,dtype = inp.dtype,device = inp.device)

    n_elements = inp.numel()
    grid = lambda meta: (triton.cdiv(n_elements, meta["BLOCK_SIZE"]),)
    with torch_device_fn.device(inp.device):
        masked_select_kernel[grid](inp, mask_flattened, prefix_sum_, out, n_elements)
    return out
