...
"""Private transposed-2d convolution implementation used by the ``convolution``
dispatcher.

This is a self-contained copy owned by the ``convolution`` operator. It exists
so ``convolution`` can route the transposed-2d case without depending on, or
modifying, the shared ``conv_transpose2d`` operator. The residue path here
performs the ``residue`` split on the host so every kernel launch receives a
compile-time-constant residue, which the shared operator's dynamic kernel does
not do.
...
def conv_transpose2d_impl(
    ...
    logger.debug("GEMS CONVOLUTION CONV_TRANSPOSE2D")
