...
def reduce_variance(input, dim=None, *, correction=None, keepdim=False, unbiased=True):
    logger.debug("GEMS REDUCE VARIANCE")
    ...
    if dim is None or len(dim) == input.ndim:
        ...
        shape = [1] * input.ndim
        N = input.numel()
        var = torch.empty(shape, dtype=input.dtype, device=input.device)
        BLOCK_N = 1024
        BLOCK_NUM = triton.cdiv(N, BLOCK_N)
        acc = torch.empty([BLOCK_NUM], dtype=input.dtype, device=input.device)
        average = torch.empty([BLOCK_NUM], dtype=input.dtype, device=input.device)
        count = torch.empty([BLOCK_NUM], dtype=input.dtype, device=input.device)

        with torch_device_fn.device(input.device):
            variance_kernel_1[(BLOCK_NUM,)](input, acc, average, count, N, BLOCK_N=BLOCK_N)
            variance_kernel_2[(1,)](
                acc, average, count, var, N, correction, BLOCK_NUM
            )
    else:
        ...
        with torch_device_fn.device(input.device):
            variance_welford_kernel[grid](input, var, M, N, correction)
