...
def _fused_dbdsqr_kernel(
    ...
    Compute dtype is inferred from the D buffer (fp64 on the generic path;
    no-fp64 vendors call this with fp32 buffers and their own EPS)."""
    pid = tl.program_id(0)
    idx = tl.arange(0, BLOCK_K)
    dmask = idx < K
    emask = idx < K - 1
...
# ===========================================================================
# Two-sided symmetric Jacobi kernels (Gram-Jacobi SVD path).
...
def _svdvals_hybrid(input):
    """Hybrid SVD on the generic path (CUDA and other fp64-capable backends
    without a vendor override): QR + parallel Jacobi + DBDSQR.

    Dispatch on this path (all computation in fp64):

      k > 10       QR (fp64) + Jacobi (60/50 sweeps).
      k ∈ (3, 10]  bidiagonalisation + fused DBDSQR.
      Jacobi → DBDSQR fallback on non-convergence.

    The per-vendor backend files (thead, hygon, iluvatar, metax, ascend)
    implement their own dispatch and fp64 policy.
    """
    ...
    # fp64 throughout on the generic path; per-vendor backends handle their
    # own fp64 policy.
    M_qr, N_qr = (m, n) if tall else (n, m)
    block_m = triton.next_power_of_2(min(M_qr, 256))
    block_n = 32
    ...
    # ---- 2. Jacobi SVD on R ------------------------------------------------
    # Jacobi for k > 10.  TSQR matrices always retain Jacobi: DBDSQR doesn't
    # drown out the ULPs-level differences from the two-level QR partitioning,
    # so final singular values vary non-deterministically across runs on
    # SM 8.0 GPUs.  Jacobi's 60 sweeps drive the off-diagonal deep below the
    # convergence floor, producing identical results regardless of GPU arch.
    use_jacobi = k > 10 or _tsqr_applied

    if use_jacobi:
        # Rf is always fp64 here: qr_use_fp64 is k >= 4 and Jacobi only runs
        # for k > 10 (or after TSQR, which also requires qr_use_fp64).
    ...
    work_dtype = torch.float64  # fp64 throughout on the generic path
