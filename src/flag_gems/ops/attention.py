...
def scaled_dot_product_flash_attention(
    ...
    *,
    ...
    """Wrapper for aten::_scaled_dot_product_flash_attention.default.

    ATen dispatches _scaled_dot_product_flash_attention with tensors in
    (batch, num_heads, seqlen, head_dim) = (B, H, S, D) layout, matching the
    standard PyTorch SDPA convention.  flash_attention_forward (and the
    underlying mha_fwd kernel) expects (B, S, H, D) seqlen-first layout, so
    we transpose in and out.

    Returns:
        9-tuple matching the ATen schema:
        (output, logsumexp, cum_seq_q, cum_seq_k,
         max_q, max_k, rng_state, unused, debug_attn_mask)
        cum_seq_q/k are empty tensors for the non-varlen (dense-batch) case.
    """
    logger.debug("GEMS _SCALED_DOT_PRODUCT_FLASH_ATTENTION")
    # query/key/value arrive as (B, H, S, D); seqlen is at dim 2.
    seqlen_q = query.shape[2]
    seqlen_k = key.shape[2]

    # Transpose to (B, S, H, D) required by flash_attention_forward / mha_fwd.
    q = query.transpose(1, 2)
    k = key.transpose(1, 2)
    v = value.transpose(1, 2)

    out, lse, philox_seed, philox_offset, p = flash_attention_forward(
        q,
        k,
        v,
    ...
    # out is (B, S, H, D); transpose back to (B, H, S, D) for ATen.
    out = out.transpose(1, 2)
    # Empty cum_seq tensors represent the non-varlen (dense-batch) case.
    empty_seq = torch.empty((0,), dtype=torch.int32, device=query.device)
    return (
        out,  # output
        lse,  # logsumexp
        empty_seq,  # cum_seq_q
        empty_seq,  # cum_seq_k
        seqlen_q,  # max_q (SymInt)
        seqlen_k,  # max_k (SymInt)
        philox_seed,  # rng_state
        philox_offset,  # unused
        p,  # debug_attn_mask
    )
