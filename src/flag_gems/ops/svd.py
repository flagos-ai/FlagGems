...
# Symmetric Jacobi is correct for Gram matrices, but the rotations are
# sequential updates to shared matrix state. Keep it single-warp; multi-warp
# execution can reorder those updates and destroys eigenvector orthogonality.
_SYMEIGH_NUM_WARPS = 1
_SUPPORTED_SVD_DTYPES = (torch.float16, torch.bfloat16, torch.float32)


@libentry()
@triton.jit
def sym_eigh_jacobi_kernel(
    gram,
    g_work,
    evals,
    eigvecs,
    ...
    gram_base = gram + pid * N * N
    gw_base = g_work + pid * gw_batch_stride
    ev_base = eigvecs + pid * ev_batch_stride

    g_block = tl.load(
        gram_base + rows[:, None] * N + cols[None, :],
        mask=(rows[:, None] < N) & (cols[None, :] < N),
        other=0.0,
    ).to(tl.float32)
    tl.store(
        gw_base + rows[:, None] * N + cols[None, :],
        g_block,
        mask=(rows[:, None] < N) & (cols[None, :] < N),
    )
    ...
    for _ in range(NUM_SWEEPS):
        for p in range(N):
            for q in range(p + 1, N):
                app = tl.load(gw_base + p * N + p)
                aqq = tl.load(gw_base + q * N + q)
                apq = tl.load(gw_base + p * N + q)
                ...
                row_p = tl.load(gw_base + p * N + idx, mask=mask, other=0.0)
                row_q = tl.load(gw_base + q * N + idx, mask=mask, other=0.0)
                col_p = tl.load(gw_base + idx * N + p, mask=mask, other=0.0)
                col_q = tl.load(gw_base + idx * N + q, mask=mask, other=0.0)
                ...
                new_app = c * c * app - 2.0 * c * sn * apq + sn * sn * aqq
                new_aqq = sn * sn * app + 2.0 * c * sn * apq + c * c * aqq
                tl.store(gw_base + p * N + p, new_app)
                tl.store(gw_base + q * N + q, new_aqq)
                tl.store(gw_base + p * N + q, 0.0)
                tl.store(gw_base + q * N + p, 0.0)

                v_p = tl.load(ev_base + idx * N + p, mask=mask, other=0.0)
                v_q = tl.load(ev_base + idx * N + q, mask=mask, other=0.0)
                tl.store(
                    ev_base + idx * N + p,
                    c * v_p - sn * v_q,
                    mask=mask,
    ...
    diag_vals = tl.load(gw_base + idx * N + idx, mask=mask, other=0.0)
    tl.store(evals + pid * N + idx, diag_vals, mask=mask)
...
def _sym_eigh_jacobi(gram, num_sweeps):
    """Compute eigenpairs of a real symmetric matrix with Triton Jacobi sweeps.

    This helper is intentionally not the default SVD large-k path yet. It is
    useful as a pure-Triton correctness path, but it is much slower than
    cuSOLVER's symmetric eigensolver once n reaches a few hundred.
    """
    n = gram.shape[-1]
    batch = gram.numel() // (n * n)
    work = torch.empty_like(gram)
    evals = torch.empty(gram.shape[:-1], dtype=gram.dtype, device=gram.device)
    eigvecs = torch.empty_like(gram)
    sym_eigh_jacobi_kernel[(batch,)](
        gram,
        work,
        evals,
        eigvecs,
        batch,
        work.reshape(batch, n, n).stride(0),
        eigvecs.reshape(batch, n, n).stride(0),
        N=n,
        BLOCK_N=triton.next_power_of_2(n),
        NUM_SWEEPS=num_sweeps,
        num_warps=_SYMEIGH_NUM_WARPS,
    )
    order = torch.argsort(evals, dim=-1, descending=True)
    evals = torch.gather(evals, -1, order)
