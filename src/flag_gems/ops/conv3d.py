...
def conv3d(input, weight, bias=None, stride=1, padding=0, dilation=1, groups=1):
    ...
    if padding == "same":
        ...
        padding_depth = math.ceil(
            (stride_depth * (id - 1) + 1 + dilation_depth * (kernel_size_d - 1) - id)
            / 2
        )
        padding_height = math.ceil(
            (stride_height * (ih - 1) + 1 + dilation_height * (kernel_size_h - 1) - ih)
            / 2
        )
        padding_width = math.ceil(
            (stride_width * (iw - 1) + 1 + dilation_width * (kernel_size_w - 1) - iw)
            / 2
        )
        od = int(
