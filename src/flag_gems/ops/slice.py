...
def _as_strided_slice(input_tensor, output_shape, dim, start, step):
    """Build the sliced view without redispatching aten::slice.Tensor."""
    output_strides = list(input_tensor.stride())
...
def slice(input_tensor, dim=0, start=None, end=None, step=1):
    ...
    if input_tensor.is_contiguous() and step == 1:
        # Avoid tensor indexing here: when this implementation is registered for
        # aten::slice.Tensor, indexing would dispatch straight back into slice.
        view = _as_strided_slice(input_tensor, output_shape, dim, start, step)
        output.copy_(view)
        return output
    ...
    if ndim == 2:
        ...
        # using an as_strided view (correctness first for rare high-rank cases).
        view = _as_strided_slice(input_tensor, output_shape, dim, start, step)
        output.copy_(view)
