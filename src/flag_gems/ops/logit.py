...
@triton.jit
def logit_backward_kernel(dy, x, eps):
    x_f32 = x.to(tl.float32)
    dy_f32 = dy.to(tl.float32)
    denom = x_f32 * (1.0 - x_f32)
    # eps < 0 means no clamping (eps=None case)
    has_eps = eps >= 0.0
    in_range = (x_f32 > eps) & (x_f32 < 1.0 - eps)
    raw_grad = dy_f32 / denom
    clamped_grad = tl.where(in_range, raw_grad, 0.0)
    return tl.where(has_eps, clamped_grad, raw_grad)


def logit_backward(grad_output, self, eps=None):
    logger.debug("GEMS LOGIT BACKWARD")
    eps_val = float(eps) if eps is not None else -1.0
    return logit_backward_kernel(grad_output, self, eps_val)
