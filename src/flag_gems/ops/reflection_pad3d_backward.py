...
def reflection_pad3d_backward_kernel(
    ...
    h_idx = hw_idx // W_out
    w_idx = hw_idx % W_out
    mask_out = (offs_n < D_out * H_out * W_out) & (pid_b < B)
    ...
    # Compute reflected index for depth
    ...
    grad_input_offset = base_in + id_ * (H_in * W_in) + ih * W_in + iw
...
def _launch_reflection_pad3d_backward(
    ...
    if (
        ...
        if D_out != D_in or H_out != H_in or W_out != W_in:
            raise RuntimeError(
                "Internal error: D_out/H_out/W_out should equal D_in/H_in/W_in when no padding"
            )
        BLOCK_DHW = 256
        grid = (B, triton.cdiv(D_in * H_in * W_in, BLOCK_DHW))
        with torch_device_fn.device(x.device):
            _copy_tensor_kernel_f32[grid](
                grad_output, grad_input, B, D_in, H_in, W_in, BLOCK_DHW=BLOCK_DHW
            )
        if grad_input.dtype == x.dtype:
            return grad_input
        result = torch.empty_like(x)
        with torch_device_fn.device(x.device):
            _copy_tensor_kernel[grid](
                grad_input, result, B, D_in, H_in, W_in, BLOCK_DHW=BLOCK_DHW
            )
        return result
    ...
    if grad_input.dtype == x.dtype:
        return grad_input
    result = torch.empty_like(x)
    cast_grid = (B, triton.cdiv(D_in * H_in * W_in, BLOCK_DHW))
    with torch_device_fn.device(x.device):
        _copy_tensor_kernel[cast_grid](
            grad_input, result, B, D_in, H_in, W_in, BLOCK_DHW=BLOCK_DHW
        )
    return result
...
@triton.jit
def _copy_tensor_kernel_f32(in_ptr, out_ptr, B, D, H, W, BLOCK_DHW: tl.constexpr):
    pid_b = tl.program_id(axis=0)
    pid_n = tl.program_id(axis=1)
...
@triton.jit
def _copy_tensor_kernel(in_ptr, out_ptr, B, D, H, W, BLOCK_DHW: tl.constexpr):
    pid_b = tl.program_id(axis=0)
    pid_n = tl.program_id(axis=1)
...
def reflection_pad3d_backward(grad_output: torch.Tensor, input: torch.Tensor, padding):
    logger.debug("GEMS REFLECTION_PAD3D_BACKWARD")
    return _launch_reflection_pad3d_backward(grad_output, input, padding)
