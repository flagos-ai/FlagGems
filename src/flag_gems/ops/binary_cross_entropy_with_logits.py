...
def binary_cross_entropy_with_logits_pos_weight_kernel(x, y, pos_weight):
    ...
    #   x<0: -pos_weight * log(exp(x)/(1+exp(x)))
    #        = -pos_weight * (x - log(1+exp(x)))
    #        = -pos_weight*x + pos_weight*log(1+exp(x))
...
def binary_cross_entropy_with_logits(
    ...
    # BCE with logits is numerically sensitive; float16/bf16 may have
    # reduced precision compared to the float64 reference in tests.
    assert self.dtype in (
        torch.float16,
        torch.float32,
        torch.bfloat16,
        torch.float64,
    ), f"binary_cross_entropy_with_logits only supports floating dtypes, got {self.dtype}"
