...
def linalg_eig_kernel_2x2(
    ...
    """
    Compute eigenvalues and eigenvectors for 2x2 matrices using the characteristic equation.
    For a 2x2 matrix [[a, b], [c, d]]:
    eigenvalues = (a + d ± sqrt((a+d)^2 - 4(ad-bc))) / 2
    """
    ...
    sqrt_disc = tl_extra_shim.sqrt(tl.abs(discriminant))

    # Eigenvalues: real and imaginary parts
    # When discriminant >= 0: lambda = (trace +/- sqrt(disc)) / 2 (real eigenvalues)
    # When discriminant < 0: lambda = trace/2 +/- i*sqrt(-disc)/2 (complex conjugate pair)
    lambda1_real = tl.where(discriminant >= 0, (trace + sqrt_disc) / 2.0, trace / 2.0)
    lambda2_real = tl.where(discriminant >= 0, (trace - sqrt_disc) / 2.0, trace / 2.0)
    lambda1_imag = tl.where(
        discriminant < 0, tl_extra_shim.sqrt(-discriminant) / 2.0, 0.0
    )
    lambda2_imag = tl.where(
        discriminant < 0, -tl_extra_shim.sqrt(-discriminant) / 2.0, 0.0
    )
    ...
    # Normalize
    norm1 = tl_extra_shim.sqrt(v1_1 * v1_1 + v1_2 * v1_2 + 1e-8)
    v1_1 = v1_1 / norm1
    v1_2 = v1_2 / norm1
    ...
    # Normalize
    norm2 = tl_extra_shim.sqrt(v2_1 * v2_1 + v2_2 * v2_2 + 1e-8)
    v2_1 = v2_1 / norm2
    v2_2 = v2_2 / norm2
...
def linalg_eig(A: torch.Tensor):
    ...
    # Check input dtype - linalg_eig only supports float32 or complex types
    # Half/BFloat16 lack sufficient precision for eigenvalue decomposition
    if A.dtype not in [torch.float32, torch.float64, torch.complex64, torch.complex128]:
        raise ValueError(
            f"linalg_eig: Unsupported dtype {A.dtype}. "
            f"Expected float32, float64, complex64, or complex128."
        )
