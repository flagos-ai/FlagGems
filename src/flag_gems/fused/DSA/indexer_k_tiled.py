...
def triton_lighting_indexer_k_tiled(
    ...
    mask_cu = offs_cu < CU
    bos_vec, eos_vec = tl.load(cu_bg_seqlens + offs_cu, mask_cu, 1000000000) + i_k*TK, tl.load(cu_ed_seqlens + offs_cu, mask_cu, -1000000000) # [BQ]
    ...
    if CK > 0:
        ...
        for ck in range(CK, warp_specialize=True):
            ...
            out_ptr = o_base + offs_boq[:, None]*stride_lm + offs_bk[None, :]*stride_ln
            out_msk = mask_boq[:, None] & mask_bk[None, :] & (bos_vec[:, None] <= offs_bk[None, :]+bos) & (eos_vec[:, None] > offs_bk[None, :]+bos)
            # out_blk = tl.where((bos_vec[:, None] <= offs_bk[None, :]+bos) & (eos_vec[:, None] > offs_bk[None, :]+bos), out_blk, float('-inf'))
            tl.store(out_ptr, out_blk.to(tl.float16), out_msk)


def triton_lighting_indexer_k_tiled_interface(q, kv, weights, cu_seqlen_ks, cu_seqlen_ke) :
