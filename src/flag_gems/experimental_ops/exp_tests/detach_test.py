...
def test_detach_tensor(shape, dtype, requires_grad):
    input_tensor = torch.randn(shape, dtype=dtype, device=flag_gems.device, requires_grad=requires_grad)
...
def test_detach_benchmark_tensor(shape, dtype, requires_grad):
    ...
    ms_torch, _, _ = triton.testing.do_bench(
        lambda: torch.ops.aten.detach(ref_input),
        rep=100,
        quantiles=quantiles
    )
    ...
    with flag_gems.use_gems():
        ms_triton, _, _ = triton.testing.do_bench(
            lambda: gems_detach(input_tensor),
            rep=100,
            quantiles=quantiles
        )
