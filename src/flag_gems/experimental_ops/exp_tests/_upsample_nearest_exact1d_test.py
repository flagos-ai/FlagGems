...
import flag_gems
from flag_gems.experimental_ops._upsample_nearest_exact1d import _upsample_nearest_exact1d as gems__upsample_nearest_exact1d, _upsample_nearest_exact1d_out as gems__upsample_nearest_exact1d_out
...
def test__upsample_nearest_exact1d_out(shape, dtype, factor):
    ...
    ref_out = torch.ops.aten._upsample_nearest_exact1d.out(ref_x, [out_len], None, out=ref_out_buf)
...
def test__upsample_nearest_exact1d_vec(shape, dtype, factor, mode):
    ...
    ref_out = torch.ops.aten._upsample_nearest_exact1d.vec(ref_x, output_size, scale_factors)

    with flag_gems.use_gems():
        act_out = torch.ops.aten._upsample_nearest_exact1d.vec(x, output_size, scale_factors)
...
def test__upsample_nearest_exact1d_benchmark_out(shape, dtype, factor):
    ...
    # PyTorch reference implementation
    ms_torch, _, _ = triton.testing.do_bench(
        lambda: torch.ops.aten._upsample_nearest_exact1d.out(ref_x, [out_len], None, out=ref_out_buf),
        rep=100,
        quantiles=quantiles
    )
...
def test__upsample_nearest_exact1d_vec_performance(shape, dtype, factor, mode):
    ...
    # PyTorch reference implementation
    ms_torch, _, _ = triton.testing.do_bench(
        lambda: torch.ops.aten._upsample_nearest_exact1d.vec(ref_x, output_size, scale_factors),
        rep=100,
        quantiles=quantiles
    )
    ...
    with flag_gems.use_gems():
        ms_triton, _, _ = triton.testing.do_bench(
            lambda: torch.ops.aten._upsample_nearest_exact1d.vec(x, output_size, scale_factors),
            rep=100,
            quantiles=quantiles
        )
