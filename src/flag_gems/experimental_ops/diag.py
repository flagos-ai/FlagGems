...
@triton.jit
def _diag_extract_kernel(a_ptr, out_ptr,
                         i0, j0, L,
...
@triton.jit
def _diag_write_kernel(v_ptr, out_ptr,
                       i0, j0, N,
                       stride_row, stride_col,
                       BLOCK_SIZE: tl.constexpr):
    pid = tl.program_id(0)
...
def diag(*args, **kwargs):
    ...
    if input.dim() == 1:
        ...
        if N > 0:
            ...
            _diag_write_kernel[grid](input, out, i0, j0, N,
                                     stride_row, stride_col,
                                     BLOCK_SIZE=1024)
        return out
    elif input.dim() == 2:
        ...
        if L > 0:
            ...
            _diag_extract_kernel[grid](input, out, i0, j0, L,
                                       stride_row, stride_col,
                                       BLOCK_SIZE=1024)
        return out
    else:
...
def diag_out(*args, **kwargs):
    ...
    if input is None and len(args) >= 1 and isinstance(args[0], torch.Tensor):
        ...
        if out is None and len(args) >= 3 and isinstance(args[2], torch.Tensor) and isinstance(args[1], int):
            input = args[0]
            diagonal = int(args[1])
            out = args[2]
        elif out is None and len(args) >= 3 and isinstance(args[0], torch.Tensor) and isinstance(args[1], torch.Tensor) and isinstance(args[2], int):
            out = args[0]
            input = args[1]
            diagonal = int(args[2])
        else:
    ...
    if input.dim() == 1:
        ...
        if out.dim() != 2 or out.shape[0] != size or out.shape[1] != size:
            raise RuntimeError(f"diag_out: expected out shape ({size}, {size}), got {tuple(out.shape)}")
        ...
        if out.dim() != 1 or out.numel() != L:
            raise RuntimeError(f"diag_out: expected out shape ({L},), got {tuple(out.shape)}")
        if out.dtype != input.dtype or out.device != input.device:
            raise RuntimeError("diag_out: out dtype/device must match input")
