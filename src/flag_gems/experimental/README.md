...
# Testing
## Accuracy Tests
Add accuracy test in `tests/test_experimental_ops.py` following the pattern:
```
import pytest  
import torch  
import flag_gems  
...
## Performance Tests
Add performance test in `benchmark/test_experimental_perf.py` following the pattern:
```
import pytest  
import torch  
import time  
...
# CI Integration
Accuracy tests to the CI workflow is already integrated in `.github/workflows/gems-test-on-hopper.yaml` . No changes needed there.
