...
def _moe_kernel_w8a16(
    ...
    for k in range(K):
        ...
        # 加载权重: W[expert, k, :]
        # B 形状 (E, K, H), stride_be, stride_bk, stride_bn
...
def fused_moe(
    ...
    W1 形状: (E, inter_dim, hidden_dim) = (E, K, H)
    量化后:
      W_q: (E, K, H) uint8
      scales: (E, K, num_groups) 其中 num_groups = H // group_size
      zeros: (E, K, num_groups)
    """
    ...
    if mode_str == "fp16":
        for pid in range(num_valid_tokens):
            token_id = sorted_token_ids[pid].item()
            expert_id = sorted_expert_ids[pid].item()
            weight = sorted_weights[pid].item()
            inp_slice = x[token_id:token_id+1]
            result = torch.mm(inp_slice, w1[expert_id].T)
