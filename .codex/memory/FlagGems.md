...
### 2026-07-21 adaptive_avg_pool2d 修复

- 状态：fixed；分支 `klx/adaptive-avg-pool2d-fix`，基于 `upstream/master` (`848a4e6c1`) 新建，避免继承旧 `klx/sinc-fix` 中的 agent 记忆文件。
- 基线：`python -m pytest tests/test_adaptive_avg_pool2d.py -q --ref=cpu` 为 `33 failed`，各 dtype/shape 广泛出现错误值。
- 根因分类：昆仑芯定制 Triton 编译器与通用 kernel 不兼容；同设备原生 `torch.ops.aten._adaptive_avg_pool2d` 语义和精度正确。
- 修复：新增昆仑芯 vendor override，委托原生 ATen 实现，并在 vendor `ops/__init__.py` 导出。未修改通用实现或测试。
- 验证：相同原始命令为 `33 passed`。未 commit/push。
- 交付约束：`.codex/memory/FlagGems.md`、`AGENTS.md`、skill、报告和结果仅作本地记录，不得进入算子提交。
