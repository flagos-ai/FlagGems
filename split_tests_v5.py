...
This version correctly handles multiple consecutive decorators.
"""
...
def find_decorator_block_start(lines, func_line):
    ...
    while i >= 0:
        line = lines[i].strip()
        # If it's a decorator or empty line, continue
        if line.startswith('@') or line == '':
            i -= 1
        else:
            # Found a non-decorator, non-empty line
            break

    # i is now pointing to the last non-decorator line before the decorator block
    # So the decorator block starts at i+1
    return i + 1
