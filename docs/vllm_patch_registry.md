...
### patch_util.py - Signatures
```python
_VLLM_OPS_SIGNATURES = {
    "lib_name": {
        "op_name": "PyTorch schema signature",
    },
}
```

### patch_vllm_all.py - Implementations
```python
_VLLM_OPS_IMPLS = {
    "lib_name": {
        "op_name": custom_function,  # 直接引用函数对象
    },
}
```
...
在 `patch_util.py` 的 `_VLLM_OPS_SIGNATURES` 中添加:
...
在 `patch_vllm_all.py` 的 `_VLLM_OPS_IMPLS` 中添加:

```python
_VLLM_OPS_IMPLS = {
    "_C": {
        # ... existing ops
        "my_new_op": custom_my_new_op,
    },
}
```
