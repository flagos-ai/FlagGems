...
@pytest.mark.reflection_pad2d
def test_reflection_pad2d_edge_values():
    # Verify edge pixels are not repeated (characteristic of reflection padding)
    shape = (1, 6, 6)
    dtype = torch.float32
    x = torch.arange(36, dtype=dtype, device=flag_gems.device).reshape(1, 6, 6)
    padding = (2, 2, 2, 2)
    ...
    # Verify edge values are correctly reflected
    # The edge at index 0 should not be repeated in the padding
    assert torch.equal(act_out[0, 2, 2], x[0, 0, 0])  # Original (0,0)


@pytest.mark.reflection_pad2d
def test_reflection_pad2d_invalid_padding():
    shape = (2, 4, 32, 64)
    dtype = torch.float32
    x = torch.randn(shape, dtype=dtype, device=flag_gems.device)

    # Padding >= input dimension should fail
    with pytest.raises(ValueError):
        gems_reflection_pad2d(x, (64, 0, 0, 0))

    # Negative padding should fail
    with pytest.raises(ValueError):
        gems_reflection_pad2d(x, (-1, 0, 0, 0))


@pytest.mark.reflection_pad2d
def test_reflection_pad2d_shape_too_small():
    # Input must be at least 2x2 for reflection padding with pad > 0
    dtype = torch.float32

    # 1x1 input
    x = torch.randn(1, 1, 1, 1, dtype=dtype, device=flag_gems.device)
    with pytest.raises(ValueError):
        gems_reflection_pad2d(x, (0, 0, 0, 1))

    # 1x2 input (height too small)
    x = torch.randn(1, 1, 1, 2, dtype=dtype, device=flag_gems.device)
    with pytest.raises(ValueError):
        gems_reflection_pad2d(x, (0, 0, 1, 0))

    # 2x1 input (width too small)
    x = torch.randn(1, 1, 2, 1, dtype=dtype, device=flag_gems.device)
    with pytest.raises(ValueError):
        gems_reflection_pad2d(x, (1, 0, 0, 0))
