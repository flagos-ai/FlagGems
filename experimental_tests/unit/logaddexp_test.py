...
try:
    from tests.accuracy_util import TO_CPU, gems_assert_close
...
def test_logaddexp_tensor(shape, dtype):
    ...
    ref_out = torch.ops.aten.logaddexp(self, other)
...
def test_accuracy_logaddexp_out(shape, dtype):
    ...
    torch.ops.aten.logaddexp.out(ref_self, ref_other, out=ref_out)

    act_out = torch.empty(shape, dtype=dtype, device=flag_gems.device)
    with flag_gems.use_gems():
        gems_logaddexp_out(self, other, act_out)
