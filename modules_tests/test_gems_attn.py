...
@pytest.mark.skipif(not HAS_VLLM, reason="vLLM is not installed")
@torch.inference_mode()
def test_flash_attn_varlen_func_none_paged(
    seq_lens: List[Tuple[int, int]],
    num_heads: Tuple[int, int],
    head_size: int,
    sliding_window: Optional[int],
    ...
    with torch.device(flag_gems.device):
        ...
        key_cache = torch.randn(total_kv_tokens, num_kv_heads, head_size, dtype=dtype)
        value_cache = torch.randn_like(key_cache)
        cu_query_lens = torch.tensor([0] + query_lens, dtype=torch.int32).cumsum(
            dim=0, dtype=torch.int32
        )
        ...
        cu_kv_lens = torch.tensor(
            [0] + kv_lens, dtype=torch.int32, device=query.device
