...
This version correctly handles multiple consecutive decorators with empty lines.
"""
...
def find_decorator_block_start(lines, func_line):
    """
    Find the start of the decorator block before a function.
    Goes backwards from func_line to find all consecutive decorators.
    Stops at the first empty line that's followed by non-decorator content.
    """
    if func_line <= 0:
        return func_line
    ...
    # First, skip backwards through decorators (no empty lines yet)
    while i >= 0:
        line = lines[i].strip()
        if line.startswith('@'):
            i -= 1
        elif line == '':
            # Found an empty line, check if there are more decorators before it
            break
        else:
            # Found a non-decorator, non-empty line - shouldn't happen
            return i + 1

    # Now i is pointing to an empty line (or -1)
    # Check if there are more decorators before this empty line
    if i >= 0 and lines[i].strip() == '':
        # Look backwards to see if there are decorators before the empty line
        j = i - 1
        while j >= 0:
            line = lines[j].strip()
            if line.startswith('@'):
                # Found more decorators, continue from here
                i = j - 1
                break
            elif line == '':
                # Another empty line, keep looking
                j -= 1
            else:
                # Found non-decorator content, stop here
                break

    # i+1 is the start of the decorator block
    return i + 1
...
def split_performance_file(source_lines):
    ...
    # Find the start of the decorator block for the first performance function
    perf_start = find_decorator_block_start(source_lines, first_perf_func)
