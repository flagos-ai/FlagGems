...
def test__ctc_loss_out_variants(overload):
    ...
    if overload == "out":
        ...
        lengths_arg = (
            torch.tensor(input_lengths, device=flag_gems.device),
            torch.tensor(target_lengths, device=flag_gems.device),
        )

    ref_op = getattr(torch.ops.aten._ctc_loss, overload)
    ref_log_probs = utils.to_reference(log_probs, upcast=False).to(torch.float32)
    ref_targets = utils.to_reference(targets)
    ref_lengths = tuple(
        utils.to_reference(a) if torch.is_tensor(a) else a for a in lengths_arg
    )
    ref_out = ref_op(
        ref_log_probs,
        ref_targets,
