...
def test_accuracy_abs(shape, dtype):
    ...
    inp_ = torch.randn(shape, dtype=dtype, device='cpu')
...
def test_accuracy_bitwisenot(shape, dtype):
    if dtype in BOOL_TYPES:
        inp_ = torch.randint(0, 2, size=shape, dtype=dtype, device="cpu")
    else:
        inp_ = torch.randint(
...
def test_accuracy_bitwisenot_(shape, dtype):
    if dtype in BOOL_TYPES:
        res_inp = torch.randint(0, 2, size=shape, dtype=dtype, device="cpu").to(flag_gems.device)
    else:
...
def test_accuracy_exp_(shape, dtype):
    ...
    ref_inp = to_reference(inp_, True)
...
def test_accuracy_gelu(shape, dtype, approximate):
    res_inp = torch.randn(shape, dtype=dtype, device="cpu").to(flag_gems.device)
...
def test_accuracy_gelu_backward(shape, dtype, approximate):
    res_inp_ = torch.randn(shape, dtype=dtype)
    res_inp = res_inp_.to(flag_gems.device)

    res_out = torch.randn_like(res_inp_)
...
def test_accuracy_isinf(shape, dtype):
    ...
    inp = torch.masked_fill(inp, inp > 1.0, -float("inf")).to(flag_gems.device)
    ref_inp = to_reference(inp)
...
def test_accuracy_isnan(shape, dtype):
    inp = torch.randn(shape, dtype=dtype, device="cpu")
    inp = torch.masked_fill(inp, inp > 1.0, float("nan")).to(flag_gems.device)
    ref_inp = to_reference(inp)
...
def test_accuracy_sigmoid_backward(shape, dtype):
    ...
    res_out_ = torch.randn(shape, dtype=dtype)
    res_out = res_out_.to(flag_gems.device)

    res_grad_ = torch.randn_like(res_out_)
    res_grad = res_grad_.to(flag_gems.device)
...
def test_accuracy_sigmoid_(shape, dtype):
    ...
    ref_inp =  to_reference(res_inp_, True)
...
def test_accuracy_log_sigmoid(shape, dtype):
    ...
    if len(shape) == 1:
        ...
        # )
        special_inputs = torch.tensor(
            SPECIAL_VALUES, dtype=dtype
        )
        # inp = torch.cat((inp, special_inputs))
        inp_ = torch.cat((inp_, special_inputs))
    inp = inp_.to(flag_gems.device)
...
def test_accuracy_silu_backward(shape, dtype):
    ...
    res_grad_ = torch.randn_like(res_inp_)
    res_grad = res_grad_.to(flag_gems.device)
...
def test_accuracy_isfinite(shape, dtype):
    inp_ = torch.randn(shape, dtype=dtype)
    inp_ = torch.masked_fill(inp_, inp_ > 1.0, float("inf"))
    inp_ = torch.masked_fill(inp_, inp_ < -1.0, float("-inf"))
    inp_ = torch.masked_fill(inp_, (inp_ > -0.1) & (inp_ < 0.1), float("nan"))
    inp = inp_.to(flag_gems.device)
    ref_inp = to_reference(inp)
...
def test_accuracy_flip_general(shape, dtype, dims):
    if dtype in ALL_FLOAT_DTYPES:
        ...
        inp_ = torch.randint(-1000, 1000, shape).to(dtype)
    inp = inp_.to(flag_gems.device)
...
def test_accuracy_flip_with_non_dense_input(shape, dtype, dims):
    ...
    if dtype in ALL_FLOAT_DTYPES:
        inp_ = torch.randn(shape_dialted, dtype=dtype)
    else:
        inp_ = torch.randint(-1000, 1000, shape_dialted).to(
            dtype
        )
    inp = inp_.to(flag_gems.device)
    inp = inp[::2, ::2]
    ref_inp = to_reference(inp_[::2, ::2], False)
    with flag_gems.use_gems():
        res_out = torch.flip(inp, dims)
    del inp
    ref_out = torch.flip(ref_inp, dims)
...
@pytest.mark.parametrize(
    "value",
    [
        #torch.tensor(1024).to(flag_gems.device),
        torch.tensor(1024),
        torch.scalar_tensor(1024),
        1024,
    ],
)
def test_accuracy_masked_fill(shape, dtype, threshold, value):
    inp = torch.zeros(shape, dtype=dtype).to(flag_gems.device)
    mask_ = torch.randn(shape, dtype=dtype) < threshold
    mask = mask_.to(flag_gems.device)
    ref_inp = to_reference(inp)
...
def test_accuracy_tile(shape, dims, dtype):
    inp = torch.randn(shape, dtype=dtype).to(flag_gems.device)
    ref_inp = to_reference(inp)
...
def test_accuracy_repeat(shape, sizes, dtype):
    inp = torch.randn(shape, dtype=dtype).to(flag_gems.device)
    ref_inp = to_reference(inp)
    sizes = unsqueeze_tuple(sizes, ref_inp.ndim)
...
def test_accuracy_fill_(value, shape, dtype):
    ...
    ref_value_tensor  = to_reference(value_tensor_, False)
