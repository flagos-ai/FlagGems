...
class TestMeshgrid:
    ...
    @pytest.mark.parametrize("shapes,indexing", [
        ([(3,), (4,)], 'ij'),
        ([(3,), (4,)], 'xy'),
        ([(2,), (3,), (4,)], 'ij'),
        ([(2,), (3,), (4,)], 'xy'),
    ...
    def test_accuracy_cuda(self, shapes, indexing):
        ...
        tensors = [torch.arange(s[0], device='cuda', dtype=torch.float32) for s in shapes]
    ...
    def test_accuracy_cpu(self, shapes, indexing):
        """Test CPU device"""
        tensors = [torch.arange(s[0], device='cpu', dtype=torch.float32) for s in shapes]
        torch_out = torch.meshgrid(tensors, indexing=indexing)
        our_out = meshgrid(tensors, indexing=indexing)
