...
def test_nextafter_zero_boundary():
    ...
    x_pos_zero = torch.zeros(10, dtype=torch.float32, device=flag_gems.device)
    y_neg = torch.full((10,), -1.0, dtype=torch.float32, device=flag_gems.device)
    ...
    x_neg_zero = -torch.zeros(10, dtype=torch.float32, device=flag_gems.device)
    y_pos = torch.full((10,), 1.0, dtype=torch.float32, device=flag_gems.device)
    ...
    with flag_gems.use_gems():
        ...
        utils.gems_assert_close(imp_x2, ref_x2, torch.float32)
...
def test_nextafter_nan():
    ...
    if non_nan_mask.any():
        utils.gems_assert_close(imp_x[non_nan_mask], ref_x[non_nan_mask], torch.float32)
...
@pytest.mark.nextafter_
def test_nextafter_scalar_y():
    """Test nextafter_ with tensor x and scalar y."""
    x = torch.randn(16, dtype=torch.float32, device=flag_gems.device)
    scalar_y = 0.5
...
def test_nextafter_scalar_x():
    ...
    with flag_gems.use_gems():
        ...
        imp_x = torch.tensor(scalar_x, dtype=torch.float32, device=flag_gems.device)
...
@pytest.mark.nextafter_
def test_nextafter_scalar_nan_y():
    """Test nextafter_ with tensor x and NaN scalar y."""
    x = torch.ones(8, dtype=torch.float32, device=flag_gems.device)
    scalar_y = float("nan")
...
@pytest.mark.nextafter_
def test_nextafter_scalar_inf_y():
