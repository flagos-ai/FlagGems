...
@pytest.mark.parametrize("dtype", utils.FLOAT_DTYPES)
def test_sub_tensor_tensor_out(shape1, shape2, out_shape, alpha, dtype):
    ...
    torch.ops.aten.sub.out(ref_inp1, ref_inp2, alpha=alpha, out=ref_out)

    out = torch.empty(out_shape, dtype=dtype, device=flag_gems.device)
    with flag_gems.use_gems():
        res_out = torch.ops.aten.sub.out(inp1, inp2, alpha=alpha, out=out)
