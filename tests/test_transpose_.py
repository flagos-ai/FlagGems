...
# Dimension pairs to swap. Covers first/last, middle pairs, negative dims,
# and the identity (same-dim) no-op case.
TRANSPOSE_DIM_PAIRS = [
    (0, 1),
    (0, -1),
    (-1, -2),
...
# Shapes covering 0D, 1D, 2D, 3D, 4D. transpose_ is an in-place view op, so we
# exercise shapes where swapping dimensions is observable (ndim >= 2) plus the
# degenerate 0D/1D cases.
TRANSPOSE_SHAPES = [
    (),
    (5,),
    (2, 3),
...
def test_transpose_(shape, dim_pair, dtype):
    dim0, dim1 = dim_pair
    ...
    if ndim == 0:
        ...
        if not (d0 == 0 and d1 == 0):
            pytest.skip("0D tensor only supports transpose_(0, 0)")
    elif ndim == 1:
        # 1D: only (0, 0) is valid.
        if not (d0 == 0 and d1 == 0):
            pytest.skip("1D tensor only supports transpose_(0, 0)")
    elif d0 >= ndim or d1 >= ndim:
        pytest.skip(f"dim pair {dim_pair} out of range for {ndim}D tensor")

    # In-place: clone the inputs so reference and GEMS run on independent copies,
    # then compare both the returned tensor and the mutated input.
    ...
    # Rule 43: also compare the mutated input against the reference result.
    utils.gems_assert_equal(inp1, ref_out)
    # transpose_ is a view op: verify shape/strides match aten.
...
@pytest.mark.parametrize("dtype", utils.FLOAT_DTYPES)
def test_transpose__same_dim(shape, dim0, dim1, dtype):
    inp = torch.randn(shape, dtype=dtype, device=flag_gems.device)
    ref_inp = utils.to_reference(inp)
...
@pytest.mark.parametrize("dtype", utils.FLOAT_DTYPES)
def test_transpose__non_contiguous(shape, dtype):
    # Verify transpose_ works correctly on a non-contiguous input. Build the
    # non-contiguous slice on both the test device and the reference device so
    # the two inputs share the same memory layout (calling utils.to_reference after
    # slicing would densify the CPU copy and make the stride comparison compare
    # mismatched layouts).
