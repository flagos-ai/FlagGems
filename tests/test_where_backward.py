...
def test_where_self_backward(shape, dtype):
    ...
    ref_grad_input = torch.ops.aten.where.Self_backward(
        ref_grad, ref_cond, ref_self, ref_other
    )

    with flag_gems.use_gems():
        res_grad_input = torch.ops.aten.where.Self_backward(
            grad_output, condition, self, other
        )
...
def test_where_other_backward(shape, dtype):
    ...
    ref_grad_input = torch.ops.aten.where.Other_backward(
        ref_grad, ref_cond, ref_self, ref_other
    )

    with flag_gems.use_gems():
        res_grad_input = torch.ops.aten.where.Other_backward(
            grad_output, condition, self, other
        )
