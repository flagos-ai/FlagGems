...
@pytest.mark.parametrize("dtype", FLOAT_DTYPES)
def test_accuracy_diff(shape, dtype):
    """Test basic diff operation along default dimension."""
    inp = torch.randn(shape, dtype=dtype, device=flag_gems.device)
    ref_inp = to_reference(inp)
...
@pytest.mark.parametrize("dtype", FLOAT_DTYPES)
@pytest.mark.parametrize("dim", [0, -1])
def test_accuracy_diff_dim(shape, dtype, dim):
    """Test diff operation along specified dimension."""
    inp = torch.randn(shape, dtype=dtype, device=flag_gems.device)
    ref_inp = to_reference(inp)
...
def test_accuracy_diff_n(shape, dtype, n):
    """Test diff operation with n > 1 (multiple iterations)."""
    inp = torch.randn(shape, dtype=dtype, device=flag_gems.device)
    ref_inp = to_reference(inp)
...
def test_accuracy_diff_prepend_append(shape, dtype):
    """Test diff operation with prepend and append tensors."""
    inp = torch.randn(shape, dtype=dtype, device=flag_gems.device)
    prepend = torch.randn(shape[:-1] + (3,) if len(shape) > 1 else (3,), dtype=dtype, device=flag_gems.device)
    append = torch.randn(shape[:-1] + (2,) if len(shape) > 1 else (2,), dtype=dtype, device=flag_gems.device)
