...
# output_mask selects which of (grad_input, grad_weight, grad_bias) are computed.
# Include partial masks so the False branches (a skipped gradient returns None) are
# exercised, not just the all-True case.
OUTPUT_MASKS = [
    [True, True, True],
    [True, False, False],
    [False, True, False],
...
def test_convolution_backward_overrideable(
    shape, kernel, groups, stride, padding, output_mask, dtype
):
...
def test_convolution_backward_overrideable_out(
    ...
    # The .out variant always returns the provided out tensors; only the gradients
    # selected by output_mask are written, so compare just those.
    if output_mask[0]:
        utils.gems_assert_close(res_gi, ref_gi, dtype, reduce_dim=weight.shape[2])
    if output_mask[1]:
        utils.gems_assert_close(res_gw, ref_gw, dtype, reduce_dim=weight.shape[0])
