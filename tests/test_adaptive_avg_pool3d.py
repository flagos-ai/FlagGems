...
def test_accuracy_adaptive_avg_pool3d_forward(shape, output_size, dtype):
    ...
    res_out = flag_gems._adaptive_avg_pool3d(inp, output_size)
...
@pytest.mark.parametrize("shape, output_size", ADAPTIVE_AVG_POOL3D_CONFIGS)
@pytest.mark.parametrize("dtype", FLOAT_DTYPES)
def test_accuracy_adaptive_avg_pool3d_out(shape, output_size, dtype):
    inp = torch.randn(shape, dtype=dtype, device=flag_gems.device)
    ref_inp = utils.to_reference(inp, True)
    ...
    res_out = flag_gems._adaptive_avg_pool3d_out(inp, output_size, out=act_out_buf)
