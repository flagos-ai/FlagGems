...
class MultiTaskLoss(nn.Module):
    ...
    def forward(self, logits_cls, target_cls, pred_reg, target_reg, 
        ...
        # 加权组合
        total_loss = self.weights[0] * loss1 + self.weights[1] * loss2 + self.weights[2] * loss3
        return total_loss
...
class TestCrossEntropyCombination:
    ...
    @pytest.mark.parametrize(
        ...
        [
            ...
            pytest.param(32, 1000, 64, id="large",
                        marks=pytest.mark.skipif(QUICK_MODE, reason="Quick mode")),
        ],
    )
    def test_cross_entropy_step_by_step(self, batch_size, num_classes, seq_len, use_gems):
        """测试分步实现的CrossEntropy"""
        ...
        assert_loss_close(loss_step, loss_standard, logits.dtype, name="CrossEntropy step-by-step")
    ...
    def test_cross_entropy_large_logits(self, use_gems):
        ...
        assert_loss_close(loss, ref_loss, logits.dtype, name="CrossEntropy large logits")
...
class TestMarginRankingLoss:
    ...
    def test_margin_ranking_basic(self, use_gems):
        ...
        negative = torch.randn(8, device=device)
...
class TestMultiTaskLossCombination:
    ...
    def test_multi_task_loss_weighted(self, use_gems):
        ...
        total_loss = model(logits_cls, target_cls, pred_reg, target_reg,
                           anchor, positive, label)
