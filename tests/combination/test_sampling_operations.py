...
class TestDropoutCombination:
    ...
    def test_dropout_training_vs_eval(self, use_gems):
        ...
        # 训练模式
        model.train()
        output_train1 = model(x, training=True)
        output_train2 = model(x, training=True)
    ...
    def test_dropout_scaling_factor(self, use_gems):
        ...
        if non_zero_mask.any():
            ...
            assert torch.allclose(actual_values, 
                                 torch.ones_like(actual_values) * expected_scale,
                                 rtol=0.1)
...
class TestSamplingOperations:
    ...
    def test_multinomial_sampling(self, use_gems):
        ...
        # 验证概率和为1
        assert torch.allclose(probs.sum(dim=-1), torch.ones(100, device=device), atol=1e-5)

        # Reference comparison for the deterministic part (probabilities)
        ref_probs = F.softmax(logits.detach().to("cpu").to(torch.float64) / 1.0, dim=-1).to(device)
        assert_loss_close(probs.sum(), ref_probs.sum().float(), logits.dtype, name="Sampling probs sum")
