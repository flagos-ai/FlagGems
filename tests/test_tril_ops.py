...
# ├──────────────────────────┼────────────────────────────────────────────────┼────────────────────────────────────────┤
# │ test_tril_2d_shapes      │ (1,1),(8,8),(64,64),(256,256),                  │ float16,float32,bfloat16,              │
# │                          │ (1024,1024),(4096,4096)                         │ int16,int32,int64                      │
# │ test_tril_diagonals      │ (64,64),(256,256) × diag in {-2,-1,0,1,2}      │ float32,int32                          │
# │ test_tril_3d_batch       │ (4,64,64),(8,32,32)                            │ float16,float32,bfloat16,int32         │
# │ test_tril_4d_batch       │ (2,3,32,32),(4,4,16,16)                        │ float32,int32                          │
# │ test_tril_nonsquare      │ (128,64),(64,256),(1,1024),(1024,1)             │ float32,int32                          │
# │ test_tril_edge_diag      │ (64,64) × diag large-positive, large-negative  │ float32                                │
# │ test_tril_inplace        │ (1,1),(8,8),(64,64),(256,256),                  │ float16,float32,bfloat16,              │
# │                          │ (1024,1024)                                     │ int16,int32,int64                      │
# │ test_tril_inplace_diag   │ (64,64) × diag in {-2,-1,0,1,2}               │ float32,int32                          │
# │ test_tril_inplace_batch  │ (4,64,64),(2,3,32,32)                          │ float32,int32                          │
# └──────────────────────────┴────────────────────────────────────────────────┴────────────────────────────────────────┘
...
def make_input(shape, dtype):
    if dtype in FLOAT_DTYPES:
        ...
        return torch.randint(iinfo.min // 2, iinfo.max // 2, shape, dtype=dtype, device=flag_gems.device)


def assert_close(result, reference, dtype):
    if dtype in INT_DTYPES:
        assert torch.equal(result, reference), f"Integer tril mismatch for dtype={dtype}"
    else:
...
@pytest.mark.parametrize("dtype", [torch.float16, torch.float32, torch.bfloat16, torch.int32])
