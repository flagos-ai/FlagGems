...
@pytest.mark.parametrize(
    "dtype",
    [torch.float32] if cfg.QUICK_MODE else utils.FLOAT_DTYPES + utils.ALL_INT_DTYPES + [None],
)
@pytest.mark.parametrize("device", [flag_gems.device] if cfg.QUICK_MODE else [flag_gems.device, None])
