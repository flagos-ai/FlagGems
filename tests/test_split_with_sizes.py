...
@pytest.mark.parametrize("shape", SPLIT_WITH_SIZES_SHAPES)
@pytest.mark.parametrize("dtype", utils.FLOAT_DTYPES)
def test_split_with_sizes(shape, dtype):
    ...
    ref_out = torch.split_with_sizes(ref_inp, split_sizes, dim=dim)
    res_out = flag_gems.split_with_sizes(inp, split_sizes, dim=dim)
...
def test_split_with_sizes_different_dims(shape, dtype, dim):
    inp = torch.randn(shape, dtype=dtype, device=flag_gems.device)
    ref_inp = utils.to_reference(inp)
...
@pytest.mark.parametrize("dtype", utils.FLOAT_DTYPES)
def test_split_with_sizes_edge_cases(shape, dtype):
