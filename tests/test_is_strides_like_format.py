...
def test_contiguous_random():
    ...
    for _ in range(num_examples):
        ...
        assert (
            result.item() is True
        ), f"Expected True for contiguous tensor with shape {shape}, got {result.item()}"
        ...
        if not x_t.is_contiguous():
            ...
            assert (
                result2.item() is False
            ), f"Expected False for non‑contiguous tensor with shape {shape}, got {result2.item()}"
...
def test_any_random():
    ...
    for _ in range(num_examples):
        ...
        assert (
            result.item() is True
        ), f"Expected True for any tensor with shape {shape}, got {result.item()}"
        ...
        assert (
            result2.item() is True
        ), f"Expected True for non‑contiguous any tensor with shape {shape}, got {result2.item()}"
...
def test_channels_last_random():
    ...
    for _ in range(num_examples):
        ...
        if len(shape) != 4:
            ...
            assert (
                result.item() is False
            ), f"Expected False for non-4D shape {shape}, got {result.item()}"
            continue
