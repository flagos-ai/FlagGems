...
def test_accuracy_add_(shape, alpha, dtype):
    ...
    ref_inp1 =  to_reference(inp1_, True)
    ref_inp2 = to_reference(inp2, True)
...
def test_accuracy_bitwiseand_(shape, dtype):
    if dtype in BOOL_TYPES:
        inp1 = torch.randint(0, 2, size=shape, dtype=dtype, device="cpu").to(flag_gems.device)
        inp2 = torch.randint(0, 2, size=shape, dtype=dtype, device="cpu").to(flag_gems.device)

    else:
        inp1 = torch.randint(
            low=-0x7FFF, high=0x7FFF, size=shape, dtype=dtype, device="cpu").to(flag_gems.device)
...
def test_accuracy_clamp_tensor(shape, isnone, dtype):
    inp = torch.randn(shape, dtype=dtype, device="cpu").to(flag_gems.device)
    maxi = torch.randn(shape, dtype=dtype, device="cpu").to(flag_gems.device)
    mini = torch.randn(shape, dtype=dtype, device="cpu").to(flag_gems.device)
    if isnone == "min":
        mini = None
    elif isnone == "max":
        maxi = None
...
def test_accuracy_floor_div_float(shape, dtype):
    inp1 = torch.randn(shape, dtype=dtype).to(flag_gems.device)
    inp2 = torch.randn(shape, dtype=dtype).to(flag_gems.device)
    ref_inp1 = to_reference(inp1, False)
    ref_inp2 = to_reference(inp2, False)
...
def test_accuracy_floor_div_int(shape, dtype):
    ...
    if TO_CPU:
        inp1_ = replace_zeros(inp1_)
        inp2_ = replace_zeros(inp2_)
    inp1 = inp1_.to(flag_gems.device)
    inp2 = inp2_.to(flag_gems.device)
    inp_test = inp2.flatten()[:2].to("cpu")
    ref_inp1 = to_reference(inp1, False)
    ref_inp2 = to_reference(inp2, False)
    ...
    for i in range(inp2_.flatten()[:2].size(0)):
        ref_d = inp2_.flatten()[:2][i]
        d = inp2.flatten()[:2][i]
...
def test_accuracy_eq(shape, dtype):
    inp1 = torch.randint(0, 10, shape, dtype=dtype, device="cpu").to(flag_gems.device)

    inp2 = torch.randint(0, 10, shape, dtype=dtype, device="cpu").to(flag_gems.device)
...
def test_accuracy_gelu_and_mul(shape, approximate, dtype):
    inp1_ = torch.randn(shape, dtype=dtype, device = "cpu", requires_grad=True)
    inp1 = inp1_.to(flag_gems.device)
    inp2_ = torch.randn(shape, dtype=dtype, device = "cpu", requires_grad=True)
    inp2 = inp2_.to(flag_gems.device)
    ref_inp1 = to_reference(inp1, True)
    ref_inp2 = to_reference(inp2, True)
    ...
    res_out_ = to_reference(res_out)
    out_grad_ = torch.randn_like(res_out_)
    out_grad = out_grad_.to(flag_gems.device)
...
def test_accuracy_ne(shape, dtype):
    ...
    inp1_ = torch.randint(0, 10, shape, dtype=dtype)
    inp2_ =  torch.randint(0, 10, shape, dtype=dtype)
    inp1 = inp1_.to(flag_gems.device)
    inp2 = inp2_.to(flag_gems.device)
...
def test_accuracy_rsub(shape, alpha, dtype):
    ...
    ref_inp1 = to_reference(inp1_, True)
    ref_inp2 = to_reference(inp2_, True)
    ...
    #gems_assert_close(res_out, ref_out, dtype)
    print("Warning: rsub golden cpu f64, real out is bf16, assert must use cosine_similarity!!!")
...
def test_accuracy_where_self_out_cross_device(shape, dtype):
    inp1 = torch.randn(shape, dtype=dtype).to(flag_gems.device)
    inp2 = torch.randn(shape, dtype=dtype).to(flag_gems.device)
    cond = torch.randint(0, 2, shape, dtype=torch.bool).to(flag_gems.device)
...
def test_accuracy_where_self_out(shape, dtype):
    ...
    inp2 = torch.randn(shape, dtype=dtype).to(flag_gems.device)
    cond = torch.randint(0, 2, shape, dtype=torch.bool).to(flag_gems.device)
    out = torch.empty(shape, dtype=dtype).to(flag_gems.device)
...
def test_accuracy_isclose(shape, dtype, zero_tol, equal_nan, gen_nan):
    ...
    rtol = (
        torch.rand(1, dtype=torch.float32).item() * 0.0001
        if not zero_tol
        else 0
    )
    if dtype in ALL_FLOAT_DTYPES:
        ...
        if gen_nan:
            ...
            inp1_.view(-1)[0] = -(to_reference(nan_num)) if gen_nan == 3 else nan_num
            inp2_.view(-1)[0] = -(to_reference(nan_num)) if gen_nan >= 3 else nan_num
        atol = (
        ...
        inp1_ = torch.randint(-1000, 1000, shape).to(dtype)
        inp2_ = torch.randint(-1000, 1000, shape).to(dtype)
        if dtype in [torch.int64]:
            inp1_.view(-1)[0] = 2**63 - 1
            inp2_.view(-1)[0] = -(2**63)
            if inp1_.numel() > 2 and inp2_.numel() > 2:
                inp1_.view(-1)[1] = 2**60 + 2**20
                inp2_.view(-1)[1] = 2**60
                inp1_.view(-1)[2] = 2**60 + 1
                inp2_.view(-1)[2] = 2**60
            atol = 2 if not zero_tol else 0
            if gen_nan == 0:
                rtol = 0
        elif dtype in [torch.int32]:
            inp1_.view(-1)[0] = 2**31 - 1
            inp2_.view(-1)[0] = -(2**31)
            if inp1_.numel() > 2 and inp2_.numel() > 2:
                inp1_.view(-1)[1] = 2**30 + 2**5
                inp2_.view(-1)[1] = 2**30
                inp1_.view(-1)[2] = 2**30 + 1
                inp2_.view(-1)[2] = 2**30
            atol = 2 if not zero_tol else 0
            if gen_nan == 0:
                rtol = 0
...
def test_accuracy_allclose(shape, dtype, equal_nan, gen_nan):
    # [gen_nan] 1: nan, 2: inf, 3: -inf, 4: inf vs -inf
    rtol = torch.rand(1, dtype=torch.float32).item() * (
    ...
    if dtype in ALL_FLOAT_DTYPES:
        ...
        inp1_ = torch.full(shape, 1.234, dtype=dtype)
        inp2_ = torch.full(shape, 1.234, dtype=dtype)
        if gen_nan:
            nan_num = torch.full(
                (1,),
                float("nan" if gen_nan == 1 else "inf"),
            ...
            inp1_.view(-1)[0] = (
                (-(to_reference(nan_num))) if gen_nan == 3 else nan_num
            )
            inp2_.view(-1)[0] = (
                (-(to_reference(nan_num))) if gen_nan >= 3 else nan_num
            )
