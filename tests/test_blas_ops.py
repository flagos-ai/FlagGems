...
def test_accuracy_addmm(M, N, K, scalar, dtype):
    ...
    if dtype == torch.float32:
        gems_assert_cosine_similarity(res_out1, ref_out1, dtype)
    else:
    ...
    if dtype == torch.float32:
        gems_assert_cosine_similarity(res_out2, ref_out2, dtype)
    else:
...
def test_accuracy_bmm(M, N, K, dtype):
    ...
    mat1_ = torch.randn((batch, M, K), dtype=dtype)
    mat2_ = torch.randn((batch, K, N), dtype=dtype)
    mat1 = mat1_.to(flag_gems.device)
    mat2 = mat2_.to(flag_gems.device)
...
def test_accuracy_mm(M, N, K, dtype):
    ...
    mat1_ = torch.randn((M, K), dtype=dtype)
    mat2_ = torch.randn((K, N), dtype=dtype)
    mat1 = mat1_.to(flag_gems.device)
    mat2 = mat2_.to(flag_gems.device)
...
def test_accuracy_mv(M, N, dtype):
    ...
    matrix_ = torch.randn((N, M), dtype=dtype)
    vector_ = torch.randn((M,), dtype=dtype)
    matrix = matrix_.to(flag_gems.device)
    vector = vector_.to(flag_gems.device)
...
def test_accuracy_outer(M, N, dtype):
    inp1 = torch.randn(M, dtype=dtype, requires_grad=True).to(flag_gems.device)
    inp2 = torch.randn(N, dtype=dtype, requires_grad=True).to(flag_gems.device)
    ...
    #res_out_ = res_out.to("cpu")
    out_grad_ = torch.randn_like(to_reference(res_out))
    #ref_grad = to_reference(out_grad_, True)
    out_grad = out_grad_.to(flag_gems.device)
    ref_in1_grad, ref_in2_grad = torch.autograd.grad(
        ref_out, (ref_inp1, ref_inp2), out_grad_
    )
