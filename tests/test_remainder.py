...
def test_remainder(shape, dtype):
    ...
    # scalar_tensor
    scalar = 7
    ref_out = torch.remainder(torch.tensor(scalar, dtype=dtype), ref_inp1)
    with flag_gems.use_gems():
        res_out = torch.remainder(scalar, inp1)
    utils.gems_assert_equal(res_out, ref_out)
...
def test_remainder_(shape, dtype):
    ...
    ref_inp = utils.to_reference(inp1.clone(), False)
    ref_out = ref_inp.remainder_(scalar)

    with flag_gems.use_gems():
        res_out = inp1.remainder_(scalar)
