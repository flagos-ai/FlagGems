...
def _scatter_reduce_upcast(reduce, dtype):
    return dtype in FLOAT_DTYPES and reduce in {"sum", "mean"}


def _assert_scatter_reduce_result(res_out, ref_out, dtype, *, equal_nan=False):
    if dtype in FLOAT_DTYPES:
        gems_assert_close(res_out, ref_out, dtype, equal_nan=equal_nan)
    else:
        gems_assert_equal(res_out, ref_out, equal_nan=equal_nan)
...
@pytest.mark.parametrize(
    "inp_shape, src_shape",
    [((16, 8, 4), (8, 4, 4))] if QUICK_MODE else [((64, 16, 8), (32, 8, 4))],
)
@pytest.mark.parametrize("dim", [0, 1, 2])
@pytest.mark.parametrize("include_self", [True, False])
@pytest.mark.parametrize("reduce", SCATTER_REDUCE_REDUCTIONS)
...
def test_accuracy_scatter_reduce_negative_dim(dim, include_self, reduce, dtype):
...
@pytest.mark.parametrize("reduce", SCATTER_REDUCE_REDUCTIONS)
@pytest.mark.parametrize("dtype", [torch.float32, torch.int32])
def test_accuracy_scatter_reduce_scalar(dim, include_self, reduce, dtype):
...
@pytest.mark.parametrize("include_self", [True, False])
@pytest.mark.parametrize("dtype", [torch.float32, torch.int32])
def test_accuracy_scatter_reduce_empty_src(reduce, include_self, dtype):
    dim = 1
    if dtype in INT_DTYPES:
...
@pytest.mark.parametrize("reduce", SCATTER_REDUCE_REDUCTIONS)
@pytest.mark.parametrize("include_self", [True, False])
def test_accuracy_scatter_reduce_special_values(reduce, include_self):
    dim = 1
    dtype = torch.float32
    inp = torch.tensor(
...
@pytest.mark.parametrize("include_self", [True, False])
@pytest.mark.parametrize("dtype", [torch.float32, torch.int32])
def test_accuracy_scatter_reduce_empty_self_out(reduce, include_self, dtype):
...
@pytest.mark.parametrize("include_self", [True, False])
@pytest.mark.parametrize("dtype", [torch.float32, torch.int32])
def test_accuracy_scatter_reduce_out_alias_input(reduce, include_self, dtype):
    dim = 2
    inp, index, src = _make_scatter_reduce_tensors((16, 8, 4), (8, 4, 4), dim, dtype)
    ref_inp = to_reference(inp, upcast=_scatter_reduce_upcast(reduce, dtype))
    ref_index = to_reference(index)
...
def test_accuracy_scatter_reduce_invalid_inputs(case):
    ...
    ref_args = tuple(to_reference(arg) if torch.is_tensor(arg) else arg for arg in args)
