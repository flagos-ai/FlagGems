...
@pytest.mark.acosh
@pytest.mark.parametrize("dtype", utils.FLOAT_DTYPES)
def test_acosh_boundary(dtype):
    # Test x=1 boundary: acosh(1) = 0
    inp = torch.ones(32, dtype=dtype, device=flag_gems.device)
    ref_inp = utils.to_reference(inp)
...
@pytest.mark.acosh
@pytest.mark.parametrize("dtype", utils.FLOAT_DTYPES)
def test_acosh_large_input(dtype):
    # Test large input values for numerical stability
    inp = torch.empty(32, dtype=dtype, device=flag_gems.device).uniform_(100.0, 1000.0)
    ref_inp = utils.to_reference(inp)
