...
def test_accuracy_groupnorm(N, C, H, W, num_groups, dtype, wb_none):
    ...
    res_inp = torch.randn(size=(N, C, H, W), dtype=dtype, device="cpu").to(flag_gems.device)
    if wb_none:
        res_weight = None
        res_bias = None
    else:
        res_weight = torch.randn(size=(C,), dtype=dtype, device="cpu").to(flag_gems.device)
        res_bias = torch.randn(size=(C,), dtype=dtype, device="cpu").to(flag_gems.device)
    eps = 1e-5
...
def test_accuracy_groupnorm_backward(N, C, H, W, num_groups, dtype, wb_none):
    ...
    res_inp_ = torch.randn(size=(N, C, H, W), dtype=dtype, device="cpu")
    res_inp = res_inp_.to(flag_gems.device)

    res_grad = torch.randn_like(res_inp_).to(flag_gems.device)
    res_mean = torch.randn([N, num_groups], dtype=dtype, device="cpu").to(flag_gems.device)
    res_rstd = torch.randn([N, num_groups], dtype=dtype, device="cpu").to(flag_gems.device)

    if wb_none:
        res_weight = None
        output_mask = [True, False, False]
    else:
        res_weight = torch.randn(C, dtype=dtype, device="cpu").to(flag_gems.device)
...
def test_accuracy_layernorm(shape, dtype, wb_none):
    ...
    if wb_none:
        ...
        res_weight = torch.randn(shape[1:], dtype=dtype).to(flag_gems.device)
        res_bias = torch.randn(shape[1:], dtype=dtype).to(flag_gems.device)
    eps = 1e-5
...
def test_accuracy_layernorm_backward(shape, dtype, wb_none):
    ...
    res_mean_ = torch.randn(shape[0], dtype=dtype)
    res_rstd_ = torch.randn(shape[0], dtype=dtype)
    res_mean = res_mean_.to(flag_gems.device)
    res_rstd = res_rstd_.to(flag_gems.device)
    if wb_none:
        res_weight = None
        res_bias = None
        output_mask = [True, False, False]
    else:
        res_weight_ = torch.randn(shape[1:], dtype=dtype)
        res_weight = res_weight_.to(flag_gems.device)
        res_bias_ = torch.randn(shape[1:], dtype=dtype)
        res_bias = res_bias_.to(flag_gems.device)
        output_mask = [True, True, True]
    normalized_shape = shape[1:]

    ref_inp = to_reference(res_inp_, True)
    ref_grad = to_reference(res_grad_, True)
    ref_mean = to_reference(res_mean_, True)
    ref_rstd = to_reference(res_rstd_, True)
    ref_weight = to_reference(res_weight, True)
    ref_bias = to_reference(res_bias, True)
