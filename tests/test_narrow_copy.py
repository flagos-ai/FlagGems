import pytest
import torch

import flag_gems

from . import accuracy_utils as utils

# Shape configurations for narrow_copy testing: 2D, 3D, and 1D tensors
NARROW_COPY_SHAPES = [(16, 32, 64), (32, 64), (64,)]


@pytest.mark.narrow_copy
@pytest.mark.parametrize("shape", NARROW_COPY_SHAPES)
@pytest.mark.parametrize("dtype", utils.FLOAT_DTYPES)
def test_narrow_copy(shape, dtype):
    inp = torch.randn(shape, dtype=dtype, device=flag_gems.device)
    ref_inp = utils.to_reference(inp)

    # Test different dims and start positions
    for dim in range(inp.ndim):
        dim_size = inp.size(dim)
        for start in [0, dim_size // 4, dim_size // 2]:
            for length in [1, dim_size // 4, dim_size // 2]:
                if start + length <= dim_size:
                    ref_out = torch.narrow_copy(ref_inp, dim, start, length)
                    with flag_gems.use_gems():
                        res_out = torch.narrow_copy(inp, dim, start, length)
                    utils.gems_assert_equal(res_out, ref_out)


@pytest.mark.narrow_copy
@pytest.mark.parametrize("dtype", utils.FLOAT_DTYPES)
def test_narrow_copy_negative_start(dtype):
    # Test negative start index
    shape = (8, 16, 32)
    inp = torch.randn(shape, dtype=dtype, device=flag_gems.device)
    ref_inp = utils.to_reference(inp)

    ref_out = torch.narrow_copy(ref_inp, 1, -8, 4)
    with flag_gems.use_gems():
        res_out = torch.narrow_copy(inp, 1, -8, 4)
    utils.gems_assert_equal(res_out, ref_out)


@pytest.mark.narrow_copy
@pytest.mark.parametrize("dtype", utils.FLOAT_DTYPES)
def test_narrow_copy_full_slice(dtype):
    # Test when length equals the full dimension
    shape = (4, 8, 16)
    inp = torch.randn(shape, dtype=dtype, device=flag_gems.device)
    ref_inp = utils.to_reference(inp)

    ref_out = torch.narrow_copy(ref_inp, 0, 0, 4)
    with flag_gems.use_gems():
        res_out = torch.narrow_copy(inp, 0, 0, 4)
    utils.gems_assert_equal(res_out, ref_out)


@pytest.mark.narrow_copy
@pytest.mark.parametrize("dtype", utils.FLOAT_DTYPES)
def test_narrow_copy_1d(dtype):
    # Test 1D tensor
    inp = torch.randn(64, dtype=dtype, device=flag_gems.device)
    ref_inp = utils.to_reference(inp)

    ref_out = torch.narrow_copy(ref_inp, 0, 10, 20)
    with flag_gems.use_gems():
        res_out = torch.narrow_copy(inp, 0, 10, 20)
    utils.gems_assert_equal(res_out, ref_out)
