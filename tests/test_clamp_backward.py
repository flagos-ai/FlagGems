...
def test_clamp_tensor_backward(shape, dtype):
    ...
    max_val = (
        min_val
        + torch.abs(torch.randn(shape, dtype=dtype, device=flag_gems.device))
        + 0.5
    )
    ...
    ref_grad_input = torch.ops.aten.clamp.Tensor_backward(
        ref_grad, ref_x, ref_min, ref_max
    )

    with flag_gems.use_gems():
        res_grad_input = torch.ops.aten.clamp.Tensor_backward(
            grad_output, x, min_val, max_val
        )
...
def test_clamp_min_tensor_backward(shape, dtype):
    ...
    with flag_gems.use_gems():
        res_grad_input = torch.ops.aten.clamp_min.Tensor_backward(
            grad_output, x, min_val
        )
...
def test_clamp_max_tensor_backward(shape, dtype):
    ...
    with flag_gems.use_gems():
        res_grad_input = torch.ops.aten.clamp_max.Tensor_backward(
            grad_output, x, max_val
        )
