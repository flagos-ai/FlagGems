...
# ---------------------------------------------------------------------------
# Out-of-place variant: aten::_fused_adagrad  (returns fresh tensors, no mutation)
# ---------------------------------------------------------------------------


@pytest.mark.fused_adagrad
@pytest.mark.parametrize("shape", FUSED_ADAGRAD_SHAPES)
@pytest.mark.parametrize("dtype", utils.FLOAT_DTYPES)
def test_fused_adagrad(shape, dtype):
    """Out-of-place Adagrad step returns new tensors without mutating inputs."""
    atol = TOLS[dtype]

    torch.manual_seed(42)
    params, grads, state_sums, steps = _make_inputs(shape, dtype, flag_gems.device)
    param_orig = params[0].clone()
    grad_orig = grads[0].clone()
    state_sum_orig = state_sums[0].clone()
    ...
    assert torch.equal(grads[0], grad_orig)
    assert torch.equal(state_sums[0], state_sum_orig)

    utils.gems_assert_close(
        utils.to_reference(out_params[0]), utils.to_reference(ref_p), dtype, atol=atol
    )
    utils.gems_assert_close(
        utils.to_reference(out_state_sums[0]),
        utils.to_reference(ref_s),
        dtype,
        atol=atol,
    )
