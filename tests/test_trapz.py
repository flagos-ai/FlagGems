...
def test_trapz_fp64(shape, dim):
    ...
    ref_out = torch.trapezoid(ref_inp, dx=0.5, dim=dim)
    res_out = flag_gems.trapz(inp, dx=0.5, dim=dim)
...
def test_trapz_complex(shape, dtype, dim):
    """Complex inputs fall back to the native CPU implementation."""
