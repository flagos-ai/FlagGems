...
import flag_gems
from flag_gems.experimental_ops.amin import amin as gems_amin, amin_out as gems_amin_out
...
def test_amin_tensor_reduce_2d_performance(shape, dtype, dim, keepdim):
    ...
    if dim is None and not keepdim:
        ...
        with flag_gems.use_gems():
            ms_triton, _, _ = triton.testing.do_bench(
                lambda: gems_amin(x, use_dim, keepdim),
                rep=100,
                quantiles=quantiles
            )
...
def test_amin_out_reduce_2d_performance(shape, dtype, dim, keepdim):
    ...
    if dim is None and not keepdim:
        ...
        with flag_gems.use_gems():
            ms_triton, _, _ = triton.testing.do_bench(
                lambda: gems_amin_out(x, act_out_t),
                rep=100,
                quantiles=quantiles
            )
    else:
