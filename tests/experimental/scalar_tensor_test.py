...
import flag_gems
from flag_gems.experimental_ops.scalar_tensor import scalar_tensor as gems_scalar_tensor, scalar_tensor_out as gems_scalar_tensor_out
...
def test_scalar_tensor_default(val, dtype):
    ref_out = torch.ops.aten.scalar_tensor(val, dtype=dtype, device=flag_gems.device, layout=None, pin_memory=None)
    with flag_gems.use_gems():
        act_out = gems_scalar_tensor(val, dtype=dtype, device=flag_gems.device, layout=None, pin_memory=None)
...
def test_perf_aten_scalar_tensor():
    ...
    for dtype in dtypes:
        ...
        ms_torch, _, _ = triton.testing.do_bench(
            lambda: torch.ops.aten.scalar_tensor(val, dtype=dtype, device=flag_gems.device),
            rep=100,
            quantiles=quantiles
        )
        ...
        print(f"scalar_tensor {dtype}: FlagGems={ms_gems:.3f}ms, Speedup={speedup:.2f}x")