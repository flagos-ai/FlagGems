...
import flag_gems
from flag_gems.experimental_ops.zeros_like import zeros_like as gems_zeros_like, zeros_like_out as gems_zeros_like_out
...
@pytest.mark.parametrize("opts_case", ["none", "dtype_override", "contig_memfmt", "channels_last", "layout_device"])
...
def test_zeros_like_out_overload(shape, in_dtype, out_dtype, memfmt_case):
    ...
    if memfmt_case == "none":
        ...
        memfmt = torch.contiguous_format if memfmt_case == "contiguous" else (torch.channels_last if inp.ndim == 4 else torch.contiguous_format)
        ref_out = torch.ops.aten.zeros_like.out(ref_inp, memory_format=memfmt, out=out_ref)
        with flag_gems.use_gems():
...
def test_zeros_like_default_overload_performance(shape, in_dtype, opts_case):
    ...
    with flag_gems.use_gems():
        ms_triton, _, _ = triton.testing.do_bench(
            lambda: gems_zeros_like(act_inp, **kwargs),
            rep=100,
            quantiles=quantiles
        )
...
def test_zeros_like_out_overload_performance(shape, in_dtype, out_dtype, memfmt_case):
    ...
    if memfmt_case == "none":
        ...
        # PyTorch reference implementation
        ms_torch, _, _ = triton.testing.do_bench(
            lambda: torch.ops.aten.zeros_like.out(ref_inp, memory_format=memfmt, out=out_ref),
            rep=100,
            quantiles=quantiles
        )
