...
@pytest.mark.parametrize("padding", [0, 1])
@pytest.mark.parametrize("dtype", [torch.float32, torch.float16])
def test_accuracy_conv_transpose1d_groups(shape, kernel, groups, stride, padding, dtype):
    if flag_gems.vendor_name == "mthreads" and dtype == torch.float16:
        os.environ["MUSA_ENABLE_SQMMA"] = "1"
    ...
    ref_out = torch.nn.functional.conv_transpose1d(
        ref_inp, ref_weight, bias=None, stride=stride, padding=padding, dilation=1, groups=groups
    )

    res_out = flag_gems.conv_transpose1d(
        inp, weight, bias=None, stride=stride, padding=padding, dilation=1, groups=groups
    )
    ...
    kernel_width = kernel[2]
    gems_assert_close(res_out, ref_out, dtype, reduce_dim=kernel_width * in_channels_per_group)
