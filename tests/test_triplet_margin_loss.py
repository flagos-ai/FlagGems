...
def test_triplet_margin_loss(shape, dtype, margin, p, reduction, swap):
    ...
    # Use dtype and reduction-specific tolerances due to exp2/log2
    # approximation errors. swap=True adds extra distance computation.
    # For large D (dimension), fp16 accumulation has higher absolute error.
    N, D = shape
    if dtype == torch.float16:
        # Base tolerance for small shapes
        base_atol = 1.5e-2 if reduction == 0 else 5e-3
        # Scale linearly by D for large dimensions due to accumulation
        if D >= 128:
            base_atol *= D / 32
        atol = base_atol
    elif dtype == torch.bfloat16:
        base_atol = 2e-2 if reduction == 0 else 1e-2
        if D >= 128:
            base_atol *= D / 32
        atol = base_atol
    else:  # float32
