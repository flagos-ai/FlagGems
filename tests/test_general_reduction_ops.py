...
def test_accuracy_all_without_dim(shape, dtype, kind):
    if kind == "allTrue":
        inp = torch.ones(shape, dtype=dtype, device="cpu").to(flag_gems.device)
    else:
...
def test_accuracy_any_without_dim(shape, dtype, kind):
    if kind == "allFalse":
        inp = torch.zeros(shape, dtype=dtype, device="cpu").to(flag_gems.device)
    else:
        inp = torch.randint(0, 2, shape, dtype=dtype, device="cpu").to(flag_gems.device)
    ref_inp = to_reference(inp)
...
def test_accuracy_max_without_dim(shape, dtype):
    if dtype in FLOAT_DTYPES:
        ...
        inp = torch.randint(-10000, 10000, shape, dtype=dtype, device="cpu").to(flag_gems.device)
    ref_inp = to_reference(inp)
...
def test_accuracy_max_without_dim_all_neg_inf(shape, dtype):
    inp = torch.full(
        shape, fill_value=float("-inf"), dtype=dtype, device="cpu").to(flag_gems.device)
    ref_inp = to_reference(inp)
...
@pytest.mark.max
@pytest.mark.skipif(
    flag_gems.vendor_name != "cambricon" and flag_gems.vendor_name != "metax" and flag_gems.vendor_name != "tsingmicro",
...
def test_accuracy_max_without_dim_uncontiguous(shape, dtype):
    if dtype in FLOAT_DTYPES:
        ...
        inp = inp_.to(flag_gems.device)[::2, ::2]
    else:
        inp_ = torch.randint(-10000, 10000, shape, dtype=dtype, device="cpu")
        inp = inp_.to(flag_gems.device)[::2, ::2]

    # ref_inp = to_reference(inp)
    ref_inp = to_reference(inp_[::2, ::2])
...
def test_accuracy_quantile_dim(shape, dim, keepdim, dtype, q, interpolation):
    ...
    with flag_gems.use_gems():
        ...
        if res_out.is_contiguous() is False:
