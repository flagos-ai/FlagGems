...
# ---------------------------------------------------------------------------
# Ascend specialized grouped_matmul (torch_npu.npu_grouped_matmul baseline).
# Same math as group_mm above (jagged 2D x 3D grouped GEMM), but the optimized
# kernel targets Ascend and supports both group_list semantics (cumulative
# ends and per-group sizes).
# ---------------------------------------------------------------------------
...
GROUPED_MATMUL_CASES = (
    (GROUPED_MATMUL_FULL_CASES[0], GROUPED_MATMUL_FULL_CASES[5], GROUPED_MATMUL_FULL_CASES[7])
    if cfg.QUICK_MODE
...
def test_grouped_matmul_group_boundaries(
    ...
    weight = torch.randn((len(counts), 256, 256), device=flag_gems.device, dtype=torch.float16)
