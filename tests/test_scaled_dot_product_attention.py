...
# Non-power-of-2 head dims: kernel uses next_power_of_2 as SRAM block size
# and masks out the padding columns inside the kernel (no tensor padding).
NON_POWER_OF_2_HEAD_DIMS = [
...
@pytest.mark.scaled_dot_product_attention
@pytest.mark.parametrize("head_size", NON_POWER_OF_2_HEAD_DIMS)
...
def test_scaled_dot_product_attention_non_power_of_2_head_dim_forward(
    head_size, is_causal, dtype
):
    """Forward pass with non-power-of-2 head dimensions.

    The kernel pads HEAD_DIM to the next power of 2 as the SRAM block size
    and uses a mask to zero out the padding columns; no tensor padding is done.
    """
    batch, num_head, q_seq_len, kv_seq_len = 2, 4, 512, 512
    device = torch_device_fn.current_device()
...
def test_scaled_dot_product_attention_non_power_of_2_head_dim_backward(
    head_size, is_causal, dtype
):
    """Backward pass with non-power-of-2 head dimensions.

    Verifies that dQ, dK, dV are computed correctly when the kernel internally
    pads HEAD_DIM to the next power of 2 and masks the padding columns.
    """
    batch, num_head, q_seq_len, kv_seq_len = 2, 4, 512, 512
    device = torch_device_fn.current_device()
