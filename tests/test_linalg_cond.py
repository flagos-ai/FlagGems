...
# Condition number involves SVD which amplifies numerical error;
# use relaxed tolerance (relative ~0.1% for typical results ~O(100)).
LINALG_COND_ATOL = 1.0
...
def test_linalg_cond(shape, dtype):
    ...
    utils.gems_assert_close(res_out, ref_out, dtype, atol=LINALG_COND_ATOL)
