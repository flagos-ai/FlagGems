...
def test_upsample_nearest_exact1d_backward_uint8():
    ...
    try:
        ...
        # Some builds (incl. the vendor XPU build used on CI) have no native
        # uint8 kernel for this backward op, so there is nothing to compare
        # against; the differential check needs a working reference.
        pytest.skip(f"native backward has no uint8 support on this build: {exc}")
