...
if cfg.QUICK_MODE:
    ...
    NLL_LOSS_ND_SHAPES = [(2, 3, 4, 5)]
else:
    FLOAT_DTYPES = utils.FLOAT_DTYPES
    NLL_LOSS_ND_SHAPES = [(2, 32), (16, 50, 128), (4, 3, 5, 5), (2, 10, 8, 16, 16)]
...
@pytest.mark.parametrize("dtype", FLOAT_DTYPES)
@pytest.mark.parametrize("ignore_index", [1, 200, -100])
def test_nll_loss_nd_forward_accuracy(shape, dtype, ignore_index, reduction, weight):
    if flag_gems.vendor_name == "kunlunxin":
        torch.manual_seed(0)
        torch.cuda.manual_seed_all(0)
        np.random.seed(0)
