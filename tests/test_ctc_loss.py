...
def _make_targets(
    batch, max_target, classes, blank, layout, *, repeated=False
):
    target_lengths = torch.empty(
        batch, device=flag_gems.device, dtype=torch.long
    )
    padded = torch.zeros(
        batch, max_target, device=flag_gems.device, dtype=torch.long
    )
    pieces = []
    ...
    for row in range(batch):
        ...
        current = torch.tensor(
            row_values, device=flag_gems.device, dtype=torch.long
        )
...
def _reference(log_probs, targets, input_lengths, target_lengths, **kwargs):
    ref_log_probs = utils.to_reference(log_probs.detach(), False).to(
        torch.float32
    )
...
def _assert_forward_backward(
    ...
    reduce_dim = max(
        1,
        log_probs.shape[0]
        * int(torch.as_tensor(target_lengths).max().item() + 1),
    )
...
def test_ctc_loss_matches_pytorch(dtype, layout, reduction):
    ...
    targets, target_lengths = _make_targets(
        batch, max_target, classes, 0, layout
    )
    input_lengths = torch.tensor(
        [12, 10, 9], device=flag_gems.device, dtype=torch.long
    )
...
def test_ctc_loss_nonzero_blank_repeated_and_zero_infinity(dtype):
    ...
    targets, target_lengths = _make_targets(
        2, 4, 6, 2, "padded", repeated=True
    )
    input_lengths = torch.tensor(
        [2, 2], device=flag_gems.device, dtype=torch.long
    )
    _assert_forward_backward(
        log_probs,
        targets,
...
def test_ctc_loss_unbatched_and_intlist_registered_paths():
    ...
    targets = torch.tensor(
        [1, 3, 4], device=flag_gems.device, dtype=torch.long
    )
...
def test_ctc_loss_registered_tensor_backward_path():
    ...
    input_lengths = torch.tensor(
        [9, 8], device=flag_gems.device, dtype=torch.long
    )
    ...
    (ref_grad,) = torch.autograd.grad(
        ref_out, ref_log_probs, grad.to(ref_out.dtype)
    )
    (res_grad,) = torch.autograd.grad(res_out, log_probs, grad)
...
def test_ctc_loss_invalid_lengths_raise():
    ...
    targets = torch.tensor(
        [[1, 2], [2, 3]], device=flag_gems.device, dtype=torch.long
    )
    ...
    with pytest.raises(RuntimeError):
        flag_gems.ctc_loss(
            log_probs, targets, bad_input_lengths, target_lengths
        )
