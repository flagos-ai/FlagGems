...
def test_accuracy_randn(shape, dtype):
    ...
    mean = torch.mean(to_reference(res_out))
    std = torch.std(to_reference(res_out))
...
def test_accuracy_rand_like(shape, dtype):
    x = torch.randn(size=shape, dtype=dtype).to(flag_gems.device)
    with flag_gems.use_gems():
...
def test_accuracy_zeros_like(shape, dtype):
    x_ = torch.empty(size=shape, dtype=dtype, device="cpu" if TO_CPU else device)
    if TO_CPU:
        x = x_.to(flag_gems.device)
    else:
        x = x_
    ...
    ref_out = torch.zeros_like(ref_x)
    gems_assert_equal(res_out, ref_out)
...
def test_accuracy_full_like(shape, dtype, xdtype, fill_value):
    x_ = torch.empty(size=shape, dtype=xdtype, device="cpu" if TO_CPU else device)
    if TO_CPU:
        x = x_.to(flag_gems.device)
    else:
        x = x_
    ...
    ref_x = to_reference(x)
    gems_assert_equal(res_out, torch.full_like(ref_x, fill_value))

    # with dtype
    with flag_gems.use_gems():
        res_out = torch.full_like(x, fill_value, dtype=dtype)
    gems_assert_equal(res_out, torch.full_like(ref_x, fill_value, dtype=dtype))
