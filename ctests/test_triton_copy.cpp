...
TEST(CopyTest, DifferentDtypes) {
  const torch::Device device(torch::kCUDA, 0);

  auto check_dtype = [&](at::ScalarType src_dtype, at::ScalarType dst_dtype) {
    torch::Tensor src = torch::randn({4, 4}, torch::TensorOptions().device(device).dtype(src_dtype));
    torch::Tensor dst = torch::empty({4, 4}, torch::TensorOptions().device(device).dtype(dst_dtype));

    flag_gems::copy_(dst, src);

    torch::Tensor ref = src.to(dst_dtype);
    EXPECT_TRUE(torch::allclose(dst, ref));
  };

  check_dtype(torch::kFloat16, torch::kFloat32);
  check_dtype(torch::kFloat32, torch::kFloat64);
  check_dtype(torch::kInt32, torch::kInt64);
}