"""Run from inside FlagGems folder to write correct layernorm.py"""
...
def layer_norm(
    ...
    layer_norm_kernel[(num_rows,)](
        out, x2d, weight, bias, n_elements, eps,
...
def layer_norm_backward(
    ...
    layer_norm_backward_kernel[(num_rows,)](
        dx, dy2d, x2d, w, mean, rstd, n_elements,
        BLOCK_SIZE=BLOCK_SIZE, num_warps=num_warps,
    )
...
import os
path = os.path.join("src", "flag_gems", "ops", "layernorm.py")
...
print("Has layer_norm:          ", "layer_norm"          in funcs)
print("Has layer_norm_backward: ", "layer_norm_backward" in funcs)
print("DONE")
