...
# -*- coding: utf-8 -*-
"""
Read qcgem_w8a16_data.csv / qcgem_w4a16_data.csv from FlagGems benchmark,
write editable Markdown + Word summary (H20 QC-GEM vs PyTorch FP16 GEMM).
Layout mirrors benchmark/qcmoe_analysis/export_qcmoe_terminal_tables.py.
"""
...
DEFAULT_FLAGGEMS = ZH_ROOT.parent / "FlagEnv" / "FlagGems" / "benchmark" / "qcgem_analysis"
...
def write_docx(rows_w8: list[dict], rows_w4: list[dict], out: Path) -> None:
    ...
    doc = Document()
    h = doc.add_heading("QC-GEM vs PyTorch FP16 GEMM — Benchmark Summary (H20 GPU)", level=1)
    h.alignment = WD_ALIGN_PARAGRAPH.CENTER

    doc.add_paragraph(
        "环境（示例）：NVIDIA H20 (sm_90)，PyTorch 2.10.0+cu128，Triton 3.6.0。"
        " 对比：QC-GEM 量化 GEMM 内核 vs PyTorch FP16 GEMM。"
    )

    cols = [
        "#", "Category", "Shape (M,N,K)",
        "QC (ms)", "FP16 ref (ms)",
        "QC TFLOPS", "FP16 TFLOPS",
        "Speedup", "MaxAbsErr",
    ]
...
def main() -> None:
    ...
    if env:
        root = Path(env)
    elif (ZH_ROOT / "qcgem_w8a16_data.csv").is_file():
        root = ZH_ROOT
    else:
        root = DEFAULT_FLAGGEMS
    p8 = root / "qcgem_w8a16_data.csv"
    p4 = root / "qcgem_w4a16_data.csv"
    ...
    md = ZH_ROOT / "qc_gem_vs_pytorch_fp16_benchmark_summary.md"
    dx = ZH_ROOT / "qc_gem_vs_pytorch_fp16_benchmark_summary.docx"
