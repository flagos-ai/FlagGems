...
# -*- coding: utf-8 -*-
"""
QC-MoE W4A16 Benchmark — Plotting Script
=========================================
Reads qcmoe_w4a16_data.csv and generates PNG + PDF charts.

Usage:
    python plot_results_w4a16.py

Output:
    qcmoe_w4a16_benchmark.png
    qcmoe_w4a16_benchmark.pdf
"""
...
CSV_PATH   = os.path.join(SCRIPT_DIR, "qcmoe_w4a16_data.csv")
...
def plot(data):
    ...
    fig.suptitle(
        "QC-MoE W4A16 vs vLLM-style FP16 MoE",
        fontsize=16, fontweight="bold", color="#e8e8f0", y=0.98,
    )

    # ── 1. Latency ────────────────────────────────────────────────────────
    ax = axes[0, 0]
    ax.bar(x - w/2, qc_ms,   width=w, label="QC W4A16",  color="#62d17a", edgecolor="none")
    ...
    # ── 3. TFLOPS ─────────────────────────────────────────────────────────
    ax = axes[1, 0]
    ax.bar(x - w/2, qc_tflops,   width=w, label="QC W4A16",  color="#62d17a", edgecolor="none")
    ...
    fig.text(
        0.5, 0.01,
        "FlagGems QC-MoE W4A16 Kernel Benchmark  ·  "
...
if __name__ == "__main__":
    ...
    out_png = os.path.join(SCRIPT_DIR, "qcmoe_w4a16_benchmark.png")
    out_pdf = os.path.join(SCRIPT_DIR, "qcmoe_w4a16_benchmark.pdf")
