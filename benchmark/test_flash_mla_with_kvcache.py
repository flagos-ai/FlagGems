...
def _make_kv_cache(n_tokens, block_size, dev):
    ...
    cuda_fn(
        q_dummy, kv_data, k_cache, slot_mapping,
        pos, cos_sin, 1e-6, block_size,
    )
    return k_cache


def _cuda_ref_fn(
    q, k_cache_4d, indices, topk_length, attn_sink,
    softmax_scale, extra_k_cache_4d, extra_indices, extra_topk_length,
):
    ...
    return _cuda_flash_mla(
        ...
        head_dim_v=512, tile_scheduler_metadata=sched_meta,
        softmax_scale=softmax_scale, causal=False, is_fp8_kvcache=True,
        indices=indices, attn_sink=attn_sink,
...
class FlashMLAWithKVCacheBenchmark(base.Benchmark):
    ...
    def get_input_iter(self, dtype):
        for B, swa_topk, extra_topk, swa_bs, extra_bs in self.shapes:
            ...
            q = torch.randn(
                B, 1, _H_Q, _HEAD_DIM, device=device, dtype=torch.bfloat16
            ) * 0.1
            swa_cache = _make_kv_cache(swa_tokens, swa_bs, device)
            swa_indices = torch.stack([
                torch.randperm(
                    swa_tokens, device=device, dtype=torch.int32
                )[:swa_topk]
                for _ in range(B)
            ]).unsqueeze(1)
            swa_lens = torch.full(
                (B,), swa_topk, device=device, dtype=torch.int32
            )
            ...
            if extra_topk > 0:
                ...
                extra_cache_4d = extra_cache.view(
                    -1, extra_bs, 1, _HEAD_BYTES
                )
            ...
            yield (
                q, swa_cache, swa_cache_4d, swa_indices, swa_lens,
                attn_sink, softmax_scale,
                extra_cache, extra_cache_4d, extra_indices, extra_lens,
            )
    ...
    def get_gems_op(self):
        def triton_op(
            ...
            return flash_mla_with_kvcache(
                q=q, k_cache=swa_cache, block_table=None,
                cache_seqlens=None, head_dim_v=512,
    ...
    def get_torch_op(self):
        ...
        def cuda_op(
            ...
            return _cuda_ref_fn(
                q, swa_cache_4d, swa_indices, swa_lens,
                attn_sink, softmax_scale,
                extra_cache_4d, extra_indices, extra_lens,
            )
