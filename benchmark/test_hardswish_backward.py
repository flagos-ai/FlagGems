...
class HardswishBackwardBenchmark(base.Benchmark):
    def get_input_iter(self, dtype: torch.dtype) -> Generator:
        for shape in self.shapes:
            input = utils.generate_tensor_input(
                shape, dtype, self.device, requires_grad=True
            )
            yield input
...
def test_hardswish_backward():
    bench = HardswishBackwardBenchmark(
        ...
        torch_op=lambda x: torch.ops.aten.hardswish(x).backward(
            torch.ones_like(torch.ops.aten.hardswish(x))
        ),
