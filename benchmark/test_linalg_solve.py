...
def _solve_input_fn(batch, n, k, dtype, device):
...
class LinalgSolveBenchmark(base.Benchmark):
    def set_shapes(self, shape_file_path=None):
        self.shapes = SOLVE_SHAPES

    def get_input_iter(self, cur_dtype):
        for shape in self.shapes:
            batch, n, k = shape
            A, B = _solve_input_fn(batch, n, k, cur_dtype, self.device)
            yield A, B


@pytest.mark.linalg_solve
def test_linalg_solve():
    bench = LinalgSolveBenchmark(
        op_name="linalg_solve",
        torch_op=torch.linalg.solve,
        dtypes=SOLVE_DTYPES,
    )
    bench.run()
