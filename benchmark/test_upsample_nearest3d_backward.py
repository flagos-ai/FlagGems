    ...
    def get_input_iter(self, dtype: torch.dtype) -> Generator:
        for input_size, output_size in self.shapes:
            input = utils.generate_tensor_input(
                input_size, dtype, self.device, requires_grad=True
            )
...
def test_upsample_nearest3d_backward():
    bench = UpsampleNearest3DBackwardBenchmark(
        ...
        torch_op=lambda x, sz: torch.ops.aten.upsample_nearest3d(x, sz).backward(
            torch.ones_like(torch.ops.aten.upsample_nearest3d(x, sz))
        ),
