...
# -*- coding: utf-8 -*-
"""
QC-GEM W8A16 Performance Analysis - Dark Theme
================================================
Mimics the benchmark_w8a16_fp16.png style with dark theme.

Usage:
    python plot_results.py

Output:
    qcgem1_benchmark.png
    qcgem1_benchmark.pdf
"""
...
data = [
    (32768, 3584, 1024, 2.560480, 2.564400, 0.998, 93.791, 0.119, 0.119),
    (16384, 3584, 1024, 1.400416, 1.391984, 1.006, 86.394, 0.110, 0.111),
    (8192, 3584, 1024, 0.815424, 0.811264, 1.005, 74.118, 0.097, 0.098),
    (4096, 3584, 1024, 0.510176, 0.502720, 1.015, 59.804, 0.081, 0.083),
    (2048, 3584, 1024, 0.354688, 0.348608, 1.017, 43.121, 0.064, 0.065),
    (1024, 3584, 1024, 0.277248, 0.313424, 0.885, 23.981, 0.047, 0.042),
    (512, 3584, 1024, 0.235104, 0.279296, 0.842, 13.456, 0.036, 0.030),
    (256, 3584, 1024, 0.219392, 0.261328, 0.840, 7.190, 0.028, 0.023),
    (32768, 1024, 3584, 2.637664, 2.619440, 1.007, 91.820, 0.116, 0.117),
    (16384, 1024, 3584, 1.406016, 1.396032, 1.007, 86.144, 0.110, 0.111),
    (8192, 1024, 3584, 0.797296, 0.789568, 1.010, 76.155, 0.099, 0.100),
    (4096, 1024, 3584, 0.490720, 0.483008, 1.016, 62.245, 0.085, 0.086),
    (2048, 1024, 3584, 0.345712, 0.338912, 1.020, 44.355, 0.065, 0.067),
    (32768, 1024, 7168, 5.226288, 5.198688, 1.005, 92.530, 0.104, 0.105),
    (16384, 1024, 7168, 2.754464, 2.754240, 1.000, 87.327, 0.100, 0.100),
    (8192, 1024, 7168, 1.548672, 1.540960, 1.005, 78.042, 0.091, 0.092),
    (4096, 1024, 7168, 0.943376, 0.934080, 1.010, 64.373, 0.079, 0.080),
    (2048, 1024, 7168, 0.636144, 0.630864, 1.008, 47.657, 0.064, 0.065),
    (32768, 1024, 128, 0.186880, 0.303936, 0.615, 28.262, 0.405, 0.249),
    (16384, 1024, 128, 0.187488, 0.274464, 0.683, 15.649, 0.202, 0.138),
    (4096, 1024, 128, 0.182944, 0.238272, 0.768, 4.506, 0.052, 0.040),
]
...
xlabels = [f"M={m//1024}k\nN={n//1024}k\nK={k}" for m, n, k in zip(M, N, K)]
n = len(data)
x = np.arange(n)
w_single = 0.5
w_half = 0.22
...
def plot_benchmark():
    ...
    fig.suptitle(
        "QC-GEM W8A16 vs PyTorch FP16 GEMM — Comprehensive Shapes (H20 GPU)",
    ...
    ax.bar(x + w_half/2, gems_ms,   width=w_half, label="QC W8A16",   color=PALETTE["Gems"],   edgecolor="none")
    ...
    ax.set_title("TFLOPS — QC W8A16", fontsize=11, fontweight="bold", color="#7a7a9a")
    ax.set_xticks(x)
    ax.set_xticklabels([f"M={m//1024}k" for m in M], rotation=45, ha="right", fontsize=7.5)
    ...
    fig.text(
        0.5, 0.003,
        "FlagGems QC-GEM W8A16 Kernel  ·  FP16 reference = PyTorch GEMM  ·  GPU = NVIDIA H20",
...
if __name__ == "__main__":
    ...
    out_png = SCRIPT_DIR / "qcgem1_benchmark.png"
    out_pdf = SCRIPT_DIR / "qcgem1_benchmark.pdf"
    ...
    summary = f"""
================================================================================
QC GEM W8A16 Performance Summary (dtype=torch.float16)
================================================================================
...
"""
    ...
    with open(SCRIPT_DIR / 'qcgem1_summary.txt', 'w') as f:
        f.write(summary)
    print(f"[OK] Summary → {SCRIPT_DIR / 'qcgem1_summary.txt'}")
