...
class ConcatAndCacheMlaBenchmark(Benchmark):
    """
    ...
    def get_input_iter(self, cur_dtype):
        for config in self.shapes:
            yield from self.concat_and_cache_mla_input_fn(
                config, cur_dtype, self.device
            )

    def concat_and_cache_mla_input_fn(self, config, dtype, device):
        num_tokens, kv_lora_rank, pe_dim, block_size, num_blocks = config
        kv_c = torch.randn(num_tokens, kv_lora_rank, device=device, dtype=dtype)
        k_pe = torch.randn(num_tokens, pe_dim, device=device, dtype=dtype)
        ...
        for kv_cache_dtype in ("auto", "fp8e4m3", "fp8e5m2"):
            cache_dtype = torch.uint8 if kv_cache_dtype != "auto" else dtype
            ...
            yield kv_c, k_pe, kv_cache, slot_mapping, {
...
def test_perf_concat_and_cache_mla():
    def _convert_fp8_uint8(src: torch.Tensor, scale: torch.Tensor, kv_dtype: str):
        if kv_dtype == "fp8e4m3":
            fp8_dtype = torch.float8_e4m3fn
        elif kv_dtype == "fp8e5m2":
            fp8_dtype = torch.float8_e5m2
        else:
            raise ValueError(f"Unsupported kv_cache_dtype: {kv_dtype}")
        return (src / scale).to(fp8_dtype).view(torch.uint8)

    def torch_op(kv_c, k_pe, kv_cache, slot_mapping, *, kv_cache_dtype, scale):
        ...
        if kv_cache_dtype == "auto":
            kv_cache[block_id, block_offset, :kv_lora_rank] = kv_c[valid]
            kv_cache[block_id, block_offset, kv_lora_rank:] = k_pe[valid]
        else:
            scale_val = scale.to(kv_c.dtype)
            kv_c_q = _convert_fp8_uint8(kv_c[valid], scale_val, kv_cache_dtype)
            k_pe_q = _convert_fp8_uint8(k_pe[valid], scale_val, kv_cache_dtype)
            kv_cache[block_id, block_offset, :kv_lora_rank] = kv_c_q
            kv_cache[block_id, block_offset, kv_lora_rank:] = k_pe_q
        return kv_cache
...
class ReshapeAndCacheBenchmark(Benchmark):
    """
    ...
    def get_input_iter(self, cur_dtype):
        for config in self.shapes:
            yield from self.reshape_and_cache_input_fn(config, cur_dtype, self.device)

    def reshape_and_cache_input_fn(self, config, dtype, device):
        num_tokens, num_heads, head_size, block_size, num_blocks = config
        ...
        yield key, value, key_cache, value_cache, slot_mapping, {
            "kv_cache_dtype": "auto",
            "k_scale": k_scale,
            "v_scale": v_scale,
        }
...
class ReshapeAndCacheFlashBenchmark(Benchmark):
    """
    ...
    def get_input_iter(self, cur_dtype):
        for config in self.shapes:
            yield from self.reshape_and_cache_flash_input_fn(
                config, cur_dtype, self.device
            )

    def reshape_and_cache_flash_input_fn(self, config, dtype, device):
        num_tokens, num_heads, head_size, block_size, num_blocks = config
        num_slots = block_size * num_blocks
        slot_mapping = torch.tensor(
            random.sample(range(num_slots), num_tokens),
