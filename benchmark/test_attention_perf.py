...
class ScaledDotProductAttentionMathBenchmark(GenericBenchmark):
    """
    Benchmark for _scaled_dot_product_attention_math with smaller shapes
    """

    def set_more_shapes(self):
        # Use smaller shapes to avoid OOM
...
@pytest.mark.parametrize("is_causal", [True, False])
def test_perf_scaled_dot_product_attention_math(is_causal):
    """Benchmark for _scaled_dot_product_attention_math"""
    if flag_gems.vendor_name == "hygon":
        os.environ["TRITON_HIP_USE_NEW_STREAM_PIPELINE"] = "0"
