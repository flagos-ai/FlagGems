...
def test_fused_adagrad():
    def gems_op(param, grad, state_sum, state_step):
        out_params, _, _, _ = flag_gems._fused_adagrad(
            [param],
            [grad],
            [state_sum],
            [state_step],
    ...
    def torch_op_out(param, grad, state_sum, state_step):
        out_params, _, _, _ = torch.ops.aten._fused_adagrad(
            [param],
            [grad],
            [state_sum],
            [state_step],
    ...
    bench = FusedAdagradBenchmark(
        input_fn=fused_adagrad_input_fn,
        op_name="fused_adagrad",
        torch_op=torch_op_out,
        # _fused_adagrad only supports float32 for optimizer state precision
        dtypes=[torch.float32],
    )
    bench.set_gems(gems_op)
